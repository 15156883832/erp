<%@ page contentType="text/html;charset=UTF-8"%>
<%@ include file="/WEB-INF/views/include/taglib.jsp"%>
<html>
<head>
<meta charset="utf-8">
<meta name="renderer" content="webkit|ie-comp|ie-stand">
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
	<meta name="viewport" content="width=device-width,initial-scale=0.3,user-scalable=yes" />
<meta http-equiv="Cache-Control" content="no-siteapp" />
<link rel="Bookmark" href="${ctxPlugin}/favicon.ico" >
<link rel="Shortcut Icon" href="${ctxPlugin}/favicon.ico" />
<!--[if lt IE 9]>
<script type="text/javascript" src="${ctxPlugin}/lib/html5shiv.js"></script>
<script type="text/javascript" src="${ctxPlugin}/lib/respond.min.js"></script>
<![endif]-->
<link rel="stylesheet" type="text/css" href="${ctxPlugin}/static/h-ui/css/H-ui.css" />
<link rel="stylesheet" type="text/css" href="${ctxPlugin}/lib/Hui-iconfont/1.0.8/iconfont.css" />
<link rel="stylesheet" type="text/css" href="${ctxPlugin}/static/h-ui.admin/css/helpStyle.css" />
<!--[if IE 6]>
<script type="text/javascript" src="${ctxPlugin}/lib/DD_belatedPNG_0.0.8a-min.js" ></script>
<script>DD_belatedPNG.fix('*');</script>
<![endif]-->
<title>思方ERP帮助中心——商品销售</title>
<meta name="keywords" content="思方海尔,Haier,思方">
<meta name="description" content="思方海尔">
<style type="text/css">
	body{
		background:#fff;
	}
</style>
</head>
<body >
	
	<h1 class="jpageTitle">商品销售</h1>
	<h2 class="partTitle">电脑端商品的零售</h2>
	<div class="partContainer">
		<div class="partNote mb-15">当有客户到网点购买商品时，商品管理员可通过电脑端的【零售】功能对商品直接零售。</div>
		<img src="${ctxPlugin}/static/h-ui.admin/images/help_v2/41商品管理_商品销售.png" class="img" />
	</div>
	<h2 class="partTitle">服务工程师APP端的商品零售</h2>
	<div class="partContainer">
		<div class="partNote mb-15">新增商品后服务工程师在APP端即可向服务商展示商品及商品销售了。</div>
		<div class="cl mb-20 pos-r">
			<img src="${ctxPlugin}/static/h-ui.admin/images/help_v2/41商品管理_商品销售02.png" class="f-l" />
			<img src="${ctxPlugin}/static/h-ui.admin/images/help_v2/41商品管理_商品销售03.png" class="f-l ml-15" />
	<!-- 	</div>
		<div class="cl mb-20 pos-r"> -->
			<img src="${ctxPlugin}/static/h-ui.admin/images/help_v2/41商品管理_商品销售04.png" class="f-l" />
			<img src="${ctxPlugin}/static/h-ui.admin/images/help_v2/41商品管理_商品销售05.png" class="f-l ml-15" />
		</div>
		
	</div>
	

<!--_footer 作为公共模版分离出去-->
<script type="text/javascript" src="${ctxPlugin}/lib/jquery/1.9.1/jquery.min.js"></script>
<script src="${ctxPlugin}/static/h-ui.admin/js/sifang.js" type="text/javascript" charset="utf-8"></script>
<!--/_footer 作为公共模版分离出去-->

<!--请在下方写此页面业务相关的脚本-->
<script type="text/javascript">
$(function(){

	$.setIframe();
});


</script> 

</body>
</html>