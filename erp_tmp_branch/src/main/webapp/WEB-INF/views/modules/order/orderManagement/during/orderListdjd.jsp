...
<div class="page-orderWait">
	<div id="tab-system" class="HuiTab">
		<div class="tabBar cl mb-10">
			...
			<sfTags:pagePermission authFlag="ORDERMGM_DURINGORDER_DJORDER_TAB" html='<a class="btn-tabBar current"  href="${ctx }/order/getWwgList/djd">待接工单<sup id="tab_c2">0</sup></a>'></sfTags:pagePermission>
		...
		<div class="tabCon">
						...
						<tr>
							...
							<td id="reloadSpan">
								<span class="w-140 dropdown-sin-2" >
									<select class="select-box w-120"  id="employs"  multiple name="employeNames" >
			...
			<div class="pt-10 pb-5 cl">
				<div class="f-l">
				<sfTags:pagePermission authFlag="ORDERMGM_DURINGORDER_DIORDER_ZPORDER_BTN" html='<a href="javascript:Turntosend();" class="sfbtn sfbtn-opt"><i class="sficon sficon-turnorder"></i>转派</a>'></sfTags:pagePermission>
				<sfTags:pagePermission authFlag="ORDERMGM_DURINGORDER_DJORDER_WXORDER_BTN" html='<a href="javascript:showwxgd();" class="sfbtn sfbtn-opt"><i class="sficon sficon-invalid"></i>无效工单</a>'></sfTags:pagePermission>
				<sfTags:pagePermission authFlag="ORDERMGM_DURINGORDER_DJORDER_NOTERECIEVE_BTN" html='<a href="javascript:noteRecieve();" class="sfbtn sfbtn-opt"><i class="sficon sficon-caution"></i>提醒接收</a>'></sfTags:pagePermission>
				...
				<div class="f-r">
					...
					<%-- <sfTags:pagePermission authFlag="ORDERMGM_DURINGORDER_DJORDER_IMPORT_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt2"><i class="sficon sficon-import"></i>导入</a>'></sfTags:pagePermission> --%>
					<sfTags:pagePermission authFlag="ORDERMGM_DURINGORDER_DJORDER_EXPORT_BTN" html='<a onclick="return exports()"class="sfbtn sfbtn-opt2" target="_blank"><i class="sficon sficon-export"></i>导出</a>'></sfTags:pagePermission>
					<sfTags:pagePermission authFlag="ORDERMGM_DURINGORDER_DJORDER_TBSETTLE_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt2" id="setHeadersBtn"><i class="sficon sficon-setting"></i>表头设置</a>'></sfTags:pagePermission>
				</div>
...
$(".resetSearchBtn").on("click",function(){
    ...
    html += '<select class="select-box w-120"  id="employs" multiple  multiline="true" name="employeNames"  >';
...
function noteRecieve(){
	var idArr=$('#table-waitdispatch').jqGrid('getGridParam','selarrrow');
	if(idArr.length < 1){layer.msg("请至少选择一条工单后提醒接单。");return;}
	var idNums = "";
	for(var i = 0 ; i < idArr.length; i++){
		var rowData = $('#table-waitdispatch').jqGrid('getRowData', idArr[i]);
		idNums += "," + idArr[i] + "@" + rowData.number;
	}
	$.post("${ctx}/commonAjax/notifyOrder", {ids: idNums, notifyType:"1"}, function(result){
		layer.msg("已发送提醒接单信息至服务工程师APP!");
	});
}

function initSfGrid(){
	$("#table-waitdispatch").sfGrid({
		url : '${ctx}/order/getOrderMenuList/djd?ptype='+${ptype},
   ...
   /*  if (nowM >= start && nowM <= end) {
	...
	if(idArr>10000){
		$('body').popup({
			...
			content:content,
			 fnConfirm :function(){
				 location.href="${ctx}/order/export?formPath=/a/order/getWwgList/djd&&maps="+$("#searchForm").serialize(); 
			 },
			fnCancel: function () {
