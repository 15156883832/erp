			...
			<div class="page-orderWait">
				<div id="tab-system" class="HuiTab">
					<div class="tabBar cl mb-10">
						...
						<c:if test="${map.haier eq 'has'}">
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HAIERORDER_TAB"
								html='<a id="haier" class="btn-tabBar current"  href="${ctx }/order/ChangeSelfOrder/haierHeaderList">海尔工单</a>'></sfTags:pagePermission>
						</c:if>
						...
						<form id="searchForm">
							...
							<div class="bk-gray pt-10 pb-5">
										...
										<td>
											<span class="w-140">
												<select class="select easyui-combobox" id="statusFlag" multiple="true" multiline="false" name="status" style="width: 100%; height: 25px" panelMaxHeight="300px">
													...
													<option value="已到商">已到商</option>
													<option value="已到兵">已到兵</option>
													<option value="已响应">已响应</option>
													<option value="已预约">已预约</option>
													<option value="处理中">处理中</option>
													<option value="已完工">已完工</option>
													<option value="未完工">未完工</option>
													<option value="已强制结单">已强制结单</option>
													<option value="服务商已结单">服务商已结单</option>
													<option value="海尔已结单">海尔已结单</option>
													<option value="已作废">已作废</option>
													<option value="审核中">审核中</option>
													<option value="已鉴定待结单">已鉴定待结单</option>
												</select>
											</span>
										</td>
										<!-- <td id="reloadSpan">
								...
								<select class="select" name="status" id="status" multiple>
										...
										<option value="结算通过">结算通过</option>
										<option value="已人力调配">已人力调配</option>
										<option value="已派单">已派单</option>
										<option value="费用已生成">费用已生成</option>
										<option value="异常结算">异常结算</option>
										<option value="网点结算">网点结算</option>
				...
				<div class="col-5-1 ordertj-item" onclick="warningOrder(4)" style="cursor:pointer;">
					<div class="bk-gray">
						<div class="ordertxt1">
							<p class="bg bg41 "></p>
							<p class="txt">超48h预警工单（<span class="f-18 c-f36a5a va-t" id="20hCount">0</span>）</p>
						</div>
					</div>
				</div>
				<div class="col-5-1 ordertj-item" onclick="warningOrder(5)" style="cursor:pointer;">
					<div class="bk-gray">
						<div class="ordertxt1">
							<p class="bg bg42"></p>
							<p class="txt">超72h预警工单（<span class="f-18 c-f36a5a va-t" id="24hCount">0</span>）</p>
						</div>
					</div>
				</div>
				--%>
						...
						<div class="pt-10 pb-5 cl">
							<div class="f-l">
								<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HUNDRANDORDERHAIER_ZHUANZIJIE_BTN"
									html='<a href="javascript:;" class="sfbtn sfbtn-opt" onclick="zzj();"><i class="sficon sficon-zzj"></i>转自接</a>'></sfTags:pagePermission>
								<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HUNDRANDORDERHAIER_DELETEMORE_BTN"
								...
								<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HUNDRANDORDERHAIER_PILIANGPRINT_BTN"
							...
							<div class="f-r">
								<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HUNDRANDORDERHAIER_EXPORT_BTN"
								...
								<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HUNDRANDORDERHAIER_HEADSET_BTN"
	...
	<script type="text/javascript">
		...
		function initSfGrid() {
			$("#table-waitdispatch").sfGrid({
				url : '${ctx}/order/ChangeSelfOrder/FourHaierOrder',
				sfHeader : defaultHeader,
				sfSortColumns : sortHeader,
				rownumbers : true,
		...
		function reset() {
			...
			html += '<option value="服务中">服务中</option>';
			html += '<option value="结算通过">结算通过</option>';
			html += '<option value="已人力调配">已人力调配</option>';
			html += '<option value="已派单">已派单</option>';
			html += '<option value="费用已生成">费用已生成</option>';
			html += '<option value="异常结算">异常结算</option>';
			html += '<option value="网点结算">网点结算</option>';
		...
		function exports() {
			...
			if (idArr > 10000) {
				$('body').popup({
					...
					content : content,
					fnConfirm : function() {
						location.href = "${ctx}/order/ChangeSelfOrder/exportHaier?formPath=/a/order/ChangeSelfOrder/haierHeaderList&&maps=" + $("#searchForm").serialize();
					}
				});
			} else {
				location.href = "${ctx}/order/ChangeSelfOrder/exportHaier?formPath=/a/order/ChangeSelfOrder/haierHeaderList&&maps=" + $("#searchForm").serialize();
