			...
			<form id="neworderForm"  action="" method="post" >
							...
							<label class="lb lb2"><em class="mark">*</em>服务类型：</label>
								...
								<select class="select w-110 mustfill" name="serviceType" id="serviceType" datatype="*" errormsg="请选择服务类型" nullmsg="请选择服务类型">
									...
									<c:forEach items="${fns:getNewServiceType() }" var="stype">
										<option value="${stype.columns.name }" ${stype.columns.name eq order.service_type ? 'selected': ''}>${stype.columns.name }</option>
									</c:forEach>
								</select>
						...
						<label class="lb lb2"><em class="mark">*</em>服务方式：</label>
							<select class="select w-100 mustfill" name="serviceMode" id="serviceMode" datatype="*" errormsg="请选择服务方式" nullmsg="请选择服务方式">
								...
								<c:forEach items="${fns:getNewServiceMode() }" var="stype">
									<option value="${stype.columns.name }" ${stype.columns.name eq order.service_mode ? 'selected': ''}>${stype.columns.name }</option>
								</c:forEach>
							</select>
				...
				<div class="pt-10 mb-15">
					<div class="cl">
						<div class="f-l pos-r txtwrap1 mb-10">
							...
							<input type="text" class="input-text w-140 mustfill" name="customerName" id="customerName" datatype="*" errormsg="用户姓名格式不正确" nullmsg="请输入用户姓名" value="${order.customer_name }"/>
		                    ...
		                    <select  style="border: none" class="select ${mustfill.columns.customerType?'mustfill':''}" <c:if test="${mustfill.columns.customerType}">datatype="*" nullmsg="请选择用户类型"</c:if>  id="customerType" name="customerType">
			                    ...
			                    <option value="${to.columns.name }" ${to.columns.name==order.customer_type ?'selected':''}>${to.columns.name }</option>
			                    </c:forEach>
		                    </select>
							</span>
							</c:if>
						...
						<div class="f-l pos-r txtwrap2">
							...
							<input type="text" class="input-text w-110 mustfill" name="customerMobile" id="customerMobile" datatype="tel" errormsg="手机号码格式不正确" nullmsg="请输入手机号码" value="${order.customer_mobile }"/>
							<p class="errorwran">请输入联系电话</p>
							</div>
						<div class="f-l pos-r txtwrap2">
							<label class="lb lb2">其他联系方式：</label>
							<input type="text" class="input-text w-110" id="customerTelephone" name="customerTelephone" ignore="ignore" datatype="dh" errormsg="联系方式2格式错误" value="${order.customer_telephone }"/>
							<p class="errorwran"></p>
						</div>
						<div class="f-l pos-r " style="padding-left: 15px;">
							<!-- <label class="lb lb2">联系方式3：</label> -->
							<input type="text" class="input-text w-110" id="customerTelephone2" name="customerTelephone2" ignore="ignore" datatype="dh" errormsg="联系方式3格式错误" value="${order.customer_telephone2 }"/>
							<p class="errorwran"></p>
						</div>
					</div>
					<div class="pos-r txtwrap1" >
						...
						<input type="text" class="input-text w-260 mustfill" id="customerAddress1" placeholder="详细地址" datatype="*" errormsg="详细地址格式错误" nullmsg="请输入详细地址" value="${order.customer_address}"/>
						<a class="btn-dw" onclick="curmaskAddr()" id="btn-showaddr"></a>
						<span class="ml-5 c-fe0101 hide" id="distancealert">距离xxx公里</span>
						<%-- display: inline-block --%>
						<input type="hidden" class="input-text w-260 mustfill" value="${order.customer_address}" name="customerAddress" id="customerAddress" placeholder="详细地址" datatype="*" errormsg="详细地址格式错误" ignore="ignore"/>
				...
				<div class="pt-10">
							...
							<label class="lb lb1"><em class="mark">*</em>家电品牌：</label>
								<select class="select w-140 mustfill" name="applianceBrand" id="applianceBrand" datatype="*" nullmsg="请选择家电品牌！" >
									...
									<c:forEach items="${brand }" var="ba" varStatus="sta">
									 <option value="${ba.key }" ${ba.key eq order.appliance_brand ? 'selected': ''}>${ba.value }</option>
								</c:forEach>
								</select>
						...
						<div class="f-l pos-r txtwrap2">
							<label class="lb lb2"><em class="mark">*</em>家电品类：</label>
								...
								<c:forEach items="${category }" var="ca" varStatus="cast">
									 <option value="${ca.columns.name }" ${ca.columns.name eq order.appliance_category ? 'selected': ''}>${ca.columns.name }</option>
								</c:forEach>
								</select>
					...
					<div class="cl mb-10">
						<div class="f-l pos-r txtwrap1 h-50">
							...
							<textarea type="text" class="input-text w-340 h-50 ${mustfill.columns.customerFeedback?'mustfill':''}" <c:if test="${mustfill.columns.customerFeedback}">datatype="*" nullmsg="请输入服务描述"</c:if> name="customerFeedback" id="customerFeedback" >${order.customer_feedback}</textarea>
						<p class="errorwran"></p>
						</div>
					...
					<div class="cl mb-10">
						<div class="f-l pos-r txtwrap1">
							...
							<input type="text" class="input-text w-140 ${mustfill.columns.applianceModel?'mustfill':''}" <c:if test="${mustfill.columns.applianceModel}">datatype="*" nullmsg="请输入产品型号"</c:if> id="applianceModel" name="applianceModel" value="${order.appliance_model}"/>
						</div>
						<div class="f-l pos-r txtwrap2">
							<label class="lb lb2"><c:if test="${mustfill.columns.applianceNum}"><em class="mark">*</em></c:if>产品数量：</label>
							<input type="text"  class="input-text w-110 ${mustfill.columns.applianceNum?'mustfill':''}" <c:if test="${mustfill.columns.applianceNum}">datatype="anum" errormsg="产品数量格式不正确"  nullmsg="请输入产品数量"</c:if> name="applianceNum" id="applianceNum" value="${order.appliance_num}"/>
						</div>
						<div class="f-l pos-r txtwrap2">
							<label class="lb lb2"><c:if test="${mustfill.columns.applianceBarcode}"><em class="mark">*</em></c:if>内机条码：</label>
							<input type="text" style="width:180px;" class="input-text  ${mustfill.columns.applianceBarcode?'mustfill':''}" <c:if test="${mustfill.columns.applianceBarcode}">datatype="*" nullmsg="请输入内机条码"</c:if> name="applianceBarcode" id="applianceBarcode" value="${order.appliance_barcode}"/>
							...
							<span class="va-t underline c-fe0101 codeConnectShow cPointer " preData="${order.appliance_barcode}"  id="codeInshow"></span >
						</div>
					...
					<div class="mb-10 cl">
						<div class="f-l pos-r txtwrap1">
							...
							<input type="text" onfocus="WdatePicker({maxDate: '%y-%M-%d' })"  id="applianceBuyTime" name="applianceBuyTime" class="input-text Wdate w-140 ${mustfill.columns.applianceBuyTime?'mustfill':''}" value="<fmt:formatDate value='${order.appliance_buy_time }' pattern='yyyy-MM-dd'/>" readonly>
						</div>
						<div class="f-l pos-r txtwrap2">
								...
								<c:when test="${not empty malllist}">
									...
									<select class="select ${mustfill.columns.pleaseReferMall?'mustfill':''}"  id="pleaseReferMall"  multiline="false" name="pleaseReferMall" style="width:100%;height:25px" panelMaxHeight="300px" <c:if test="${mustfill.columns.pleaseReferMall}">datatype="*" nullmsg="请选择购机商场"</c:if>>
										...
										<c:forEach items="${malllist }" var="mall">
											<c:if test="${mall.columns.mall_name eq order.please_refer_mall}"><c:set var="hadMall" value="1"></c:set></c:if>
											<option value="${mall.columns.mall_name } " ${order.please_refer_mall eq mall.columns.mall_name ?'selected':''}>${mall.columns.mall_name }</option>
										</c:forEach>
                                        <c:if test="${hadMall eq '0' and not empty order.please_refer_mall}"><option value="${order.please_refer_mall}" selected>${order.please_refer_mall}</option></c:if>
									</select>
								</span>
								</c:when>
								<c:otherwise>
									<label class="lb lb2"><c:if test="${mustfill.columns.pleaseReferMall}"><em class="mark">*</em></c:if>购机商场：</label>
									<input type="text" value="${order.please_refer_mall}" name="pleaseReferMall" class="input-text w-110 ${mustfill.columns.pleaseReferMall?'mustfill':''}" <c:if test="${mustfill.columns.pleaseReferMall}">datatype="*" nullmsg="请输入购机商场"</c:if> value="${order.please_refer_mall}">
								</c:otherwise>
							</c:choose>
						...
						<div class="f-l pos-r txtwrap2">
							...
							<input type="text" style="width:180px;" class="input-text  ${mustfill.columns.applianceMachineCode?'mustfill':''}" <c:if test="${mustfill.columns.applianceMachineCode}">datatype="*" nullmsg="请输入外机条码"</c:if> name="applianceMachineCode" id="applianceMachineCode" value="${order.appliance_machine_code}"/>
							...
							<span class="va-t underline c-fe0101 codeConnectShow cPointer" preData="${order.appliance_machine_code}"  id="codeOutshow"></span>
						</div>
						...
						<div class="f-l pos-r txtwrap1">
							<label class="lb lb1"><c:if test="${mustfill.columns.warrantyType}"><em class="mark">*</em></c:if>保修类型：</label>
								<select class="select w-140 ${mustfill.columns.warrantyType?'mustfill':''}" <c:if test="${mustfill.columns.warrantyType}">datatype="*" nullmsg="请选择保修类型"</c:if> id="warrantyType" name="warrantyType">
									<option value="">请选择</option>
									<option value="1" ${order.warranty_type eq '1' ? 'selected' : ''}>保内</option>
									<option value="2" ${order.warranty_type eq '2' ? 'selected' : ''}>保外</option>
								</select>
						</div>
						<div class="f-l pos-r txtwrap2">
				...
				<div class="mb-10 cl">
						...
						<div class="f-l pos-r txtwrap2 ">
							...
							<input type="text"  class="input-text  w-130 " id="distributionTime"   onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="distributionTime" value ="<fmt:formatDate value="${order.repair_time}" pattern="yyyy-MM-dd"/>" >
						</div>
						<div class="f-l pos-r pl-80">
