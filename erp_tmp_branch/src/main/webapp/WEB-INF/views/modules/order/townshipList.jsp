...
<div class="page-orderWait">
	...
	<%-- 	<sfTags:pagePermission authFlag="SYSSETTLE_ORDERMSGSET_MALTIONTYPE_TAB" html='<a class="btn-tabBar" href="${ctx}/order/malfunction">故障类型</a>'></sfTags:pagePermission>  --%>
			...
			<a class="btn-tabBar current" href="${ctx}/order/township">乡镇设置</a>
...
<script type="text/javascript">
	...
	$('#btn-add').click(function(){
		layer.open({
			...
			type : 2,
			content:'	${ctx}/order/township/form',
			title:false,
			area: ['100%','100%'],
			closeBtn:0,
			shade:0,
...
function initSfGrid(){
	sfGrid = $("#table-waitdispatch").sfGrid({
		url : '${ctx}/order/township/townshipList', 
...
function updateMsg(id){
	layer.open({
		type : 2,
		content:'${ctx}/order/township/edite?id='+id,
		title:false,
		area: ['100%','100%'],
		closeBtn:0,
		shade:0,
...
function showwxgd(){
	...
	if(idArr.length<1){layer.msg("请选择数据！");}else{
		var content = "确认要删除"+idArr.length+"条乡镇信息？";
