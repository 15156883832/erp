...
<body>
		...
		<div class="sfpage table-header-settable">
			<div class="page-orderWait">
				<div id="tab-system" class="HuiTab">
					<div class="tabBar cl mb-10">
						...
						<c:if test="${map.meidi eq  'has'}">
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HUNDRANDORDER_TAB"
								html='<a id="meidi" class="btn-tabBar current"  href="${ctx }/order/ChangeSelfOrder/headerList">美的工单</a>'></sfTags:pagePermission>
						</c:if>
						...
						<c:if test="${map.aokesi eq 'has'}">
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_AOKESIORDER_TAB"
								html='<a id="aokesi" class="btn-tabBar "  href="${ctx }/order/ChangeSelfOrder/aokesiHeaderList">奥克斯工单</a>'></sfTags:pagePermission>
						</c:if>
						...
						<c:if test="${map.meiling eq 'has'}">
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_MEILINGORDER_TAB"
								html='<a id="meiling" class="btn-tabBar "  href="${ctx }/order/ChangeSelfOrder/meilingHeaderList">美菱工单</a>'></sfTags:pagePermission>
						</c:if>
						...
						<c:if test="${map.guomei eq 'has'}">
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_GUOMEI_TAB" html='<a id="guomei" class="btn-tabBar  "  href="${ctx }/order/ChangeSelfOrder/guomeiHeaderList">国美工单</a>'></sfTags:pagePermission>
						</c:if>
						<c:if test="${map.jingdong eq 'has'}">
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_JINGDONG_TAB"
								html='<a id="jingdong" class="btn-tabBar  "  href="${ctx }/order/ChangeSelfOrder/jingdongHeaderList">京东工单</a>'></sfTags:pagePermission>
						</c:if>
						<p class="f-r btnWrap">
							...
							<a href="javascript:;" onclick="reset()" class="sfbtn sfbtn-opt resetSearchBtn">
					...
					<div class="tabCon">
							...
							<div class="bk-gray pt-10 pb-5">
								<table class="table table-search">
										...
										<td id="reloadSpan">
											...
											<span class="dropdown-sin-2 w-140">
												<select class="select" name="status" id="status" multiple>
													...
													<option value="用户签收">用户签收</option>
													<option value="用户拒签">用户拒签</option>
													<option value="接收机器">接收机器</option>
													<option value="抢单锁定中">抢单锁定中</option>
												</select>
											</span>
										</td>
									...
									<tr class="moreCondition" style="display: none;">
										...
										<td colspan="3">
											<input type="text" onfocus="WdatePicker({startDate:'%y-%M-%d 00:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'repairTimeMax\')||\'%y-%M-%d\'}'})"
												id="repairTimeMin" name="repairTimeMin" value="" class="input-text Wdate w-140" style="width: 140px" readonly>
											至
											<input type="text"
												onfocus="WdatePicker({startDate:'%y-%M-%d 23:59:59',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'repairTimeMin\')}',maxDate:'%y-%M-%d 23:59:59'})"
												id="repairTimeMax" name="repairTimeMax" value="" class="input-text Wdate w-140" style="width: 140px" readonly>
										</td>
										<th style="width: 76px;" class="text-r">预约时间：</th>
										<td colspan="3">
											<input type="text" onfocus="WdatePicker({startDate:'%y-%M-%d 00:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss'})" id="promiseTimeMin" name="promiseTimeMin" value=""
												class="input-text Wdate w-140" style="width: 140px" readonly>
											至
											<input type="text" onfocus="WdatePicker({startDate:'%y-%M-%d 23:59:59',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'promiseTimeMin\')}'})" id="promiseTimeMax"
												name="promiseTimeMax" value="" class="input-text Wdate w-140" style="width: 140px" readonly>
										</td>
									</tr>
									<tr class="moreCondition" style="display: none;">
										<th style="width: 76px;" class="text-r">信息来源：</th>
										<td id="reloadSourceSpan">
											<span class="w-140 dropdown-sin-2">
												<select class="select-box w-120" id="origin" style="display: none" multiple multiline="true" name="origin">
													<option value="服务热线">服务热线</option>
													<option value="网点自建">网点自建</option>
													<option value="分中心自建">分中心自建</option>
													<option value="用户自助微信">用户自助-微信</option>
													<option value="用户自助官网">用户自助-官网</option>
													<option value="计划提醒">计划提醒</option>
													<option value="物流平台">物流平台</option>
													<option value="投诉创建">投诉创建</option>
													<option value="网络接入">网络接入</option>
													<option value="邮件接入">邮件接入</option>
													<option value="舆情接入">舆情接入</option>
													<option value="电商接入">电商接入</option>
													<option value="安得电商接入">安得电商接入</option>
													<option value="国美接入">国美接入</option>
													<option value="京东接入">京东接入</option>
													<option value="网点鉴定改派">网点鉴定改派</option>
													<option value="总部自建">总部自建</option>
													<option value="天猫接入">天猫接入</option>
													<option value="经销商微信">经销商微信</option>
													<option value="菜鸟接入">菜鸟接入</option>
													<option value="奇门接入">奇门接入</option>
													<option value="手工单">手工单</option>
													<option value="美的通接入">美的通接入</option>
													<option value="苏宁">苏宁</option>
													<option value="项目监理派工">项目监理派工</option>
													<option value="美居App">美居App</option>
										...
										<td colspan="3">
											<input type="text" onfocus="WdatePicker({startDate:'%y-%M-%d 00:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'dispatchTimeMax\')||\'%y-%M-%d\'}'})"
												id="dispatchTimeMin" name="dispatchTimeMin" value="" class="input-text Wdate w-140" style="width: 140px" readonly>
											至
											<input type="text"
												onfocus="WdatePicker({startDate:'%y-%M-%d 23:59:59',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'dispatchTimeMin\')}',maxDate:'%y-%M-%d 23:59:59'})"
												id="dispatchTimeMax" name="dispatchTimeMax" value="" class="input-text Wdate w-140" style="width: 140px" readonly>
										</td>
										<th style="width: 76px;" class="text-r">完工时间：</th>
										<td colspan="3">
											<input type="text" onfocus="WdatePicker({startDate:'%y-%M-%d 00:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'endTimeMax\')||\'%y-%M-%d\'}'})"
												id="endTimeMin" name="endTimeMin" value="" class="input-text Wdate w-140" style="width: 140px" readonly>
											至
											<input type="text" onfocus="WdatePicker({startDate:'%y-%M-%d 23:59:59',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'endTimeMin\')}',maxDate:'%y-%M-%d 23:59:59'})"
												id="endTimeMax" name="endTimeMax" value="" class="input-text Wdate w-140" style="width: 140px" readonly>
										</td>
									</tr>
								</table>
							</div>
						...
						<div class="ordertjbox mt-10 cl">
							<div class="ordertj-item" style="cursor: pointer; width: 100%">
								<div class="orderNum_">
									<div class="ordertxt1 " onclick="warningOrder(1)">
										<p class="bg bg11"></p>
										<p class="txt">
											今日工单总数（
											<span class="f-18 c-0e8ee7 va-t" id="totalCounts">0</span>
											）
										</p>
									</div>
									<div class="ordertxt1" onclick="warningOrder(2)">
										...
										<p class="txt">
											待接收（
											<span class="f-18 c-0e8ee7 va-t" id="djsCount">0</span>
											）
										</p>
									</div>
									<div class="ordertxt1 " onclick="warningOrder(3)">
										...
										<p class="txt">
											未完工（
											<span class="f-18 c-4baf4b va-t" id="wwgCount">0</span>
											）
										</p>
									</div>
									<div class="ordertxt1" onclick="warningOrder(6)">
										...
										<p class="txt">
											超45分钟未预约（
											<span class="f-18 c-f36a5a va-t" id="45mintCount">0</span>
											）
										</p>
									</div>
				...
				<div class="col-4-1 ordertj-item"  style="cursor:pointer;">
					<div class="bk-gray">
						<div class="ordertxt1 " onclick="warningOrder(7)">
							<p class="bg bg62"></p>
				...
				--%>
						...
						<div class="pt-10 pb-5 cl">
							<div class="f-l">
								<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HUNDRANDORDER_ZHUANZIJIE_BTN"
									html='<a href="javascript:;" class="sfbtn sfbtn-opt" onclick="zzj();"><i class="sficon sficon-zzj"></i>转自接</a>'></sfTags:pagePermission>
								<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HUNDRANDORDER_DELETEMORE_BTN"
									html='<a href="javascript:;" class="sfbtn sfbtn-opt" onclick="delMore();"><i class="sficon sficon-rubbish"></i>批量删除</a>'></sfTags:pagePermission>
								<sfTags:pagePermission authFlag="ORDER_MARKORDER_BTN_OTHERS"
									html='<a href="javascript:showMarkOrders();" class="sfbtn sfbtn-opt"><i class="sficon sficon-sign"></i>标记工单</a>'></sfTags:pagePermission>
								<a href="javascript:cancelMarkOrders();" class="sfbtn sfbtn-opt">
									<i class="sficon sficon-signCancel"></i>
									取消标记
								</a>
								<sfTags:pagePermission authFlag="ORDERMGM_ALLORDER_ALLORDER_BATCHPRINT_BTN"
									html='<a href="javascript:batchPrint();" id="repeatOrder"  class="sfbtn sfbtn-opt"><i class="sficon sficon-print"></i>批量打印</a>'></sfTags:pagePermission>
								<%--<a href="javascript:batchPrintTest();" id="repeatOrder"  class="sfbtn sfbtn-opt"><i class="sficon sficon-print"></i>测试打印</a>--%>
							</div>
							<div class="f-r">
								<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HUNDRANDORDER_EXPORT_BTN"
								...
								<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_HUNDRANDORDER_HEADSET_BTN"
	...
	<div class="popupBox zzjBox dange">
		...
		<div class="popupContainer">
			<div class="popupMain">
				<div class="lh-22">
					当前服务工程师：
					<span class="c-0e8ee7 f-14 pl-5 pr-5"<%--id="employes"--%>>张三</span>
				</div>
	...
	<script type="text/javascript">
		...
		function couts() {
			$.post("${ctx}/order/ChangeSelfOrder/getCounts", {
				orderType : "2"
			}, function(result) {
				...
				$("#45mintCount").text(result.fofmintCount || '0');
				//        $("#2hCount").text(result.twhCount||'0');
			});
		}

		function initSfGrid() {
			$("#table-waitdispatch").sfGrid({
				url : '${ctx}/order/ChangeSelfOrder/FourHandurandOrder',
				sfHeader : defaultHeader,
				sfSortColumns : sortHeader,
				rownumbers : true,
		...
		function reset() {
			...
			html += '<option value="待接收">待接收</option>';
			html += '<option value="一级网点改派申请">一级网点改派申请</option>';
			html += '<option value="抢单中">抢单中</option>';
			html += ' <option value="已接单">已接单</option>';
			html += '<option value="已预约">已预约</option>';
			html += '<option value="已派工">已派工</option>';
			html += '<option value="服务完成">服务完成</option>';
			html += '<option value="网点封单">网点封单</option>';
			html += '<option value="封单审核通过">封单审核通过</option>';
			html += '  <option value="封单审核驳回">封单审核驳回</option>';
			html += '<option value="已结单">已结单</option>';
			html += '<option value="用户取消">用户取消</option>';
			html += ' <option value="二级网点改派申请">二级网点改派申请</option>';
			html += '<option value="中心改派申请">中心改派申请</option>';
			html += '<option value="到货签收">到货签收</option>';
			html += '<option value="破损拒签">破损拒签</option>';
			html += '<option value="安得到货">安得到货</option>';
			html += '<option value="安得到货">用户签收</option>';
			html += '<option value="安得到货">用户拒签</option>';
			html += '<option value="安得到货">接收机器</option>';
			html += '<option value="安得到货">抢单锁定中</option>';
			html += '</select>  </span>';
			$("#reloadSpan").empty().html(html);
			...
			var html3 = '<span class="w-140 dropdown-sin-2">';
			html3 += '<select class="select-box w-120"  id="origin" style="display:none" multiple  multiline="true" name="origin"  >';
			html3 += '<option value="服务热线">服务热线</option>';
			html3 += '<option value="网点自建">网点自建</option>';
			html3 += '<option value="分中心自建">分中心自建</option>';
			html3 += '<option value="用户自助微信">用户自助-微信</option>';
			html3 += ' <option value="用户自助官网">用户自助-官网</option>';
			html3 += '<option value="计划提醒">计划提醒</option>';
			html3 += '<option value="物流平台">物流平台</option>';
			html3 += '<option value="投诉创建">投诉创建</option>';
			html3 += '<option value="网络接入">网络接入</option>';
			html3 += '<option value="邮件接入">邮件接入</option>';
			html3 += '<option value="舆情接入">舆情接入</option>';
			html3 += '<option value="电商接入">电商接入</option>';
			html3 += '<option value="安得电商接入">安得电商接入</option>';
			html3 += '<option value="国美接入">国美接入</option>';
			html3 += '<option value="京东接入">京东接入</option>';
			html3 += '<option value="网点鉴定改派">网点鉴定改派</option>';
			html3 += '<option value="总部自建">总部自建</option>';
			html3 += '<option value="天猫接入">天猫接入</option>';
			html3 += '<option value="经销商微信">经销商微信</option>';
			html3 += '<option value="菜鸟接入">菜鸟接入</option>';
			html3 += '<option value="奇门接入">奇门接入</option>';
			html3 += ' <option value="手工单">手工单</option>';
			html3 += '<option value="美的通接入">美的通接入</option>';
			html3 += '<option value="苏宁">苏宁</option>';
			html3 += '<option value="项目监理派工">项目监理派工</option>';
			html3 += ' <option value="美居App">美居App</option>';
			html3 += ' <option value="其他">其他</option>';
			html3 += ' </select>';
			html3 += ' </span>';

			$("#reloadSourceSpan").empty().append(html3);
			$('.dropdown-sin-2').dropdown({
				input : '<input type="text" maxLength="20" placeholder="请输入搜索">',
			});
		}
		...
		function exports() {
			...
			if (idArr > 10000) {
				$('body').popup({
					...
					content : content,
					fnConfirm : function() {
						location.href = "${ctx}/order/ChangeSelfOrder/export?formPath=/a/order/ChangeSelfOrder/headerList&&maps=" + $("#searchForm").serialize();
					}
				});
			} else {
				location.href = "${ctx}/order/ChangeSelfOrder/export?formPath=/a/order/ChangeSelfOrder/headerList&&maps=" + $("#searchForm").serialize();
		...
		function delMore() {
			...
			if (idArr.length < 1) {
				...
				$('body').popup({
					level : '3',
					type : 2,
					content : "删除的工单数据不可恢复，您确定要删除这" + leg + "条工单吗？",
					fnConfirm : function() {
						$.ajax({
							type : "post",
		...
		function batchPrintTest() {
			var idArr = $('#table-waitdispatch').jqGrid('getGridParam', 'selarrrow');
			if (idArr.length <= 0) {
				layer.msg("请选择需要打印的工单！");
			} else {
				//使用默认
				//window.open("http://localhost:8280/printer/print/index?tname=BJDY_1");
				$("#pf1").submit();
			}
		}
	</script>
	<form id="pf1" method="post" action="http://localhost:8280/printer/print/index" target="_blank">
