	...
	<div class="popupContainer">
		<div class="popupMain pos-r" >
					...
					<div class="tabCon">
						...
						<div class="cl mt-10 mb-10" >
							<div class="pos-r pl-100" id="pcd">
								...
								<input type="text" class="input-text w-280 f-l readonly" readonly="readonly" id="customerAddress1" name="customerAddress1" value="${order.customerAddress }" autocomplete="off"/>
						...
						<div class="cl mt-10">
							<div class="f-l pos-r pl-100 h-50">
								...
								<textarea  class="input-text w-380 h-50 readonly" readonly="readonly" id="customerFeedback" name="customerFeedback">${order.customerFeedback}</textarea>
							</div>
							<div class="f-l pos-r pl-80 h-50">
								<label class="lb w-80"><c:if test="${mustfill.columns.remarks}"><em class="mark"></em></c:if>备注：</label>
								<textarea class="input-text h-50 w-320 readonly" readonly="readonly" id="remarks" name="remarks" >${order.remarks}</textarea>
							</div>
						</div>
			...
			<div class="btnMenubox pb-80">
				...
				<sfTags:pagePermission authFlag="SECONDSITE_ORDER_MODORDER_BTN_OTHERS" html='<input class="sfbtn sfbtn-opt" id="xggd" value="修改工单" type="button" />'></sfTags:pagePermission>
				</c:if>
				<sfTags:pagePermission authFlag="SECONDSITE_ORDER_BACKWAITDISP_BTN_OTHERS" html='<input class="sfbtn sfbtn-opt sbtn" value="返回一级待派工" type="button" onclick="returnSiteOrder()"/>'></sfTags:pagePermission>
				<sfTags:pagePermission authFlag="SECONDSITE_ORDER_REDISP_BTN_OTHERS" html='<input class="sfbtn sfbtn-opt sbtn"  value="转派" type="button" onclick="directDisZp()"/>'></sfTags:pagePermission>
				<c:if test="${order.status ne '1'  && order.status ne '7' }">
				<sfTags:pagePermission authFlag="SECONDSITE_ORDER_FKFD_BTN_OTHERS" html='<input class="sfbtn sfbtn-opt sbtn" value="反馈封单" id="btn-fbOrder" type="button" />'></sfTags:pagePermission>
				</c:if>
			</div>
		</div>
	</div>
	...
	<div class="popupContainer">
		<form action="" id="generationOrderFrom">
			...
			<div id="fbBaseInfo">
				...
				<div class="tabCon">
					<div>
						<input type="hidden"  name="orderId" id="orderId" value="${order.id }" datatype="*">
		...
		$.ajax({
			...
			success:function(result){
				if(parseInt(result) > 0){
                    ...
                    $("#arroundCustomerAddress").val('${order.customerAddress}');
...
function address() {
	...
	function useFit(){
		...
		$.ajax({
			...
			data:{orderId:"${order.id}",siteId:siteId},
			dataType:'json',
			success:function(result){
				var html='<option value="">请选择</option>';
	...
	if(flag&&checkNum(num)&&code!=null&&code!=""){
		$.ajax({
			...
			customerAddress:"${order.customerAddress}",warrantyType:"${order.warrantyType}",applianceCategory:"${order.applianceCategory}",applianceBrand:"${order.applianceBrand}",siteId:siteId},
			dataType:'text',
			async:false,
			success:function(result){
				layer.msg("操作成功!");
...
function loadAlreadyCode(type,code){
	...
	$.ajax({
		url: "${ctx}/order/getHistoryOrdersCodeOutCountByTelDetail?code=" + $.trim(code)+"&id="+'${order.id}&f=hbc',
		type: 'GET',
		success: function(data) {
			if(changeMark){
				return;
