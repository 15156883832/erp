...
<div class="page-orderWait">
	<div id="tab-system" class="HuiTab">
		<div class="tabBar cl mb-10">
			...
			<sfTags:pagePermission authFlag="ORDERMGM_ALLORDER_JRTXBJ_TAB" html='<a class="btn-tabBar current"  href="${ctx }/order/jrtxbj">今日提醒标记</a>'></sfTags:pagePermission><!--orderWholeList -->
		...
		<div class="tabCon">
					...
					<table class="table table-search">
						...
						<tr class="moreCondition" style="display: none;">
							<td colspan="10">
								...
								<label class="lb text-r" style="width:95px;">标记提醒时间：</label>
			...
			<div class="pt-10 pb-5 cl">
				<div class="f-l">
					<sfTags:pagePermission authFlag="ORDERMGM_ALLORDER_JRTXBJ_NEWORDER_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt" onclick="addNew();" id="btnAddnew"><i class="sficon sficon-add"></i>新建工单</a>'></sfTags:pagePermission>
					<sfTags:pagePermission authFlag="ORDERMGM_ALLORDER_JRTXBJ_DISP_BTN" html='<a href="javascript:directDis();" class="sfbtn sfbtn-opt"  id="directpg"><i class="sficon sficon-dispatch"></i>派工</a>'></sfTags:pagePermission>
					<sfTags:pagePermission authFlag="ORDERMGM_ALLORDER_JRTXBJ_WAITDISP_BTN" html='<a href="javascript:emporarily();" class="sfbtn sfbtn-opt"><i class="sficon sficon-notdispatch"></i>暂不派工</a>'></sfTags:pagePermission>
					<sfTags:pagePermission authFlag="ORDER_SETTLEMENT_BTN_OTHERS" html='<a href="javascript:batchJiesuan();" class="sfbtn sfbtn-opt"><i class="sficon sficon-settlement"></i>批量结算</a>'></sfTags:pagePermission>
					<sfTags:pagePermission authFlag="ORDERMGM_ALLORDER_JRTXBJ_INVALIDORDER_BTN" html='<a href="javascript:showwxgd();" class="sfbtn sfbtn-opt"><i class="sficon sficon-invalid"></i>无效工单</a>'></sfTags:pagePermission>
					...
					<sfTags:pagePermission authFlag="ORDERMGM_ALLORDER_JRTXBJ_CANCEL_MARK_BTN" html='<a href="javascript:cancelMarkOrders();" class="sfbtn sfbtn-opt"><i class="sficon sficon-signCancel"></i>取消标记</a>'></sfTags:pagePermission>
					<%-- <sfTags:pagePermission authFlag="ORDERMGM_ALLORDER_JRTXBJ_BATCH_JD_BTN" html='<a href="javascript:plJiaodan();" class="sfbtn sfbtn-opt"><i class="sficon sficon-pljd"></i>批量交单</a>'></sfTags:pagePermission> --%>
				</div>
				<div class="f-r">
					...
					<sfTags:pagePermission authFlag="ORDERMGM_ALLORDER_JRTXBJ_EXPORT_BTN" html='<a onclick="return exports()"class="sfbtn sfbtn-opt2" target="_blank"><i class="sficon sficon-export"></i>导出</a>'></sfTags:pagePermission>
					<sfTags:pagePermission authFlag="ORDERMGM_ALLORDER_JRTXBJ_TABLESET_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt2" id="setHeadersBtn"><i class="sficon sficon-setting"></i>表头设置</a>'></sfTags:pagePermission>
				</div>
...
$(".closePopup").on("click",function(){
	...
	//$("iframe", window.top.document).attr("src", '${ctx}/order/during');
});
	...
	if (idArr.length < 1) {
		...
		for (var i = 0; i < idArr.length; i++) {
			...
			if ($.trim(rowData.status) != "待派工" && $.trim(rowData.status) != "暂不派工") {
				layer.msg("您选择的工单中有已派工的工单，请重新选择！");
				return;
			}
			if(categorys==""){
			...
			/* if (categoryList.length > 1) {
				$('body').popup({
					level: '3',
					type: 1,  // 提示是否进行某种操作
					content: "您选择工单的家电品类一致时才能操作批量派工，请您重新选择工单。",
					fnConfirm: function () {
					}
				});
				return;
...
function initSfGrid(){
	$("#table-waitdispatch").sfGrid({
		url : '${ctx}/order/jrtxbjList?ptype='+${ptype},
		sfHeader: defaultHeader,
		sfSortColumns: sortHeader,
		rownumbers:true,
		gridComplete:function(){
            ...
            <%--var re_records = $("#table-waitdispatch").jqGrid('getGridParam', 'records'); //获取数据总条数
            if(re_records==0){
...
function fmtOrderNo(rowData){
	var authFlag = 'true';
	<%--var authFlag = '${fns:checkBtnPermission("ORDER_BTN_BATCHJIESUAN")}';--%>
	var html = '';
	if(authFlag === 'true'){
...
function exports() {
    ...
    if (idArr > 10000) {
        $('body').popup({
            ...
            content: content,
            fnConfirm: function () {
                location.href = "${ctx}/order/export?formPath=/a/order/jrtxbj&&maps=" + $("#searchForm").serialize();
            },
            fnCancel: function () {
            }
        });
...
function fmtWhetherCollection(rowData) {
 	if (rowData.status == "3" || rowData.status == "4" ) {//|| rowData.status == "5"
 		if ("0" == rowData.whether_collection || "2" == rowData.whether_collection) {
 			if('${fns:checkBtnPermission("ORDERMGM_ALLORDER_JRTXBJ_RETURNMONEYCONFIRM_BTN")}' === 'true'){
 ...
 function fmtRC(rowData) {//交单情况
 	if (rowData.status == "3" || rowData.status == "4" || rowData.status == "5" || rowData.status == "8" ) {
 		if ("0" == rowData.return_card || "2" == rowData.return_card) {
 			if('${fns:checkBtnPermission("ORDERMGM_ALLORDER_JRTXBJ_RETURNCARDCONFIRM_BTN")}' === 'true'){
