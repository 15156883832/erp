	...
	<div class="popupContainer pos-r">
		<div class="popupMain">
			<form action="" id="addForm">
				<div class="cl mb-10">
					<label class="w-70 f-l">申请单号：</label><input type="text" class="input-text w-180 readonly mr-5" value="${factoryFttingNumber }" readonly="readonly" id="number" name="number"/>
					<label class="w-80"><em class="mark">*</em>收货方：</label><input type="text" class="input-text w-300 readonly mr-5" value="${site.name }" readonly="readonly" />
					<label class="w-80"><em class="mark">*</em>收货人：</label><input type="text" class="input-text w-180" maxlength="10" id="customerName" name="customerName"/>
				</div>
				<div class="cl mb-10">
					<label class="w-70 f-l"><em class="mark">*</em>联系电话：</label><input type="text" maxlength="20" class="input-text w-180 mr-5" id="customerContact" name="customerContact"/>
					<label class="w-80"><em class="mark">*</em>收货地址：</label><input type="text" maxlength="255" style="width:568px;" class="input-text" id="customerAddress" name="customerAddress"/>
				</div>
				<div class="cl mb-10 ">
					<label class="w-70 f-l">备注：</label>
					<input type="text" class="input-text f-l" style="width:837px" id="remarks" name="remarks"/>
				</div>
				<div class="cl mb-10">
					<label class="w-70 f-l">申请时间：</label>
					<input type="text" class="input-text w-180 f-l readonly" readonly="readonly" value="${nowDate }" id="applyTime" name="applyTime"/>
				...
				<div class="pcontent"> 
					<table class="table table-border table-bordered table-bg table-sdrk" style="width:907px;table-layout: auto;">
						<thead>
						<tr>
							<th class="w-160">备件条码</th>
							<th class="w-160">备件名称</th>
							<th class="w-160">备件型号</th>
							<th class="w-90">适用品类</th>
							<th class="w-80">单位</th>
...
var fla=false;
    ...
    function pageShow(){
        ...
        if(!isBlank('${ids}')){
        	$.ajax({
        		type:"post",
        		async: false,
        		url:"${ctx}/factoryfitting/Apply/getDetailsByIds",
        		data:{ids:'${ids}'},
        ...
        for (var i = 0; i < rowDatas.length; i++) {
            ...
            html += '	<input type="hidden" class="id-' + i + '" name="id" value="' + rowDatas[i].id + '">';
           /*  html += '	<td class="text-c brand-' + i + '"> ' + rowDatas[i].suit_brand + '</td>';  */
            html += '	<td class="text-c category-' + i + '">' + rowDatas[i].suit_category + '</td>';
            html += '	<td class="text-c unit-' + i + '">' + rowDatas[i].unit + '</td>';//单位
            html += '	<td class="text-c pricemark price-' + i + '">' + rowDatas[i].site_price + '</td>';//价格
            html += '	<td class="text-c"><input type="text" class="input-text numbermark num-' + i + '" name="num"/></td>';//申请数量
       ...
       /*  html += '	<td class="text-c  brand-' + ids.length + '"></td>'; */
        ...
        $.getScript("${ctxPlugin}/lib/select/zh-CN.js", function () {
            $("#sdrk_tbd").find(".code-" + rowDatas.length).select2({
                ajax: {
                    type: 'post',
        ...
        $.getScript("${ctxPlugin}/lib/select/zh-CN.js", function () {
            $("#sdrk_tbd").find(".name-" + rowDatas.length).select2({
                ajax: {
                    type: 'post',
        ...
        $.getScript("${ctxPlugin}/lib/select/zh-CN.js", function () {
            $("#sdrk_tbd").find(".version-" + rowDatas.length).select2({
                ajax: {
                    type: 'post',
    ...
    function addSave(){
    	...
    	$.ajax({
    		type:"post",
    		data:postData,
    		url:"${ctx}/factoryfitting/Apply/addApplySave",
    		success:function(result){
