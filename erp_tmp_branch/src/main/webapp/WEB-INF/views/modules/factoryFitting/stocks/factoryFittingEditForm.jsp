	...
	<div class="popupContainer pos-r">
		<div class="popupMain">
			<div class="cl mb-10">
				...
				<input type="text" name="code" datatype="/^[a-zA-Z0-9-]+$/"  nullmsg="请输入备件条码" errormsg="备件条码必须为字母或数字或字母数字的组合" class="input-text w-140 f-l ${empty fitting.id ? 'mustfill ' : ''}  ${not empty fitting.id ? 'readonly ' : ''}" ${not empty fitting.id ? 'readonly="readonly"' : '' } value="${fitting.code}" maxlength="20"/>
			...
			<div class="cl mb-10">
				...
				<input type="text" class="input-text w-140 f-l mustfill" datatype="*" nullmsg="请输入备件型号" name="version"  value="${fitting.version }" />
			...
			<div class="cl mb-10">
				<c:if test="${empty fitting.id}">
					<label class="f-l w-100"> <em class="mark">*</em>入库数量：</label>
					<input type="text" name="warning" datatype="price" nullmsg="请输入入库数量" errormsg="入库数量输入格式错误" class="input-text w-140 f-l mustfill ${not empty fitting.id ? 'readonly' : ''}" ${not empty fitting.id ? 'readonly="readonly"' : '' } value="${fitting.warning}" />
				</c:if>
				<c:if test="${not empty fitting.id}">
					<label class="f-l w-100"> <em class="mark">*</em>当前库存：</label>
					<input type="text" name="warning" datatype="price"  readonly="readonly" class="input-text w-140 f-l  readonly ${not empty fitting.id ? 'readonly' : ''}" ${not empty fitting.id ? 'readonly="readonly"' : '' } value="${fitting.warning}" />
				</c:if>
			...
			<div class="cl mb-10">
				...
				<div class="priceWrap w-140 f-l mustfill">
					<input type="text" class="input-text" value="${fitting.sitePrice}" name="sitePrice" datatype="price" errormsg="网点价格输入格式错误" nullmsg="请输入入库价格"/>
					<span class="unit">元</span>
				</div>
				<%-- <label class="f-l w-100">零售价格：</label>
				<div class="priceWrap w-140 f-l mustfill">
					<input type="text" class="input-text" name="stockPrice" value="${fitting.stockPrice }" errormsg="零售价格输入格式错误" datatype="price1"  />
					<span class="unit">元</span>
				</div> --%>
			</div>
			<div class="cl mb-10">
