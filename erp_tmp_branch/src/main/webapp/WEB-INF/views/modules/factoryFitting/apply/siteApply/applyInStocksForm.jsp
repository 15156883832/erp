...
<body>
<div class="popupBox bjsdrkbox" style="width: 960px;">
	<h2 class="popupHead">
		备件申请到货入库
		<a href="javascript:close();" class="sficon closePopup"></a>
	</h2>
		...
		<div class="popupMain">
			<div class="pcontent">
				<form action="" id="addForm">
					<div class="cl mb-10">
						<label class="w-80 f-l">申请单号：</label>
					...
					<div class="cl mb-10">
						<label class="w-80 f-l">收货地址：</label>
						<input type="text" maxlength="255" value="${ffa.customer_address }" style="width:500px;" class="input-text readonly f-l " readonly="readonly" id="customerAddress" name="customerAddress"/>
					...
					<div class="cl mb-10">
						<label class="w-80 f-l">申请总金额：</label>
						...
						<input type="text" class="input-text w-190  f-l readonly" readonly="readonly" value="${ffa.auditTime }" id="applyTime" name="applyTime"/>
					</div>
					<div class="cl mb-10 ">
						<label class="w-80 f-l">配送信息：</label>
					...
					<div class="">
						<table class="table table-border table-bordered table-bg table-sdrk" style="width:890px;table-layout: auto;">
							<thead>
							<tr>
...
var fla=false;
        	...
        	success:function(data){
		        ...
		        if(rowDatas.length > 0){
			        for (var i = 0; i < rowDatas.length; i++) {
			           ...
			           // html += '	<td class="text-c"><input type="text" value="'+rowDatas[i].audit_num+'" class="input-text numbermark relnum-' + i + '" name="num"/></td>';//入库数量
    ...
    function addSave(){
        ...
        $("tr[name='sdrk_tr']").each(function (index) {
            ...
            var num = $(this).find(".numbermark").text();//入库数量
            var detailId = $(this).find("input[name='detailId']").val();//申请明细id
            var price = $(this).find(".pricemark").text(); //网点价格
            var auditnum = $(this).find(".auditnum").text(); //审核数量
            /* if(num==''){

                layer.msg("您输入的第"+(lgn+1)+"行的入库数量");
                if(check){
            ...
            if(!numReg.test(num)){
                layer.msg("您输入的第"+(lgn+1)+"行的入库数量格式不正确，请重新输入!");
                if(check){
               	 $(this).find("input[name='num']").focus();
                }
                check = false;
            ...
            if((parseFloat(auditnum) < parseFloat(num))){
            	layer.msg("第"+(lgn+1)+"行的入库数量不得大于审核数量!");
            	if(check){
               	 $(this).find("input[name='num']").focus();
            	}
                check = false;
       ...
       $.ajax({
    		type:"post",
    		data:postData,
    		url:"${ctx}/factoryfitting/Apply/inStockApplySave",
    		success:function(data){
    			if(data=='200'){
    				parent.search();
    				...
    				layer.msg("提交失败，入库数量不能大于审核数量！");
    			}else if(data=='411'){
                    layer.msg("备件条码冲突，无法入库！");
                }else {
    				layer.msg("提交失败，未知错误！");
    			}
    			return;
