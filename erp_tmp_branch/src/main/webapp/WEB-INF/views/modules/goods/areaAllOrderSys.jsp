			...
			<div class="page-orderWait">
				<div id="tab-system" class="HuiTab">
					<div class="tabBar cl mb-10">
						<a class="btn-tabBar " href="${ctx }/goods/nanDao/areacommodityOrder">区域南岛订单</a>
						<a class="btn-tabBar current" href="${ctx }/goods/nanDao/areaAllOrder">平台商品</a>
						<p class="f-r btnWrap">
							...
							<div class="bk-gray pt-10 pb-5 moreCondition mb-10">
								<table class="table table-search">
									<tr class="moreCondition" style="">
										...
										<th style="width: 76px;" class="text-r">区域人员：</th>
										<td>
										<select class="select" multiline="false" name="areaName" id="areaName" placeholder="请选择" style="width:200px;height:25px" panelMaxHeight="200px">
					                          <option value="">请选择</option>
					                          <c:forEach var="service" items="${supplist}">
					                              <option value="${service.columns.id}">${service.columns.name}</option>
					                          </c:forEach>
										</select>
											<!-- <input type="text" class="input-text" value="" onkeydown="enterEvent(event)" name="areaName" id="areaName" /> -->
										</td>
		...
		//初始化jqGrid表格，传递的参数按照说明
		   function initSfGrid() {
				sfGrid = $("#table-waitdispatch").sfGrid({
					url : '${ctx}/goods/nanDao/areaAllBindingGrid',
		...
		function expotrs(){
			...
			if(idArr>10000){
				$('body').popup({
					...
					content:content,
					 fnConfirm :function(){
						 location.href="${ctx}/goods/nanDao/areaexports?formPath=/a/goods/nanDao/areaAllOrder&&type=2&&maps="+$("#searchForm").serialize(); 
					 }
				});
			}else{
				 location.href="${ctx}/goods/nanDao/areaexports?formPath=/a/goods/nanDao/areaAllOrder&&type=2&&maps="+$("#searchForm").serialize();
