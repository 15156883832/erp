			...
			<div class="page-orderWait">
				<div id="tab-system" class="HuiTab">
					<div class="tabBar cl mb-10">
						<a class="btn-tabBar current" href="${ctx }/goods/nanDao/sysOrder">南岛订单</a>
...
<div class="popupBox spqrck" style="width:600px;">
	...
	<div class="popupContainer pos-r">
		<div class="popupMain " style=" max-height:500px;overflow:auto;">
			...
			<div class="cl mb-10">
				<label class="f-l w-80 text-r">备注：</label>
				<input type="text" class="input-text f-l "  style="width: 440px" value="" name="remark" id="querenchukuremark"/>
			</div>
			<div class="cl mb-10" >
		...
		//初始化jqGrid表格，传递的参数按照说明
		   function initSfGrid() {
				sfGrid = $("#table-waitdispatch").sfGrid({
					url : '${ctx}/goods/nanDao/nandaoGrid',
    ...
    function noPassSubmit(){
		...
		function goodsDetail(id){
			...
			$.ajax({
				...
				//	$("#logisticsName1").val(data.columns.logistics_name);
					$('#logisticsName1').combobox('setValues',data.columns.logistics_name);  
					$("input[name='logisticsName1']").val(data.columns.logistics_name);
					$("#logisticsNo1").val(data.columns.logistics_no);
						$("#chukuon").hide();
						$("#xiangqingon").show();
					}
					$(".spqrck").popup();
		...
		function outStockConfirm(){
			...
			var logisticsNo = "";
			$("input[name='logisticsNo1']").each(function(index,items){
				if(isBlank(logisticsNo)){
					logisticsNo = $(this).val();
				}else{
					logisticsNo =logisticsNo+","+ $(this).val();
            ...
            var remark=$("#querenchukuremark").val();//出库数量
            ...
            if(isBlank(outAmount)){
                ...
                mark2=true;
                 $.ajax({
                    type:"post",
                    url:"${ctx}/goods/nanDao/outStockConfirm",
                    data:{
                        id:id,
                        logisticsName:logisticsName,
                        logisticsNo:logisticsNo,
		...
		function updatelogistics(){
			...
			mark3=true;
			 $.ajax({
				type:"post",
				url:"${ctx}/goods/nanDao/updatelogistics",
		...
		function fmtOper(rowData){//操作
            ...
            if(rowData.status=='0') {
                html= "<span><a onclick='confirmOrder(\"" + rowData.id + "\")' class='c-0383dc' >审核</a></span>";
                ...
                html= "<span><a onclick='confirmOrder(\"" + rowData.id + "\")' class='c-0383dc' >重新审核</a></span>";
			}
			return html;
		}
		...
		function confirmOrder(orderId){
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data!=null && data !=""){
                        $(".shfkboxjjbtg").find("input[name='orderNumber']").val(data.columns.number);
                        $(".shfkboxjjbtg").find("input[name='createName']").val(data.columns.siteName);
                        $(".shfkboxjjbtg").find("input[name='goodName']").val(data.columns.good_name);
                        $(".shfkboxjjbtg").find("input[name='goodNum']").val(data.columns.purchase_num);
                        $(".shfkboxjjbtg").find("input[name='payAmount']").val(data.columns.good_amount);
                        if(data.columns.payment_type=='0'){
                            $(".shfkboxjjbtg").find("input[name='payType']").val("微信");
                        }else if(data.columns.payment_type=='1'){
                            $(".shfkboxjjbtg").find("input[name='payType']").val("支付宝");
						}
                        $(".shfkboxjjbtg").find("input[name='recipients']").val(data.columns.customer_name);
                        $(".shfkboxjjbtg").find("input[name='mobile']").val(data.columns.customer_contact);
                        $(".shfkboxjjbtg").find("input[name='receiverAddress']").val(data.columns.province+data.columns.city+data.columns.area+data.columns.customer_address);
                        $(".shfkboxjjbtg").find("input[name='id']").val(data.columns.id);
                        var url='${commonStaticImgPath}'+data.columns.pay_confirm;
                        $(".shfkboxjjbtg").find("img").attr("src",url);
                        $(".shfkboxjjbtg").find("input[name='remark']").val(data.columns.remark);
                    }
		...
		function showDetail(orderId){
			$.ajax({
				...
				success:function(data){
					if(data!=null && data !=""){
                        if(data.columns.status == "1"){
                            $("#chongxinshenhe").show();
                        }else{
                            $("#chongxinshenhe").hide();
                        }
                        if(data.columns.status == "3" || data.columns.status == "4"){
                            $("#updateord").hide();
                        }else{
                            $("#updateord").show();
                        }
		...
		function expotrs(){
			...
			if(idArr>10000){
				$('body').popup({
					...
					content:content,
					 fnConfirm :function(){
						 location.href="${ctx}/goods/nanDao/export?formPath=/a/goods/nanDao/sysOrder&&maps="+$("#searchForm").serialize(); 
					 }
				});
			}else{
				 location.href="${ctx}/goods/nanDao/export?formPath=/a/goods/nanDao/sysOrder&&maps="+$("#searchForm").serialize();
