...
<div class="printpage wrap" style="position:relative;">
	...
	<table class="printTable"style="margin-top:5px; margin-bottom:5px;">
		...
		<c:forEach items="${god.columns.detailList }" var="dtl">
			<tr>
			<td style="width: 33%;text-align:center;">${dtl.columns.good_brand } ${dtl.columns.good_name }</td>
			<td style="width: 11%;text-align:center;">${dtl.columns.purchase_num }</td>
			<td style="width: 11%;text-align:center;">${dtl.columns.unit }</td>
			<td style="width: 22%;text-align:center;">${dtl.columns.customer_price }</td>
			<td style="width: 22%;text-align:center;">${dtl.columns.real_amount }</td>
		</tr>
		</c:forEach>
		...
		<span style=" width:25%" class="f-l">
			...
			<c:if test="${god.columns.confirm_time!=null && god.columns.confirm_time!='' }">
				<span style="  display: inline-block;width: 150px;">
					<fmt:formatDate value="${god.columns.confirm_time }" pattern="yyyy"/>年&nbsp;<fmt:formatDate value="${god.columns.confirm_time }" pattern="MM"/>月<fmt:formatDate value="${god.columns.confirm_time }" pattern="dd"/>日
				</span>
			</c:if>
		</span>
		<span style=" width:25%" class="f-l">
