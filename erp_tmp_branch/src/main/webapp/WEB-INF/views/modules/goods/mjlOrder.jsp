	...
	<div class="sfpagebg bk-gray">
		<div class="page-orderWait goodsPage">
			<div class="tabBar cl mb-10">
				<sfTags:pagePermission authFlag="GOODSMGM_GOODSORDERMSG_SELFGOODS_TAB" html='<a class="btn-tabBar "  href="${ctx }/goods/siteselfOrder/headerList">公司商品订单</a>'></sfTags:pagePermission>
				...
				<sfTags:pagePermission authFlag="GOODSMGM_GOODSORDERMSG_MEIJIELIORDER_TAB" html='<a class="btn-tabBar current"  href="${ctx }/goods/platFormOrder/headerListMjl">美洁力订单</a> '></sfTags:pagePermission>
				<p class="f-r btnWrap">
						...
						<tr>
							...
							<td>
								<select class="select w-140" name="status">
									...
									<option value="0" <c:if test="${map.status=='1' }">selected="selected"</c:if>>已取消</option>
								</select>
							</td>
							<th style="width: 76px;" class="text-r">下单人：</th>
							<td>
								<select class="select w-140" name = "placingOrderBy" id="searchPlacingOrderBy">
									<option value="">请选择</option>
									<option value="" disabled>服务商</option>
									<c:if test="${rd3.id!=null && rd3.id!='' }">
										<option value="${rd3.name }" <c:if test="${rd3.name==map.placingOrderBy }">selected="selected"</c:if>>${rd3.name }</option><!--网点-->
									</c:if>
									<option value="" disabled>信息员</option>
									<c:if test="${rd1 ne null }">
										<c:forEach items="${rd1 }" var="em">
											<option value="${em.columns.name }" <c:if test="${em.columns.name==map.placingOrderBy }">selected="selected"</c:if>>${em.columns.name }</option><!--服务工程师-->
										</c:forEach>
									</c:if>
									<option value="" disabled>服务工程师</option>
									<c:if test="${rd2 ne null }">
										<c:forEach items="${rd2 }" var="sm">
											<option value="${sm.columns.name }" <c:if test="${sm.columns.name==map.placingOrderBy }">selected="selected"</c:if>>${sm.columns.name }</option><!--信息员-->
										</c:forEach>
									</c:if> 
								</select>
							</td>
							...
							<td>
								<span class="f-l w-140  readonly" >
									<select class="select w-140" name="goodCategory" id="searchCategory">
										<option value="">请选择</option>
										<c:forEach items="${categoryType}" var="ctype">
											<option value="${ctype.columns.name}" <c:if test="${map.goodCategory == ctype.columns.name }">selected="selected"</c:if>>${ctype.columns.name}</option>
										</c:forEach>
									</select>
								</span>
							</td>
			...
			<div class="pt-10 pb-5 cl btnsWrap">
				<div class="f-l">
					<sfTags:pagePermission authFlag="GOODSMGM_GOODSORDERMSG_MEIJIELIORDER_CANCELORDER_BTN" html='<a onclick="return cancelOrder()" class="sfbtn sfbtn-opt" ><i class="sficon sficon-cancel"></i>取消订单</a>'></sfTags:pagePermission>
				</div>
				<div class="f-r">
					<sfTags:pagePermission authFlag="GOODSMGM_GOODSORDERMSG_MEIJIELIORDER_EXPORTSITESELF_BTN" html='<a  onclick="return exports()"class="sfbtn sfbtn-opt2" target="_blank"><i class="sficon sficon-export"></i>导出</a>'></sfTags:pagePermission>
				</div>
			</div>
			...
			<ul class="" id="tableBody">
				<c:forEach items="${page.list }" var="lt1">
					...
					<table class="table table-border table-bordered text-c goodsTable" style="table-layout: auto;">
						<tr>
							<td colspan="12" class="c-666 bg-fafafa text-l"><span class="pl-5">订单编号：</span> <span class="c-222">${lt1.columns.number }</span></td>
						</tr>
						<tr>
							<td class="w-40">
							...
							<td class="w-400 " style="border-left: 0;cursor: pointer;" onclick="showDetail('${lt1.columns.good_id}','${lt1.columns.good_name}')">
								<div class="imgTD">
							...
							<c:if test="${lt1.columns.confirm_amount!=null && lt1.columns.confirm_amount!='' }">
								<td class="w-110">￥${lt1.columns.confirm_amount}</td>
							</c:if>
							<td class="w-110">
								<c:if test="${lt1.columns.status eq 1 || lt1.columns.status eq 2 || lt1.columns.status eq 0   }">暂未出库</c:if>
								<c:if test="${lt1.columns.status eq 3 || lt1.columns.status eq 4 }">
							...
							<td class="w-140">${lt1.columns.xiadanTime }</td>
							<td class="w-140">${lt1.columns.order_number }</td>
							<td class="w-200">
								<p> ${lt1.columns.customer_name }${lt1.columns.customer_contact }</p>
								<p>${lt1.columns.customer_address }</p>
							</td>
							<td class="w-110">
								<c:if test="${lt1.columns.status eq 0 }"><span class="oState state-canceled">已取消</span></c:if>
								
								<c:if test="${lt1.columns.status eq 1 }">
									<c:if test="${lt1.columns.purchase_num > lt1.columns.stocks }">
										<span class='oState state-dskdck'>待收款待下单</span>
									</c:if>
									<c:if test="${lt1.columns.purchase_num <= lt1.columns.stocks }">
										<span class="oState state-dskdck">待收款待出库</span>
									</c:if>
								</c:if>
								
								<c:if test="${lt1.columns.status eq 2 }">
									<c:if test="${lt1.columns.purchase_num > lt1.columns.stocks }">
										<span class='oState state-waitXd'>已收款待下单</span>
									</c:if>
									<c:if test="${lt1.columns.purchase_num <= lt1.columns.stocks }">
										<span class='oState state-waitCk'>已收款待出库</span>
									</c:if>
								</c:if>
								
								<c:if test="${lt1.columns.status eq 3 }">
									<c:if test="${lt1.columns.outstock_type=='0' || lt1.columns.outstock_type=='1' }">
										<span class='oState state-yck'>已出库</span>
									</c:if>
									<c:if test="${lt1.columns.outstock_type=='2' }">
										<span class='oState state-yxd'>已下单</span>
									</c:if>
								</c:if>
								
								<c:if test="${lt1.columns.status eq 4 }">
									<c:if test="${lt1.columns.outstock_type=='0' || lt1.columns.outstock_type=='1' }">
										<span class='oState state-waitPay'>待收款已出库</span>
									</c:if>
									<c:if test="${lt1.columns.outstock_type=='2' }">
										<span class='oState state-waitPay'>待收款已下单</span>
									</c:if>
								</c:if> 
							</td>
							<td class="w-120">
								...
								<p><a class="c-0e8ee7" onclick="detailMsg('${lt1.columns.id}','${lt1.columns.good_name}')"><i class="sficon sficon-view"></i>详情</a></p>
							</td>
						</tr>
					</table>
				</li>
...
var ifSuccess;
	$(function(){
		...
		$(".selection").css("width","140px");
		 $(".spPayWrap .closePopup").click(function() {
	        ...
	        setTimeout(function(){
	            window.location = "${ctx}/goods/platFormOrder/headerListMjl";
	        },250);
        });
	})
	...
	function search() {
		$('#pageNo').val('1');
		window.location.href="${ctx }/goods/platFormOrder/headerListMjl?map="+$("#searchForm").serialize();
		return;
	}
	...
	function detailMsg(rowId,name){ //订单详情
	...
	function exports(){
		...
		if(idArr>10000){
			$('body').popup({
				...
				content:content,
				fnConfirm :function(){
					location.href="${ctx}/goods/platFormOrder/exportMjl?formPath=/a/goods/platFormOrder/headerList&&maps="+$("#searchForm").serialize();
				}
	 ...
	 function xiadan(rowId){
	        $.ajax({
	            ...
	            success:function(result){
	            	...
	            	$("#xdGoodAmount").text(result.columns.jiage+"元");
	                ...
	                $("#goodAmount2").val(result.columns.jiage);
    	...
    	$.ajax({
			...
			success:function(data){
				if(data=='420'){
					...
					var href="${ctx}/goods/siteself/showDetail?id="+id+"&jp=o3";
				   	updateOrCreate(name,href);
				}else{
