...
<body>
				...
				<form id="orderForm">
									...
									<td>
										<div class="wrap1 text-l ">
											...
											<p class="pos-r pl-20 c-666 f-12 mt-5">
												...
												购买后可选择订单申请<span class="pos" style="margin-left: 140px;color: #ff6601">增值税发票！</span>
										...
										<div class="wrap1 w-140" style="margin: 0 auto;">
											<div class="countWrap w-140 mb-5">
												...
												<div class="priceWrap readonly">
													<input type="text" readonly name="quantity" class="input-text text-c readonly add_nums" value="1" />
													<span class="unit unitShow">箱</span>
												</div>
												<a class="btn-plus" onclick="addNum(this)"></a>
											</div>
											<input type="hidden" name="unit10A" value="个" />
											<input class="showType" hidden="hidden" name="showType" value="1" />
					...
					<div class="cl f-14">
						<div class="f-l">
							...
							<span class="c-fd6e32">
								￥
								<span id="yunfei" class="va-t">25.00</span>
							</span>
							（满3箱免运费）
						</div>
						<div class="f-r">
    ...
    $(function(){
    	...
    	if(isVIP){
            ...
            $(".orderZong").text((parseFloat(60)*parseFloat($(".NoVIPPrice").text())).toFixed(2));
            $(".realPrice").val((parseFloat(60)*parseFloat($(".VIPPrice").text())).toFixed(2));
        }else{
            $("#openVIP").removeClass("hide");
            $("#hadYouHui").addClass("hide");
            $(".orderZong").text((parseFloat(60)*parseFloat($(".NoVIPPrice").text())).toFixed(2));
            $(".realPrice").val((parseFloat(60)*parseFloat($(".NoVIPPrice").text())).toFixed(2));
        }
    ...
    function addGoods(de,id){
        ...
        $.post("${ctx}/goods/nanDao/getAllSouthIslandsNewSlt",{de:de,id:ids},function(result){
            if(result!=null){
                var html='';
    ...
    function realLogisPrice(val,num){
    	if(parseInt(num)>=180){
            $("#yunfei").empty().append('<del>'+val.toFixed(2)+'</del>');
        }else{
            $("#yunfei").text(val.toFixed(2));
        }
    }
  ...
  //计算应付总金额和已优惠金额
    function countSumMoney(){
	    ...
	    var numCount = 0;
   	    ...
   	    if(parseInt(numCount) >= parseInt(180)){
    ...
    function collectionOrderData(){
    	...
    	$("#goodsListCollection").find("tr").each(function(){//
    		...
    		if(nowTy=='1'){
    			nowNum = parseInt(nowNum) * parseInt(60);
    		}
    	...
    	if(parseInt(numsAll) >= parseInt(180)){
    		logisMark = "0";//无运费标识
    	};
    ...
    function checkout() {
        ...
        $.ajax({
            url: "${ctx}/goods/sitePlatformGoods/createSltDaoUpgradeOrderLatest",
            type: "POST",
            data: datas,
            success: function(data) {
                if (data.code == "200") {
