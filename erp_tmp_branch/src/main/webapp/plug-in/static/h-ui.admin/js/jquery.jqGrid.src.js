		...
		if(this.p.multiselect) {
			...
			$('#cb_'+$.jgrid.jqID(ts.p.id),this).bind('click',function(){
				...
				if(chk){
					...
					$("label[for='cb_"+ts.p.id+"']").css({"background-position":"-150px -120px"});
				}else{
					$("#cb_" + ts.p.id).attr("checked", false);
					$("label[for='cb_"+ts.p.id+"']").css({"background-position":"-150px -100px"});
				}
...
//					$("div span.s-ico",this).show();
		...
		$(grid.bDiv)
			.append($('<div style="position:relative;left:0px;'+(isMSIE && $.browser.version < 8 ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;border-bottom:0 none;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');
//				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height:'+hth+'px;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');
				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10)+ 1)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
	//			$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10) + 1)+'px;overflow-y:hidden;border-top: 1px solid #ccc;" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
