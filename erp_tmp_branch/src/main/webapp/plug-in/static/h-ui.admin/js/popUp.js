;(function ($) {
	$.fn.popup = function(options){
		...
		var $parentFrame = $('#Hui-article-box',window.top.document); // 获取父页面iframe所在的区域
		var openZindex = parseInt($('#Hui-aside', window.top.document).css('z-index'));
		var zIndex = parseInt($parentFrame.css('z-index'));
		$parentFrame.css({'z-index': settings.level*1000 +zIndex}); // 获取顶层页面iframe所在区域的层级
		
		// 弹出遮罩层
		
		var $shadeBg = $('<div class="shadeBg" id="shadeBg_'+settings.level+'" style="z-index:'+(settings.level*100)+';"></div>')
		...
		return $(this).each(function(){
			...
			if(settings.level == '3'){
				var html = '<div class="popupBox promptBox">'+
								'<h2 class="popupHead">' + settings.title + '<a href="javascript:;" class="sficon closePopup"></a></h2>'+
								'<div class="popupContainer">'+
									'<div class="popupMain  f-14" >';
				if( settings.type == 1){
					html += '<p class=""><i class="iconType iconType1"></i>' + settings.content+'</p>'+
							'<div class="text-c pl-10 mt-25">'+
								'<a href="javascript:;" class="sfbtn sfbtn-opt3 w-70" id="btn-confirm">关闭</a>'+
							'</div>';
					
				}else if( settings.type == 2){
					html += '<p class=""><i class="iconType iconType2"></i>' + settings.content+'</p>'+
							'<div class="text-c mt-25 ">'+
								'<a href="javascript:;" class="sfbtn sfbtn-opt3 w-70 mr-10 " id="btn-confirm">确认</a>'+
			...
			$this.css({'z-index': openZindex + settings.level*100});
		...
		var html = '';
			html += '<div class="popupBox addHeaders header_settable_container">'
				...
				for(var i = 0; i < unselItems.length; i++){
					...
					html += '<li><label class="label-cbox2" for="sfTbhUns_'+item_.name+'"><input type="checkbox" id="sfTbhUns_'+item_.name+'" tag="'+item_.label+'" name="optionchk">'+item_.label+'</label></li>';
				}
				html += '</ul></dd></dl>';
	...
	function addToRight(){
		...
		$(".optionalHead").find("input[type='checkbox']").each(function(){
			if($(this).is(":checked")){
				...
				var id = oldId.substring(9);
				items += "," + id + "@" + $(this).attr("tag");
			}
		});
		...
		for(var i = 0; i < len; i++){
			...
			selHtml += '</p></li>';
			$("#sfTbhUns_" + id, ".header_settable_container").parent().parent().remove();
		}
