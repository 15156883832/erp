...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var eg = $("<div class='ui-jqgrid ui-widget ui-widget-content2 ui-corner-all'></div>");
		...
		formatter = function (rowId, cellval , colpos, rwdat, _act){
			...
			if(cm.formatter !== undefined) {
				...
				if($.isFunction( cm.formatter ) ) {
					...
					if(cm.formatter.indexOf("SFFmt:") != -1){
						var  func=eval(cm.formatter.substring(6, cm.formatter.length));
						v = func.call(this, rwdat);
					}else{
		...
		addMulti = function(rowid,pos,irow,checked){
			var	v = "<label for=\"jqg2_"+ts.p.id+"_"+rowid+"\"><input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + (checked ? "checked=\"checked\"" : "")+"/></label>",
		...
		sortData = function (index, idxcol,reload,sor){
			...
			// add by jxm start
			if($("#cb_" + ts.p.id).attr("checked")){
				$("#cb_" + ts.p.id).attr("checked", false);
				$("label[for='cb_"+ts.p.id+"']").css({"background-position":"-120px -20px"});
			}
			// add by jxm end
			...
			if(!reload) {
				...
				// add by ljx start
				var index_tmp = index.substring(5 + ts.p.id.length + 1); // bad to be changed!?!
				var sortTag_tmp = ts.p.colModel[idxcol].index || index_tmp;
				var curSortNode = $("#jqgh_" + ts.p.id + "_" + sortTag_tmp).find("span");
				var curSelector = curSortNode.selector;
				// end
				// add by jxm start
				var preSortTag_tmp = ts.p.colModel[ts.p.lastsort].index;
				var preSortNode = $("#jqgh_" + ts.p.id + "_" + preSortTag_tmp).find("span");
				var preSelector = preSortNode.selector;
				
				// 默认ts.p.lastsort == 1
				if( ts.p.lastsort == idxcol ) {
					if( ts.p.sortorder == 'asc') {
						ts.p.sortorder = 'desc';
						$(".ui-icon-desc", $(curSelector)).removeClass('ui-icon-triangle-1-sa');
						$(".ui-icon-asc", $(curSelector)).addClass('ui-icon-triangle-1-na');
					} else if(ts.p.sortorder == 'desc') {
						ts.p.sortorder = 'asc';
						$(".ui-icon-asc", $(curSelector)).removeClass('ui-icon-triangle-1-na');
						$(".ui-icon-desc", $(curSelector)).addClass('ui-icon-triangle-1-sa');
					}
				} else {
					...
					$(".ui-icon-desc", $(curSelector)).addClass('ui-icon-triangle-1-sa');
					// add by jxm start
					$(".ui-icon-asc", $(preSelector)).removeClass('ui-icon-triangle-1-na');
					$(".ui-icon-desc", $(preSelector)).removeClass('ui-icon-triangle-1-sa');
				}
				ts.p.page = 1;
			}
			if(sor) {
...
//					$("span.s-ico",newSelectedTh).show();
		...
		if(this.p.multiselect) {
			this.p.colNames.unshift("<label for='cb_"+this.p.id+"'><input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox'/></label>");
			this.p.colModel.unshift({name:'cb',width:40,sortable:false,resizable:false,hidedlg:true,search:false,align:'center',fixed:true});
		}
		...
		if(this.p.multiselect) {
			...
			$('#cb_'+$.jgrid.jqID(ts.p.id),this).bind('click',function(){
				...
				if (this.checked) {
					...
					$('#jqgh_'+ts.p.id+'_cb').attr('class','ui-state-highlight');
				...
				// add by Ivan start
				// to modify origin multiSelect

				if(chk){
					$("#cb_" + ts.p.id).attr("checked", true);
					$("label[for='cb_"+ts.p.id+"']").css({"background-position":"-120px -40px"});
				}else{
					$("#cb_" + ts.p.id).attr("checked", false);
					$("label[for='cb_"+ts.p.id+"']").css({"background-position":"-120px -20px"});
				}
...
//					$("div span.s-ico",this).show();
		...
		$(grid.hDiv)
			...
			.addClass("ui-state-default2 ui-jqgrid-hdiv")
			.append(hb);
		$(hb).append(hTable);
		hTable = null;
		...
		$(grid.bDiv)
			.append($('<div style="position:relative;left:-1px;'+(isMSIE && $.browser.version < 8 ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
		...
		if(rdata) {
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(gi) {
						v = "<label for=\"jqg2_"+t.p.id+"_"+rowid+"\" ><input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+t.p.id+"_"+rowid+"\" class=\"cbox\"/></label>";
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');
//				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height:'+hth+'px;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');
//				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10)+ 1)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10) + 2)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
...
"use strict";	
	...
	$.fn.fmatter.checkbox =function(cval, opts) {
		...
		return "<label><input type=\"checkbox\" " + bchk  + " value=\""+ cval+"\" offval=\"no\" "+ds+ "/></label>";
	};
