...
public class SendedSmsService extends BaseService {
	...
	public void templeteSet(Map<String, Object> map) {
		...
		if("1".equals(type)){
			String sql = "update crm_site_sms_template a set a.reviewsms_status=?,a.failed_reason=? where number=? and a.status='0' and a.reviewsms_status='0' ";
			Db.update(sql, String.valueOf(Integer.parseInt(applyStatus)-1), applyReply, id);
