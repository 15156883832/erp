...
public class EmployeDao extends BaseDao<Employe> {
	...
	public List<Employe> getListEmp(Employe employe, String siteId) {
		...
		sf.append("SELECT * FROM crm_employe e WHERE e.site_id='" + siteId + "' AND e.status in ('0','3') order by e.status asc");
	...
	public List<Record> getEmployeOrder(String siteId) {
		...
		sf.append(" LEFT JOIN crm_order co ON co.employe_id = ce.id AND co.site_id = '" + siteId
				+ "'  AND co.status IN('2','3','4','5') AND DATE_FORMAT(co.create_time,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m')");
	...
	public List<Record> getEmployeOrder2(String siteId, String category) {
		SqlKit kit = new SqlKit().forceMaster().append("SELECT a.id, a.name, l.latitude, l.longitude, l.create_date,").
				...
				.append(" AND ( o.promise_time IS NULL AND DATE(aa.dispatch_time) = CURDATE()  OR  o.promise_time =CURDATE()) ")
				.append(") as wwg,")
				//append("0 as wwg,")
				.append("(SELECT COUNT(1) AS sywwg")
				.append("FROM crm_order_dispatch aa,crm_order_dispatch_employe_rel c,crm_order as o")
				.append("WHERE c.dispatch_id = aa.id")
				.append("and o.status='2'")
				.append("and o.id=aa.`order_id`")
...
/*
		SqlKit kit = new SqlKit().forceMaster().append("")
				.append("select a.id, a.name, l.latitude, l.longitude, l.create_date, ot.wwg, ot.sywwg, ot.jrywg")
				//.append("select a.id, a.name, l.latitude, l.longitude, l.create_date, 0 as wwg, 0 as sywwg, 0 as jrywg")
				.append("FROM crm_employe a")

				.append("left join (")
				.append("	select c.emp_id, (count(case when (aa.status in ('1', '2', '4')")
				.append("		and ( o.promise_time IS NULL AND DATE(aa.dispatch_time) = CURDATE()  OR  o.promise_time =CURDATE())")
				.append("	) then 1 end)) as wwg,")
				.append("	(count(case when (aa.status IN ('1', '2', '4')) then 1 end)) as sywwg,")
				//.append("	(aa.status = '5' and DATE(aa.end_time) = CURDATE()) as jrywg")
				.append("	0 as jrywg")
				.append("	from crm_order_dispatch aa,crm_order_dispatch_employe_rel c,crm_order as o")
				.append("	where 1=1")
				.append("	and aa.order_id = o.id")
				.append("	and c.dispatch_id = aa.id")
				.append("	and c.order_id = o.id")
				.append("	and aa.site_id = o.site_id")
				.append("	and c.site_id = o.site_id")
				.append("	and o.site_id = ?")
				.append(" 	and o.status = '2' ")
				.append("	and aa.status in ('1', '2', '5', '4')")
				.append("	group by c.emp_id")
				.append(") ot on ot.emp_id = a.id")
		...
		return Db.find(kit.toString(), siteId, siteId, siteId,siteId,siteId,siteId,siteId);
		//return Db.find(kit.toString(), siteId, siteId);
	}
	...
	public List<Record> empSearch(String siteId, Integer category, String name) {
		SqlKit kit = new SqlKit().append(" SELECT a.id, a.name, ot1.jrywg, ot1.jrgds, ot2.wwg, ot3.latitude, ot3.longitude, ot3.create_date")
				.append(" FROM crm_employe a LEFT JOIN (").append(" 	SELECT b.emp_id, b.emp_name, ").append(" COUNT(CASE WHEN a.status = '5' THEN 1 END) AS jrywg,")
				.append(" COUNT(a.id) AS jrgds").append(" FROM crm_order_dispatch a ")
				.append(" INNER JOIN crm_order_dispatch_employe_rel b ON b.dispatch_id = a.id AND b.site_id = ? ").append(" WHERE a.status != '6'")
				.append(" AND a.end_time = CURDATE() ").append(" AND a.site_id = ? ").append(" GROUP BY b.emp_id").append(" ) ot1 ON ot1.emp_id = a.id").append(" LEFT JOIN (")
				.append(" SELECT a.id AS disId, b.id AS odId, b.status, c.emp_id, c.emp_name, COUNT(1) AS wwg")
				.append(" FROM crm_order_dispatch a, crm_order b, crm_order_dispatch_employe_rel c, crm_employe d").append(" WHERE 1=1 ").append(" AND a.order_id = b.id")
				.append(" AND c.dispatch_id = a.id").append(" AND d.id = c.emp_id").append(" AND d.status = '0'").append(" AND b.site_id = ? ").append(" AND a.site_id = ? ")
				.append(" AND c.site_id = ? ").append(" AND d.site_id = ? ").append(" AND a.status IN ('1', '2', '4') AND b.status = '2'").append(" GROUP BY c.emp_id ")
				.append(" ) ot2 ON ot2.emp_id = a.id").append(" LEFT JOIN (").append(" SELECT gpr.role_id, gpr.latitude, gpr.longitude,gpr.create_date FROM (")
				.append(" SELECT a.id, a.role_id, a.latitude, a.longitude, a.create_date ").append(" FROM crm_gps_record a WHERE a.site_id = ? ")
				.append(" AND a.role_type = '4' ORDER BY a.create_date DESC").append(" ) gpr WHERE 1=1 ").append(" GROUP BY gpr.role_id").append(" ) ot3 ON ot3.role_id = a.id")
				.append(" WHERE a.site_id = ? ").append(" AND a.status = '0' ");
		if (category != null) {
	...
	public List<Record> getSiteEmploye(String siteId) {
		...
		// sb.append(" LEFT JOIN crm_order co ON co.employe_id = ce.id AND co.status='2'
		// AND DATE_FORMAT(co.create_time,'%Y-%m')=DATE_FORMAT(NOW(),'%Y-%m') ");
	...
	public List<Record> findBySiteId(String siteId) {
		StringBuffer sb = new StringBuffer("");
		sb.append(" SELECT *  FROM crm_employe a WHERE a.site_id = ? AND a.status in ('0','3') order by a.status asc ");
	...
	public List<Record> filterEmployee(String siteId, Page<Record> page, Map<String, String[]> filterMap) {
		...
		SqlKit kit = new SqlKit().append(
				"SELECT e.id AS id, e.user_id AS user_id, e.province AS province, e.city AS city, e.area AS `area`, e.address AS address, e.mobile AS mobile, e.name AS `name`, e.hiredate AS hiredate, e.category AS category, e.site_id AS site_id, u.login_name as login_name")
				.append("FROM crm_employe AS e").append("INNER JOIN crm_site AS s").append("ON e.site_id=s.id").append("INNER JOIN sys_user AS u").append("ON e.user_id=u.id")
				.append("WHERE e.status=0").append("AND e.site_id=?").append(sqlOrdersByFilter(filterMap, params))
	...
	public long countFilteredEmployee(String siteId, Page<Record> page, Map<String, String[]> filterMap) {
		List<String> params = new ArrayList<>();
		SqlKit kit = new SqlKit().append("SELECT COUNT(1) as count").append("FROM crm_employe AS e").append("INNER JOIN crm_site AS s").append("ON e.site_id=s.id")
				.append("INNER JOIN sys_user AS u").append("ON e.user_id=u.id").append("WHERE e.status=0").append("AND e.site_id=?").last(sqlOrdersByFilter(filterMap, params));
	...
	public String isLocEmploye(String empId) {
		Record ct = Db.findFirst(
				"SELECT a.role_id FROM crm_gps_record a WHERE a.role_id = '" + empId + "' and a.create_date >= '" + DateUtils.formatDate(new Date(), "yyyy-MM-dd") + " 00:00:00' ");
