...
public class NonServicemanService extends BaseService {
	...
	public String doQiYong(Map<String, Object> map) {
		...
		if ("4".equals(who)) {// 修改工程师状态
			...
			String sql = "update crm_employe e left join sys_user u on e.user_id=u.id set u.status='0',e.status='0' where e.id=?";
			Db.update(sql, map.get("id"));
			return "ok";
		} else {// 修改网点员工状态
			...
			String sql = "update crm_non_serviceman ns left join sys_user u on ns.user_id=u.id set u.status='0',ns.status='0' where ns.id=?";
			Db.update(sql, map.get("id"));
			return "ok";
		}
	}
	...
	public void doJinYong(Map<String, Object> map) {
		String who = map.get("who").toString();
		if ("4".equals(who)) {// 修改工程师状态
			String sql = "update crm_employe e left join sys_user u on e.user_id=u.id set u.status='3',e.status='3' where e.id=?";
			Db.update(sql, map.get("id"));
		} else {// 修改网点员工状态
			String sql = "update crm_non_serviceman ns left join sys_user u on ns.user_id=u.id set u.status='3',ns.status='3' where ns.id=?";
			Db.update(sql, map.get("id"));
		}
	}
	 ...
	 * @param map
	 */
	public void deleteMan(Map<String, Object> map) {
		String who = map.get("who").toString();
		if ("4".equals(who)) {// 修改工程师状态
...
//			String sqlt="select * from crm_category  where site_id ='' or site_id is null ";
	...
	public void doSave(Map<String, Object> map){
		...
		if("4".equals(map.get("userType")) || "2".equals(map.get("jobType"))){//服务工程师
			...
			emp.setName(StringUtil.trim(String.valueOf(map.get("name"))));
			...
			//serviceman表
			ns.setName(StringUtil.trim(String.valueOf(map.get("name"))));
			ns.setSite(s);
...
//			每次都会向关系表添加上数据
			...
			sfCacheService.hdel(SfCacheKey.nonServicemanCateBrandMap, u.getId());
			sfCacheService.hdelDelay(SfCacheKey.nonServicemanCateBrandMap, 2, u.getId());
			sfCacheService.delDelay(0, String.format(SfCacheKey.siteUserOrderCountMap, siteId)); // 当信息员重新关联品牌品类时，信息员的全部工单数量肯定是有变化的
			sfCacheService.delDelay(2, String.format(SfCacheKey.siteUserOrderCountMap, siteId));
		}
	}
...
//		Record rd = Db.findFirst(" select * from crm_site_role_permission a where a.sys_role_id = ? ", roleIds);
	...
	public Tuple<String, String> getNonServicemanCateAndBrand(String userId, String siteId) {
		String json = sfCacheService.hget(SfCacheKey.nonServicemanCateBrandMap, userId);
		if (StringUtil.isNotBlank(json)) {
			Map map = new Gson().fromJson(json, Map.class);
			return new Tuple<>((String) map.get("c"), (String) map.get("b"));
		}

		Tuple<String, String> cateAndBrand = doGetServicemanCateAndBrand(userId, siteId);
		Map<String, String> map = new HashMap<>();
		map.put("c", cateAndBrand.getVal1());
		map.put("b", cateAndBrand.getVal2());
		sfCacheService.hset(SfCacheKey.nonServicemanCateBrandMap, userId, new Gson().toJson(map));
		return cateAndBrand;
	}

	private Tuple<String, String> doGetServicemanCateAndBrand(String userId, String siteId) {
		Record record = Db.findFirst("select category,brand from crm_non_serviceman where user_id=? and site_id=? and status='0'", userId, siteId);
		String category = StringUtil.defaultString(record.getStr("category"), "");
		String brand = StringUtil.defaultString(record.getStr("brand"), "");
		return new Tuple<>(category, brand);
	}
	...
	public List<Record> getServiceManWorkCount(Map<String,Object> map,String siteId){

		List<Record> rdsNow = getServiceManWorkCount_now(map, siteId);
		List<Record> rdsHistory = getServiceManWorkCount_history(map, siteId);
		List<Record> finalRds = ActiveRecordUtil.combineRecords("manId", rdsNow, rdsHistory);

		return finalRds;
		//return rdsNow;
	}

	public List<Record> getServiceManWorkCount_now(Map<String,Object> map,String siteId){
		StringBuffer sb = new StringBuffer();
		/*信息员*/
		sb.append(" SELECT b.id as manId, b.name as serviceName, ");
		/*新建工单*/
		sb.append("(SELECT COUNT(*) FROM crm_order o ");
		sb.append(" WHERE  o.status NOT IN(6,8) AND o.create_by=a.`id` AND o.site_id='"+siteId+"'  ");
		if(StringUtils.isNotBlank((CharSequence) map.get("startTime"))){
			sb.append(" and o.create_time >='"+map.get("startTime")+" 00:00:00' ");
		}
		if(StringUtils.isNotBlank((CharSequence) map.get("endTime"))){
			sb.append(" and o.create_time <='"+map.get("endTime")+" 23:59:59' ");
		}
		sb.append(" ) AS buildOrderCount, ");

		/*派工单*/
		sb.append(" (SELECT COUNT(*) FROM crm_order_dispatch d");
		sb.append("  WHERE  d.status <> 7 AND d.messenger_id=b.`id` AND d.site_id='"+siteId+"'  ");
		if(StringUtils.isNotBlank((CharSequence) map.get("startTime"))){
			sb.append(" and d.dispatch_time >='"+map.get("startTime")+" 00:00:00' ");
		}
		if(StringUtils.isNotBlank((CharSequence) map.get("endTime"))){
			sb.append(" and d.dispatch_time <='"+map.get("endTime")+" 23:59:59' ");
		}
		sb.append(" ) AS dispatchOrderCount,");

		/*录单*/
		sb.append(" (SELECT COUNT(*) FROM crm_order o  ");
		sb.append(" WHERE  o.status NOT IN(6,8) AND o.record_account_by=a.`id` AND o.site_id='"+siteId+"'  ");
		if(StringUtils.isNotBlank((CharSequence) map.get("startTime"))){
			sb.append(" and o.record_account_time >='"+map.get("startTime")+" 00:00:00' ");
		}
		if(StringUtils.isNotBlank((CharSequence) map.get("endTime"))){
			sb.append(" and o.record_account_time <='"+map.get("endTime")+" 23:59:59' ");
		}
		sb.append(" ) AS recordAccountCount,");

		/*回访*/
		sb.append(" (SELECT COUNT(*) FROM crm_order_callback c  ");
		sb.append(" WHERE c.create_by=a.`id` AND c.site_id='"+siteId+"'  ");
		if(StringUtils.isNotBlank((CharSequence) map.get("startTime"))){
			sb.append(" and c.create_time >='"+map.get("startTime")+" 00:00:00' ");
		}
		if(StringUtils.isNotBlank((CharSequence) map.get("endTime"))){
			sb.append(" and c.create_time <='"+map.get("endTime")+" 23:59:59' ");
		}
		sb.append(" ) AS callBackCount,");

		/*结算*/
		sb.append(" (SELECT COUNT(*) FROM crm_order_settlement s ");
		sb.append("  WHERE s.create_by=a.`id` AND s.site_id='"+siteId+"'  ");
		if(StringUtils.isNotBlank((CharSequence) map.get("startTime"))){
			sb.append(" and s.create_time >='"+map.get("startTime")+" 00:00:00' ");
		}
		if(StringUtils.isNotBlank((CharSequence) map.get("endTime"))){
			sb.append(" and s.create_time <='"+map.get("endTime")+" 23:59:59' ");
		}
		sb.append(" ) AS settlementCount");
		sb.append(" FROM crm_non_serviceman_role_rel r ");
		sb.append(" INNER JOIN crm_non_serviceman b ON b.id = r.serviceman_id ");
		sb.append(" LEFT JOIN crm_site_role_permission p ON p.id = r.site_role_id ");
		sb.append(" LEFT JOIN sys_user a ON a.id=b.`user_id` AND a.status=0 ");
		sb.append(" WHERE b.status=0 AND b.site_id = '"+siteId+"' AND p.`sys_role_id`=1 ");
		if (StringUtils.isNotBlank((CharSequence) map.get("userIds"))) {
			sb.append(" and a.id in (" + StringUtil.joinInSql(map.get("userIds").toString().split(",")) + ") ");
		}
		return Db.find(sb.toString());
	}

	public List<Record> getServiceManWorkCount_history(Map<String,Object> map,String siteId){
		...
		sb.append(" SELECT b.id as manId, b.name as serviceName, ");
		/*新建工单*/
		sb.append("(SELECT COUNT(*) FROM "+tableSplitMapper.mapOrder(siteId)+" o ");
		sb.append(" WHERE  o.status NOT IN(6,8) AND o.create_by=a.`id` AND o.site_id='"+siteId+"'  ");
		if(StringUtils.isNotBlank((CharSequence) map.get("startTime"))){
			sb.append(" and o.create_time >='"+map.get("startTime")+" 00:00:00' ");
		}
		...
		/*派工单*/
		sb.append(" (SELECT COUNT(*) FROM "+tableSplitMapper.mapOrderDispatch(siteId)+" d");
		sb.append("  WHERE  d.status <> 7 AND d.messenger_id=b.`id` AND d.site_id='"+siteId+"'  ");
		if(StringUtils.isNotBlank((CharSequence) map.get("startTime"))){
			sb.append(" and d.dispatch_time >='"+map.get("startTime")+" 00:00:00' ");
		}
		...
		/*回访*/
		sb.append(" (SELECT COUNT(*) FROM "+tableSplitMapper.mapOrderCallback(siteId)+" c  ");
		sb.append(" WHERE c.create_by=a.`id` AND c.site_id='"+siteId+"'  ");
		if(StringUtils.isNotBlank((CharSequence) map.get("startTime"))){
			sb.append(" and c.create_time >='"+map.get("startTime")+" 00:00:00' ");
		}
		...
		/*结算*/
		sb.append(" (SELECT COUNT(*) FROM "+tableSplitMapper.mapOrderSettlement(siteId)+" s ");
		sb.append("  WHERE s.create_by=a.`id` AND s.site_id='"+siteId+"'  ");
		if(StringUtils.isNotBlank((CharSequence) map.get("startTime"))){
			sb.append(" and s.create_time >='"+map.get("startTime")+" 00:00:00' ");
		}
