...
public class SiteFittingKeepDao extends BaseDao<SiteFittingKeep> {
	...
	public List<Record> getListOfSiteFittingKeep(Page<Record> page, String siteId, Map<String, Object> map) {
		...
		sql.append("select sfk.id,concat(sfk.type,sfk.amount) as mx,sfk.fitting_code,sfk.fitting_name,sf.version,sfk.applicant,sfk.confirmor,sf.type,sfk.create_time, ");
	...
	public List<Record> getListOfSiteFittingKeep2(Page<Record> page, String siteId, Map<String, Object> map) {
		StringBuilder sql = new StringBuilder("");//缺少明细,备件来源
		sql.append("select sfk.id,sfk.type as ktype,sfk.amount ,sfk.fitting_code,sfk.fitting_name,sf.version,sfk.applicant,sfk.confirmor,sf.type,sfk.create_time,sfk.remarks, ");
		sql.append("sf.brand,sf.suit_category,sfk.price,sf.supplier ");
		sql.append("from crm_site_fitting_keep sfk ");
		sql.append("inner join crm_site_fitting sf on sfk.fitting_id = sf.id ");
		sql.append(" where sfk.site_id = ? and sfk.amount<>0 ");
	...
	public String getQuery(Map<String,Object> map){
		...
		if(map != null){
			...
			if(map.get("operationtype") != null && StringUtils.isNotEmpty((map.get("operationtype").toString()))){
				sf.append(" and sfk.type in ('6','a') ");
			}
	...
	public void updateFittingKeepById(String fiId,String remarks) {
		String sql = "UPDATE crm_site_fitting_keep  SET type = 'a' ,remarks =?  WHERE id =? ";
		Db.update(sql,remarks,fiId);
	}
	...
	public void updateFittingKeep(String ids) {
		...
		if(StringUtils.isNotBlank(ids)) {
			String[] id = ids.split(",");
			for(String fid :id) {
				String sql ="UPDATE crm_site_fitting_keep a  SET a.type = 'a'  WHERE id ='"+fid+"' ";
				list.add(sql);
			}
