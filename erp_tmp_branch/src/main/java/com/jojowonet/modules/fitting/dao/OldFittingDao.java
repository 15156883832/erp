...
public class OldFittingDao extends BaseDao<OldFitting> {
	...
	// 查询所有旧件信息(旧件库存)
		public List<Record> getAllStock(Page<Record> page, String siteId,
			...
			str.append(" WHERE ");
			str.append("  o.site_id='" + siteId + "'");
			str.append(" AND o.status in ('1','3','4','5') ");
			str.append(getqueryCriteria(map));
	...
	public long getOldStocksFittingCount(String siteId, 
		...
		sf.append(" WHERE ");
		sf.append("  o.site_id=?");
		sf.append(" and o.status in('1','3','4','5') ");
		if(map!=null){
			sf.append(getqueryCriteria(map));
		}
		return Db.queryLong(sf.toString(), siteId);
	...
	public String getqueryCriteria(Map<String, Object> ma) {
		...
		if (StringUtils.isNotEmpty((CharSequence) ma.get("brand"))) {
			String brand=ma.get("brand").toString().trim();
			sf.append(" and o.brand like '%" + brand + "%' ");
		}
		...
		if (StringUtils.isNotEmpty((CharSequence) ma.get("confirmName"))) {// 旧件状态
			sf.append(" and o.confirm_name = '" + ma.get("confirmName")+ "' ");
		}
		if (StringUtils.isNotEmpty((CharSequence) ma.get("employeName"))) {// 旧件状态
			sf.append(" and o.employe_name = '" + ma.get("employeName")+ "' ");
		}
	...
	// 查询返厂信息
			public List<Record> getReturnFactoryList(Page<Record> page, String siteId,Map<String, Object> map) {
				StringBuffer str = new StringBuffer();
				str.append("SELECT o.* ");
				str.append(" FROM crm_site_old_fitting o ");
				str.append(" WHERE ");
				str.append("  o.site_id='" + siteId + "'");
				str.append(" AND o.status in ('3','5') ");
				str.append(getqueryCriteria(map));
				str.append(createOrderBy(map," ORDER BY o.confirm_time DESC "));
				if (page != null) {
					str.append(" limit " + page.getPageSize() + " offset "
	...
	// 计算返厂总数(库存)
	public long getReturnFactoryCount(String siteId, Map<String, Object> map) {
		StringBuffer sf = new StringBuffer();
		sf.append(" SELECT count(*) as count FROM crm_site_old_fitting o ");
		sf.append(" WHERE ");
		sf.append("  o.site_id=?");
		sf.append(" and o.status in ('3','5') ");
		if(map!=null){
			sf.append(getqueryCriteria(map));
		}
		return Db.queryLong(sf.toString(), siteId);
	...
	public void updateFittingKeepById(String fiId,String remarks) {
		String sql = "UPDATE crm_site_old_fitting  SET status = '5' ,remarks = CONCAT(remarks,'"+remarks+"')  WHERE id =? ";
		Db.update(sql,fiId);
	}
	...
	public void updateFittingKeep(String ids) {
		...
		if(StringUtils.isNotBlank(ids)) {
			String[] id = ids.split(",");
			for(String fid :id) {
				String sql ="UPDATE crm_site_old_fitting  SET status = '5'  WHERE id ='"+fid+"' ";
				list.add(sql);
			}
	...
	*/
	public void DeleteoldFitting(String ids) {
		...
		if(StringUtils.isNotBlank(ids)) {
			String[] id = ids.split(",");
			for(String fid :id) {
				String sql ="DELETE FROM crm_site_old_fitting WHERE id = '"+fid+"' ";
				list.add(sql);
			}
			Db.batch(list, list.size());
		}
