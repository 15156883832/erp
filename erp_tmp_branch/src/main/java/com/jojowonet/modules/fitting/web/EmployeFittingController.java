...
public class EmployeFittingController extends BaseController {
	...
	public void BjEmpFit(HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		Order rd = orderService.getOrderByNumber(orderNumber, siteId);
		if (rd != null) {
			Db.update("update crm_order a set a.auxiliary_cost=? where a.number=? and a.site_id=?", cost, orderNumber, siteId);
		} else {
			Record r = order2017Service.findOrderByNumber(orderNumber, siteId);
			String orderTable = tableSplitMapper.mapOrder(siteId);
			if (r != null  && orderTable != null) {
				Db.update("update " + orderTable + " a set a.auxiliary_cost=? where a.number=? and a.site_id=?", cost, orderNumber, siteId);
			}
		}
	}
