...
import com.jojowonet.modules.sys.util.HxRefusedPusher;
...
public class FittingController extends BaseController {
    ...
    @Autowired
    private HxRefusedPusher hxRefusedPusher;
    @Autowired
     ...
     * 配件收支明细
     */
    @RequestMapping(value = {"balanceOfPayments"})
    public String balanceOfPayments(HttpServletRequest request, HttpServletResponse response, Model model) {
    	String siteId = CrmUtils.getCurrentSiteId(UserUtils.getUser());
    	SiteTableHeaderForm stf = JqGridTableUtils.getCustomizedTableHead(siteId,request.getServletPath());
    	model.addAttribute("headerData", stf);
    	model.addAttribute("siteId",siteId);
    	model.addAttribute("emps", employeService.getListEmp(null, siteId));
    	return "modules/" + "fitting/balanceOfPayments";
    }

    @ResponseBody
    @RequestMapping(value = "balanceOfPaymentsData")
    public String balanceOfPaymentsData(HttpServletRequest request, HttpServletResponse response, Model model) {
    	String siteId = CrmUtils.getCurrentSiteId(UserUtils.getUser());
    	Map<String, Object> map = request.getParameterMap();
    	Page<Record> page = new Page<>(request, response);
    	fittingService.getbalanceOfPaymentsList(page, siteId, map);
    	return renderJson(new JqGridPage<>(page));
    }
    /**
     * 配件收支汇总
     */
    @RequestMapping(value = {"fittingRetailvolume"})
    public String fittingRetailvolume(HttpServletRequest request, HttpServletResponse response, Model model) {
    	String siteId = CrmUtils.getCurrentSiteId(UserUtils.getUser());
    	SiteTableHeaderForm stf = JqGridTableUtils.getCustomizedTableHead(siteId,request.getServletPath());
    	model.addAttribute("headerData", stf);
    	model.addAttribute("siteId",siteId);
    	model.addAttribute("emps", employeService.getListEmp(null, siteId));
    	return "modules/" + "fitting/fittingRetailvolumeAll";
    }
    
    @ResponseBody
    @RequestMapping(value = "fittingRetailvolumeData")
    public String fittingRetailvolumeData(HttpServletRequest request, HttpServletResponse response, Model model) {
    	String siteId = CrmUtils.getCurrentSiteId(UserUtils.getUser());
    	Map<String, Object> map = request.getParameterMap();
    	Page<Record> page = new Page<>(request, response);
    	fittingService.getfittingRetailvolumeList(page, siteId, map);
    	return renderJson(new JqGridPage<>(page));
    }
    ...
    @RequestMapping("verifyRefused")
    @ResponseBody
    public Object verifyRefused(HttpServletRequest request, HttpServletResponse response, Model model) {
        String id = request.getParameter("id");
        FittingUsedRecord record = fittingUsedRecordService.get(id);
        fittingUsedRecordService.refuseHx(id);
        hxRefusedPusher.notifyHxRefused(record, UserUtils.getUser());
        return "ok";
    }
    ...
    public Object verify(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        return fittingService.verify(user, id, money, flag, hxType);
    }
    ...
    public List<Map<String,Object>> twoStatusCount(){
    	...
    	map3.put("zt", fittingApplyService.getFittingApplyCount3(siteId));
    	list.add(map1);
    	list.add(map2);
    	list.add(map3);
    	return list;
    ...
    public String exportFile(HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
        try {
            ...
            String[] emp  = request.getParameterValues("emps"); 
            if(StringUtils.isNotBlank(emp[0])||emp.length>1) {
            	maps.put("empss", emp);
            }
            maps.putAll(params);
           ...
           // jarray.remove(0);
            ...
            if ("核销历史记录".equals(title)) {
                Page<Record> pages = fittingService.getVerificationHistoryList(page1, siteId, maps);
                list=pages.getList();
                for (Record fit : list) {
                    ...
                    if("1".equals(fit.get("status"))) {
                        fit.set("status", "待核销");
                    } else if ("2".equals(fit.get("status"))) {
                        fit.set("status", "已核销");
                    } else if("3".equals(fit.get("status"))) {
                        fit.set("status", "已拒绝");
                    }
                }
            } else if("待核销".equals(title)){
                jarray.remove(0);
                Page<Record> pages = fittingService.getPreVerificationList(page1, siteId, maps);
                list=pages.getList();
                for (Record fit : list) {
                    if("1".equals(fit.getStr("type"))){
                        if("0".equals(fit.getStr("collection_flag"))){//0未收款
    ...
    @RequestMapping(value = "exportBalan")
    public String exportBalan(HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
    	try {
    		String formPath = request.getParameter("formPath");
    		String siteId = CrmUtils.getCurrentSiteId(UserUtils.getUser());
    		SiteTableHeaderForm stf = JqGridTableUtils.getCustomizedTableHead(siteId, formPath);
    		...
    		List<Record> list=null;
    			...
    			Page<Record> pages = fittingService.getbalanceOfPaymentsList(page1, siteId, params);
    			list=pages.getList();
    			for (Record fit : list) {
    				if("1".equals(fit.getStr("type"))){
    					if("0".equals(fit.getStr("collection_flag"))){//0未收款
    				...
    				if("1".equals(fit.getStr("collection_flag"))){
    					fit.set("profit1", fit.getBigDecimal("collection_money").subtract(fit.getBigDecimal("site_price")));
    					fit.set("profit2", fit.getBigDecimal("collection_money").subtract(fit.getBigDecimal("employe_price")));
    				}
    		...
    		e.printStackTrace();
    		addMessage(redirectAttributes, "导出收支明细失败！失败信息：" + e.getMessage());
    	}
    	return "redirect:" + Global.getAdminPath() + "/sys/user/?repage";
    }
    ...
    public String exportSummary(HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
    	try {
    		...
    		Page<Record> pages = fittingService.getfittingRetailvolumeList(page1, siteId, params);
    		list=pages.getList();
    		...
    		e.printStackTrace();
    		addMessage(redirectAttributes, "导出收支汇总失败！失败信息：" + e.getMessage());
    	}
    	return "redirect:" + Global.getAdminPath() + "/sys/user/?repage";
    }
