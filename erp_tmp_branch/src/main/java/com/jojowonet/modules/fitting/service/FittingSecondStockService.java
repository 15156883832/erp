...
public class FittingSecondStockService extends BaseService {
	...
	public List<Record> getSecondStockList(Page<Record> page, Map<String, Object> params, String siteId) {
		...
		sb.append(
				" SELECT a.*,b.name AS sName,sum(case when c.type = '1' then c.used_num else 0 end) as useNum FROM crm_site_fitting a INNER JOIN (SELECT m.id,m.name,n.site_type FROM crm_site_parent_rel n INNER JOIN crm_site m ON n.site_id=m.id AND m.status='0'");
		sb.append(
				"  WHERE n.status='0' AND n.parent_site_id= ? ) AS b ON a.site_id=b.id LEFT JOIN  crm_site_fitting_used_record AS c ON c.fitting_id=a.id  WHERE a.status='1' and b.site_type='1' ");
	...
	public Long getSecondStockCount(Map<String, Object> params, String siteId) {
		StringBuilder sb = new StringBuilder();
		sb.append(
				" SELECT count(*) FROM crm_site_fitting a INNER JOIN (SELECT m.id,n.site_type FROM crm_site_parent_rel n INNER JOIN crm_site m ON n.site_id=m.id AND m.status='0'");
		sb.append("  WHERE n.status='0' AND n.parent_site_id= ? ) AS b ON a.site_id=b.id  WHERE a.status='1' and b.site_type='1' ");
		sb.append(sqlConditions(params));
	...
	public List<Record> getSecondSiteList(String siteId) {
		return Db.find(
				"SELECT b.id,b.name FROM crm_site_parent_rel a INNER JOIN crm_site b ON a.site_id=b.id AND b.status='0' WHERE a.status='0' AND a.parent_site_id=? AND a.site_type='1' order by a.create_time,a.id asc",
				siteId);
	}
	...
	public Map<String, Object> getFittingAllStocks(String siteId, Map<String, Object> map) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append(
				" SELECT sum(a.warning) as allStocks FROM crm_site_fitting a INNER JOIN (SELECT m.id,n.site_type FROM crm_site_parent_rel n INNER JOIN crm_site m ON n.site_id=m.id AND m.status='0'");
		sb.append("  WHERE n.status='0' AND n.parent_site_id= ?) AS b ON a.site_id=b.id  WHERE a.status='1' and b.site_type='1' ");
