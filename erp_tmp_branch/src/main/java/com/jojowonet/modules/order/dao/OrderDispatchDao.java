...
public class OrderDispatchDao extends BaseDao<OrderDispatch> {
	...
	public List<Record> getDispatchRels2017(String dispId, String siteId){
		String table = tableSplitMapper.mapOrderDispatchEmployeRel(siteId);
		if (table == null) {
			return new ArrayList<>();
		}
		return Db.find(" select * from " + table + " a where a.dispatch_id = ? and a.site_id = ? ", dispId, siteId);
	}
	...
	public Record getOrderDispatchForCallBack2017(String orderId, String siteId){
		String table = tableSplitMapper.mapOrderDispatch(siteId);
		if (table == null) {
			return null;
		}

		return Db.findFirst("select * from " + table + " a where a.order_id = ? and a.site_id = ? ", orderId, siteId);
	}
	...
	public Record getOrderId2017(String orderId,String siteId){
		...
		String sql = " SELECT * FROM " + table + " a WHERE a.site_id=? AND a.status IN('1','2','4','5') AND a.order_id=? ";
		return Db.findFirst(sql,siteId,orderId);
	}
...
//		return Db.find("select distinct * from crm_promise_limit where status='0' order by sort asc");
	...
	public List<Record> getAppFeedbacks(String orderId){
		return Db.find(" SELECT * FROM crm_order_feedback WHERE order_id='"+orderId+"' and user_type = '4' ORDER BY feedback_time DESC ");
	}

	public List<Record> getAppFeedbacks2017(String orderId, String siteId) {
		String feedbackTable = tableSplitMapper.mapOrderFeedback(siteId);
		if (feedbackTable == null) {
			return null;
		}
		return Db.find(" SELECT * FROM " + feedbackTable + " WHERE order_id='" + orderId + "' and user_type = '4' ORDER BY feedback_time DESC ");
	}
...
//	}
	...
	public List<Record> getCollectionlistByOrderNo(String orderNo,String siteId){
		return Db.find("select a.* from crm_order_collections as a where a.order_number=? and a.site_id=? and a.status='0'",orderNo,siteId);
	}
}
