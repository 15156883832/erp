...
public class OrderFittingService extends BaseService {
	...
	public List<Record> getPjMsg(String orderNumber, String siteId, String remark) {
		...
		if ("SYMsg".equals(remark)) {
			...
			sql.append("select a.id ,a.fitting_code,a.fitting_version,a.fitting_name,a.fitting_img,a.audit_marks,a.status,a.fitting_apply_num,a.create_time");
	...
	public Record getOrderOr400(String orderNumber, String siteId) {
		...
		Record rd = orderDao.findOrderByNumberIfHistory(orderNumber, siteId);
		if (rd != null) {
			return rd;
		}
		return order400Dao.findOrder400ByNumber(orderNumber, siteId);
	}

	public Record getOrderOr400Rd(String orderId, String siteId) {
		Record rd = orderDao.findOrderByIdIfHistory(orderId, siteId);
		if (rd != null) {
			return rd;
		}
		return order400Dao.findOrder400ById(orderId, siteId);
	}
	...
	public Object deleteOneFittingRecord(String id, String siteId) {
		...
		if (rd != null) {
			...
			logger.info(String.format("[DF]about to delete fitting use record, but use record %s not found siteId %s ", id,siteId));
		}
		...
		if (empKeepFitList != null && empKeepFitList.size() > 0) {
			...
			logger.info(String.format("[DF] delete emp fitting keep:%d,fitting keep=%s,fittingId=%s,siteId=%s,deleteTime=%s", count, empKeepFitList.get(0),rd.getStr("fitting_id"),siteId,DateUtils.getDateTime()));
		}
		...
		logger.info(String.format("[DF] update emp fitting, number=%s,warning=%s,use record id=%s,c=%s,fittingId=%s,siteId=%s,EmpFIttingId=%s", num, num1, id, c1,rd.getStr("fitting_id"),siteId,empFit.getStr("id")));
		...
		logger.info(String.format("[DF] deleted use record,c=%d,id=%s,fittingId=%s,siteId=%s,employe_id=%s", update, rd.getStr("id"),rd.getStr("fitting_id"),siteId,rd.getStr("employe_id")));
