...
public class OrderCallBackService extends BaseService {
    ...
    public Record getCallBackInfo2017(String orderId, String siteId) {
        return callbackDao.getLatestOrderCallback2017(orderId, siteId);
    }

    public Record getCallBackInfoIfHistory(String orderId, String siteId) {
        Order order = orderDao.get(orderId);
        return order == null ? getCallBackInfo2017(orderId, siteId) : getCallBackInfo(orderId, siteId);
    }
    ...
    public void saveCallBack(OrderCallBack orderCallback, Map<String, Object> map) {
        ...
        if ("1".equals(map.get("result"))) {//已完成
            ...
            if(StringUtil.isNotBlank(order.getParentSiteId())) {
            	orderDao.update("update Order set returnCard = :p1, returnCardTime = :p2, warrantyType = :p3,  status='" + status + "' ,processDetail =:p4 , latest_process=:p5, latest_process_time=:p6,callbackResult=:p7,callback_cost=:p8,parent_status='5' where id = :p9 ",
