...
public class GoodsPlatformTransferOrderController extends BaseController {
	/*
	 * @Autowired private SiteselfOrderService siteselfOrderService;
	 * 
	...
	public String exports(HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
		try {
			...
			for (Record rd : list) {
				 ...
				 * if("漏电保护插头 10A（升级版）".equals(rd.getStr("good_name"))||"漏电保护插头 16A（升级版）".equals
				 * (rd.getStr("good_name"))){ rd.set("review_status","可开票"); }else{
				 * rd.set("review_status","不可开票"); } }else { rd.set("review_status","不可开票"); }
				 */
			}
			jarray.remove(0);
			jarray.remove(0);
	...
	public String outStockConfirm(HttpServletRequest request, HttpServletResponse response) {
		...
		if ("1".equals(tranOrder.getStatus())) {
			result = goodsPlatformTransferOrderService.outStockConfirm(id, logisticsName, logisticsNo, goodId, outAmount, outType, remark);
		}
		// 添加出库表当前库存
	...
	public String updateOrder(HttpServletRequest request) {
		...
		// 添加出库表当前库存
		if (!"0".equals(gpto.getStatus()) && !"1".equals(gpto.getStatus())) {
			goodsPlatformTransferOrderService.getPlatFormStockAndUpdateDetail(gpto.getGoodId(), request.getParameter("orderId"),
					BigDecimal.valueOf(Double.valueOf(request.getParameter("outAmount"))));
		}
		return result;
	}
	...
	public List<Record> getAllSouthIslands(HttpServletRequest request) {
		...
		String sql = null;
		if ("1".equals(de)) {
			sql = "select * from crm_goods_platform where brand='南岛' and id != '" + id + "' and sell_flag='1' and good_sign not in (" + StringUtil.joinInSql(names)
					+ ") and status='0' ";
		} else if ("2".equals(de)) {
			sql = "select * from crm_goods_platform where brand='南岛' and id = '" + id + "' and sell_flag='1' and good_sign not in (" + StringUtil.joinInSql(names)
					+ ") and status='0' ";
	...
	@ResponseBody
	@RequestMapping(value = "getAllSouthIslandsNew")
	public List<Record> getAllSouthIslandsNew(HttpServletRequest request) {
		String id = request.getParameter("id");
		String de = request.getParameter("de");
		String[] names = { "LB18102201", "LB18102202", "BS20180107", "BS20180108" };
		String sql = "select * from crm_goods_platform where brand='南岛' and id not in (" + StringUtil.joinInSql(id.split(",")) + ") and sell_flag='1' and good_sign in ("
				+ StringUtil.joinInSql(names) + ") and status='0' ";
		List<Record> list = Db.find(sql);
		for (Record re : list) {
			if (StringUtils.isNotBlank(re.getStr("icon"))) {
				re.set("picture", re.getStr("icon").split(",")[0]);
	...
	@ResponseBody
	@RequestMapping(value = "getAllSouthIslandsNewSlt")
	public List<Record> getAllSouthIslandsNewSlt(HttpServletRequest request) {
		String id = request.getParameter("id");
		String de = request.getParameter("de");
		String[] names = { "WT18040302", "WT18040303" };
		String sql = "select * from crm_goods_platform where  id not in (" + StringUtil.joinInSql(id.split(",")) + ") and sell_flag='1' and good_sign in ("
				+ StringUtil.joinInSql(names) + ") and status='0' ";
		List<Record> list = Db.find(sql);
		for (Record re : list) {
			if (StringUtils.isNotBlank(re.getStr("icon"))) {
	...
	public List<Record> getAllSouthIslandsUpgrade(HttpServletRequest request) {
		...
		String[] ids = id.split(",");
		String sql = "select * from crm_goods_platform where brand='南岛' and id not in (" + StringUtil.joinInSql(ids) + ") and good_sign not in (" + StringUtil.joinInSql(names)
				+ ") and sell_flag='1' and status='0' ";
	...
	public List<Record> getAllDetergent(HttpServletRequest request) {
		...
		String sql = "select * from crm_goods_platform where good_sign in (" + StringUtil.joinInSql(goodsSign) + ") and id not in (" + StringUtil.joinInSql(ids)
				+ ") and sell_flag='1' and status='0' ";
		List<Record> reList = Db.find(sql);
		for (Record re : reList) {
			if (StringUtils.isNotEmpty(re.getStr("icon"))) {
	...
	public String checkPriceForFlavorBox(HttpServletRequest request) {
		...
		if (Double.valueOf(quantity10) >= 144) {
			...
			BigDecimal value = BigDecimal.valueOf(Double.valueOf(price10A)).multiply(BigDecimal.valueOf(Double.valueOf(quantity10)))
					.add(BigDecimal.valueOf(Double.valueOf(logisticsPrice)));
			if (BigDecimal.valueOf(Double.valueOf(zong10A)).compareTo(value) != 0) {
				result = "fal";
			}
		}
	...
	public String checkPrice(HttpServletRequest request) {
		...
		String logisticsPrice = request.getParameter("logisticsPrice");// 运费
	...
	@ResponseBody
	@RequestMapping(value = "checkPriceNews")
	public String checkPriceNews(HttpServletRequest request) {
		String pid10A = request.getParameter("pid10A");
		String pid16A = request.getParameter("pid16A");
		String price10A = request.getParameter("price10A");
		String price16A = request.getParameter("price16A");
	...
	*/
	@ResponseBody
	@RequestMapping(value="getGoodsOrderBySite")
	public List<Record> getGoodsOrderBySite(HttpServletRequest request,HttpServletResponse response){
		String siteId = request.getParameter("siteId");
		if(ivan.common.utils.StringUtils.isBlank(siteId)) {
			return null;
		}
		List<Record> list = goodsPlatformTransferOrderDao.getGoodsOrderBySite(siteId);
		return list;
	}
}
