...
public class OrderStatDao {
	...
	public List<Record> getAllOrderStat(String siteId, String year) {
		List<Record> nowRds = getAllOrderStat_now(siteId, year);
		List<Record> historyRds = getAllOrderStat_history(siteId, year);
		List<Record> finalRds = ActiveRecordUtil.combineRecords("lgId", nowRds, historyRds);
		return finalRds;
		//return nowRds;
	}

	public List<Record> getAllOrderStat_now(String siteId, String year) {
		StringBuilder sb = new StringBuilder("");
		sb.append(" select DATE_FORMAT(a.repair_time,'%Y-%m') as lgId, count(1) as total, ");
	...
	public List<Record> getAllOrderStat_history(String siteId, String year) {
		...
		sb.append(" count(case when b.service_attitude in ('1','2') then 1 end) as bmyd, ");
		sb.append(" DATE_FORMAT(a.repair_time,'%m') as month  ");
		sb.append(" from "+tableSplitMapper.mapOrder(siteId)+" a left join ");
		sb.append(" "+tableSplitMapper.mapOrderCallback(siteId)+" b on b.order_id = a.id ");
		sb.append(" where a.site_id = ? ");
		sb.append(" and date_format(a.repair_time, '%Y') = ? ");
		sb.append(" group by DATE_FORMAT(a.repair_time,'%Y-%m') ");
		return Db.find(sb.toString(), siteId, year);
	...
	public List<Record> getAllEmployeOrderStat(String siteId, String start, String end, String empIds, Map<String, String> params) {
		List<Record> nowRds = getAllEmployeOrderStat_now(siteId, start, end, empIds, params);
		List<Record> historyRds = getAllEmployeOrderStat_history(siteId, start, end, empIds, params);
		List<Record> finalRds = ActiveRecordUtil.combineRecords("empId", nowRds, historyRds);
		return finalRds;
		//return nowRds;
	}

	public List<Record> getAllEmployeOrderStat_now(String siteId, String start, String end, String empIds, Map<String, String> params) {
		/*
		 * StringBuilder sb = new StringBuilder("");
	...
	public List<Record> getAllEmployeOrderStat_history(String siteId, String start, String end, String empIds, Map<String, String> params) {
		/*
		 * StringBuilder sb = new StringBuilder("");
		 * sb.append(" select a.employe_name, a.employe_id, count(1) as total, ");
		 * sb.append(" count(case when a.status = '5' then 1 end) as ywc, "); sb.
		...
		tmpSb.append("  from "+tableSplitMapper.mapOrderDispatch(siteId)+"  cd ");
		tmpSb.append("  inner join "+tableSplitMapper.mapOrder(siteId)+" a on a.id = cd.order_id and a.site_id = cd.site_id and a.status != '6' ");
		tmpSb.append("  inner join "+tableSplitMapper.mapOrderDispatchEmployeRel(siteId)+" odrel on odrel.dispatch_id = cd.id and odrel.order_id = a.id  ");
		tmpSb.append(" 	and odrel.site_id = cd.site_id ");

		tmpSb.append(" left join "+tableSplitMapper.mapOrderCallback(siteId)+" b on b.order_id = a.id and b.site_id = cd.site_id ");
		tmpSb.append("  where a.site_id = ? ");
		tmpSb.append("  and cd.status in ('1', '2', '4', '5') ");
		tmpSb.append("  and a.status != '6' ");
		// tmpSb.append(" and cd.`dispatch_time` >= '"+DateUtils.formatDate(stD,
		...
		tmpSb.append(" select a2.id, sum(case when a2.service_measures != '当日支付' then a2.sum_money else 0 end) as sum_money, a2.order_id, ");
		tmpSb.append("  a2.employe_id  ");
		tmpSb.append(" from "+tableSplitMapper.mapOrderSettlementDetail(siteId)+" a2, "+tableSplitMapper.mapOrder(siteId)+" b , "+tableSplitMapper.mapOrderDispatch(siteId)+" c ");
		tmpSb.append(" where b.id = a2.order_id and b.site_id = a2.site_id and c.site_id = a2.site_id and c.order_id = a2.order_id ");
		tmpSb.append(" and a2.employe_id != 'null' and a2.employe_id is not null ");
		tmpSb.append("  and a2.site_id = ? ");
		tmpSb.append("  and c.status in ('1', '2', '4', '5') ");
	...
	public Record getOrderCompleteStat(String siteId, String startStr, String endStr) {
		Record nowRd = getOrderCompleteStat_now(siteId, startStr, endStr);
		Record historyRd = getOrderCompleteStat_history(siteId, startStr, endStr);
		Record finalRd = ActiveRecordUtil.combineRecord(nowRd, historyRd);
		return finalRd;
		//return nowRd;
	}

	public Record getOrderCompleteStat_now(String siteId, String startStr, String endStr) {
		StringBuilder sb = new StringBuilder();
	...
	public Record getOrderCompleteStat_history(String siteId, String startStr, String endStr) {
		if(!tableSplitMapper.exists(siteId)){
			return new Record();
		}
		StringBuilder sb = new StringBuilder();
		...
		sb.append(" left join  "+tableSplitMapper.mapOrderCallback(siteId)+" b on b.order_id = a.id and b.site_id = ? ");
		sb.append(" where a.status in ('3', '4', '5') ");
		sb.append(" and a.site_id = ? ");
		if (StringUtils.isNotBlank(startStr)) {
			sb.append(" and a.end_time >= '" + startStr + "' ");
	...
	public List<Record> getServiceTypeSum(String siteId, String startStr, String endStr) {
		List<Record> nowRds = getServiceTypeSum_now(siteId, startStr, endStr);
		List<Record> historyRds = getServiceTypeSum_history(siteId, startStr, endStr);
		List<Record> finalRds = ActiveRecordUtil.combineRecords("service_type", nowRds, historyRds);
		return finalRds;
		//return nowRds;
	}

	public List<Record> getServiceTypeSum_now(String siteId, String startStr, String endStr) {
	...
	public List<Record> getServiceTypeSum_history(String siteId, String startStr, String endStr) {
		if(!tableSplitMapper.exists(siteId)){
			return Lists.newArrayList();
		}
		SqlKit kit = new SqlKit().append("SELECT service_type, COUNT(service_type) as cnt").append("FROM " + tableSplitMapper.mapOrder(siteId)).append("WHERE service_type IS NOT NULL")
				.append("AND site_id=? and status='5' ");
	...
	public Record getOrderDispatchStat(String siteId, String startStr, String endStr) {
		Record nowRd = getOrderDispatchStat_now(siteId, startStr, endStr);
		Record historyRd = getOrderDispatchStat_history(siteId, startStr, endStr);
		Record finalRd = ActiveRecordUtil.combineRecord(nowRd, historyRd);
		return finalRd;
		//return nowRd;
	}

	public Record getOrderDispatchStat_now(String siteId, String startStr, String endStr) {
		StringBuilder sb = new StringBuilder("");
		sb.append(" select ");
	...
	public Record getOrderDispatchStat_history(String siteId, String startStr, String endStr) {
		if(!tableSplitMapper.exists(siteId)){
			return new Record();
		}
		StringBuilder sb = new StringBuilder("");
		...
		sb.append(" count(1) as total, ");
		sb.append(" count(case when (a.status = '2' and (TIMESTAMPDIFF(HOUR, a.dispatch_time , a.process_time)) < 1 ) then 1 end) as jdjs ");
		sb.append(" from "+tableSplitMapper.mapOrderDispatch(siteId)+" a ");
		sb.append(" where a.site_id = ? ");
		if (StringUtils.isNotBlank(startStr)) {
			sb.append(" and a.dispatch_time >= '" + startStr + "' ");
		}
	...
	public List<Record> getbmygdlist(Page<Record> page, String year, String month, String siteId) {
		...
		sf.append("SELECT a.*,b.service_attitude AS service_attitude FROM crm_order a LEFT JOIN crm_order_callback AS b ON b.order_id=a.id WHERE a.site_id=? ");
		sf.append(" AND DATE_FORMAT(a.repair_time, '%Y') = ? AND DATE_FORMAT(a.repair_time,'%m')=? ");
		sf.append(" AND  b.service_attitude IN ('1','2')");
		if(tableSplitMapper.exists(siteId)){
			sf.append(" union all ");
			sf.append("SELECT a.*,b.service_attitude AS service_attitude FROM "+tableSplitMapper.mapOrder(siteId)+" a LEFT JOIN " +
					tableSplitMapper.mapOrderCallback(siteId)+" AS b ON b.order_id=a.id WHERE a.site_id=? ");
			sf.append(" AND DATE_FORMAT(a.repair_time, '%Y') = ? AND DATE_FORMAT(a.repair_time,'%m')=? ");
			sf.append(" AND  b.service_attitude IN ('1','2')");
		}
		sf.append(" ) as ot ");
		...
		if(tableSplitMapper.exists(siteId)){
			return Db.find(sf.toString(), siteId, year, month, siteId, year, month);
		}
	...
	public long getbmycount(String year, String month, String siteId) {
		...
		sf.append(" AND DATE_FORMAT(a.repair_time, '%Y') = ? AND DATE_FORMAT(a.repair_time,'%m')=? ");
		sf.append(" AND  b.service_attitude IN ('1','2')");
		long l1 = Db.queryLong(sf.toString(), siteId, year, month);
		long l2 = 0l;
		if(tableSplitMapper.exists(siteId)){
			sf = new StringBuffer();
			sf.append("SELECT count(*) FROM "+tableSplitMapper.mapOrder(siteId)+" a LEFT JOIN "+tableSplitMapper.mapOrderCallback(siteId)+" AS b ON b.order_id=a.id WHERE a.site_id=? ");
			sf.append(" AND DATE_FORMAT(a.repair_time, '%Y') = ? AND DATE_FORMAT(a.repair_time,'%m')=? ");
			sf.append(" AND  b.service_attitude IN ('1','2')");
			l2 = Db.queryLong(sf.toString(), siteId, year, month);
		}
