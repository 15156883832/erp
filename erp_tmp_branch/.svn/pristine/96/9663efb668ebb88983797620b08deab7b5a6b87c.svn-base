			...
			<div class="pcontent pl-15 pt-10"><div class="w-830">
				...
				<div class="cl mb-10">
					...
					<div class="priceWrap w-100 f-l factoryPriceWrap">
						<input type="text" class="input-text" id="ffee" value="${st.orderSettlement.columns.factory_money}" onkeyup="updatePVAndCalcOverall();"/>
				...
				<div class="cl mb-10">
					...
					<div id="switch2" class="switch f-l mr-10" data-on-label="有" data-off-label="无">
						<input id="accCostCheck" type="checkbox" <c:if test="${st.orderSettlement.columns.fitting_costs > 0}">checked</c:if>/>
					</div>
					<div class="f-l fucaiCostWrap <c:if test='${st.orderSettlement.columns.fitting_costs == 0}'>hide</c:if>">
						<label class="text-r f-l ">公司成本：</label>
						<div class="priceWrap w-80 f-l " >
							<input type="text" class="input-text"  value="${st.orderSettlement.columns.fitting_costs}" id="afee" onkeyup="calcOverall();"/>
							<span class="unit">元</span>
						</div>
					...
					<div class="pos-r pl-160 ">
						...
						<div class="cl" style="min-height: 26px;">
							<c:forEach items="${st.dispEmpSettlementDetail}" var="detail">
								<div class="f-l mb-10">
									<label class="f-l">${detail.columns.employe_name}：</label>
									<div class="priceWrap w-70 f-l readonly mr-15">
										<input type="text" class="input-text empi" readonly="readonly" value="${detail.columns.sum_money}"/>
										<input type="hidden" class="emp-ratio" value="${detail.columns.ratio}">
										<input type="hidden" class="emp-id" value="${detail.columns.employe_id}">
										<span class="unit">元</span>
									</div>
								</div>
							</c:forEach>
						...
						<div class="sdefined sdefined99999 cl">
							...
							<div class="f-l mb-10" style="width: 774px">
								<div class="pos-r pl-90 ml-10">
									...
									<input type="text" class="input-text tmpl-memo" style="width:100%;" value="${st.orderSettlement.columns.remarks}">
								</div>
							</div>
						</div>
				...
				<div class="cl mb-10 mt-10">
					...
					<input type="text" onfocus="WdatePicker({})" id="bxdatemax" name="bxdatemax" value="<fmt:formatDate value="${st.orderSettlement.columns.settlement_time}" pattern="yyyy-MM-dd"/>" class="input-text Wdate w-140 f-l" >
				</div>
				<!-- <div class="pos-r pl-90 ml-10">
					<label class="w-90 text-r lb">备注：</label>
				...
				<div class="cl mb-10">
					...
					<div class="priceWrap w-100 f-l">
						<input type="text" class="input-text" value="${st.orderSettlement.columns.profits}" id="profit"/>
						<span class="unit">元</span>
					</div>
		...
		<div class="popupContainer pos-r">
			<div class="popupMain pt-30 pb-15">
				<c:forEach items="${st.dispEmpSettlementDetail}" var="emp">
					<div class="cl mb-10">
						<label class="f-l w-140 text-r"><span class="emp-name-sp">${emp.columns.employe_name}</span>：</label>
						<div class="priceWrap w-160 f-l">
...
//            accCostEnabled = data.value;
        ...
        <c:forEach var="item" items="${st.addedEmpSettlementDetail}">
			...
			idx++;
        	createAddedSettlementTR('${item.columns.employe_id}', '${item.columns.employe_name}', mapping['${item.columns.service_measures}'], '${item.columns.service_measures}', '${item.columns.sum_money}', '<c:out value="${item.columns.remarks}"/>', false, new Date().getTime() + idx);
        </c:forEach>
    });
...
//		parent.closeJiesuanForm();
    ...
    function saveSettlement() {
        ...
        formPosted = true;
        $.ajax({
            url: "${ctx}/order/settlement/saveEdit2017",
            type: 'POST',
