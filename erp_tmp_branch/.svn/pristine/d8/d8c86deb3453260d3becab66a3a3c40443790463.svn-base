					...
					<c:choose>
						...
						<c:otherwise>
							...
							<div class="cl mb-10">
						 	...
						 	<div class="cl mb-10">
								<label class="f-l w-80">厂家编号：</label>
								<input type="text" class="input-text w-160 readonly f-l" readonly="readonly" value="${order.columns.factory_number }"/>
								<label class="f-l w-80">备注：</label>
								<input type="text" class="input-text w-360 readonly f-l" readonly="readonly" value="${order.columns.remarks }"/>
	...
	<div class="popupContainer">
		<div class="popupMain pd-20">
			...
			<div class="cl mb-10">
				...
				<input type="hidden" value="${fa.fittingId}" id="fittingIdFa"/>
...
<script type="text/javascript">
	...
	function fittingPlan(){
        ...
        var fittingId = $("#fittingId").val();//更换后的配件id
        var fiId = $("#fittingIdFa").val();//更换前的配件id
        if(isBlank(fittingId)){
        	 layer.msg("请确认备件条码，且该备件是否存在于库存中!");
        	 return
        }else if(fittingId != fiId){
        	 layer.msg("请先进行保存操作!");
        	 return
        }
        $.ajax({
