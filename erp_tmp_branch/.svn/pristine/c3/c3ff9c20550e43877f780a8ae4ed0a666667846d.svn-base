...
<p>
<span>工单编号：${order.columns.number }</span>
<%-- <span>派单时间：<fmt:formatDate value="${order.columns.dispatch_time }" pattern="yyyy-MM-dd hh:mm"/></span> --%>
...
<table border="1" cellpadding="0" cellspacing="0" >
    ...
    <tr>
        <td>用户姓名：</td>
        <td colspan="2">${order.columns.customer_name }</td>
        <td>详细地址：</td>
        <td colspan="5">
        <c:if test="${order.columns.province eq order.columns.city}">
				详细地址：${order.columns.city}${order.columns.area}${order.columns.customer_address}
			</c:if>
			<c:if test="${order.columns.province ne order.columns.city}">
				${order.columns.province}${order.columns.city}${order.columns.area}${order.columns.customer_address}
			</c:if>
        </td>
    </tr>
    <tr>
        <td>联系电话1：</td>
        <td colspan="2">${order.columns.customer_mobile }</td>
        <td>联系电话2：</td>
        <td colspan="2">${order.columns.customer_telephone }</td>
        <td>联系电话3：</td>
        <td colspan="2">${order.columns.customer_telephone2 }</td>
    </tr>
    <tr>
        <td>服务类型：</td>
        <td colspan="2">${order.columns.service_type }</td>
        <td>服务方式:</td>
        <td colspan="2">${order.columns.service_mode }</td>
        <td>保修类型：</td>
        <td colspan="2"><c:if test="${order.columns.warranty_type eq '1'}">保内</c:if>
				<c:if test="${order.columns.warranty_type eq '2'}">保外</c:if>
				<c:if test="${order.columns.warranty_type eq '3'}">保外转保内</c:if></td>
    </tr>

    <tr>
        <td>家电品牌：</td>
        <td colspan="2">${order.columns.appliance_brand }</td>
        <td>家电品类：</td>
        <td colspan="2">${order.columns.appliance_category }</td>
        <td>家电数量：</td>
        <td colspan="2">${order.columns.appliance_num }</td>
    </tr>
    <tr>
        <td>产品型号：</td>
        <td colspan="2">${order.columns.appliance_model }</td>
        <td>内机条码：</td>
        <td colspan="2">${order.columns.appliance_barcode }</td>
        <td>外机条码：</td>
        <td colspan="2">${order.columns.appliance_machine_code }</td>
    </tr>
    <tr>
        <td>受理时间：</td>
        <td colspan="2"><fmt:formatDate value="${order.columns.create_time }" pattern="yyyy-MM-dd hh:mm"/></td>
        <td>购买时间：</td>
        <td colspan="2"><fmt:formatDate value="${order.columns.appliance_buy_time }" pattern="yyyy-MM-dd"/></td>
        <td>完工时间：</td>
        <td colspan="2"><fmt:formatDate value="${rd.columns.end_time }" pattern="yyyy-MM-dd hh:mm"/></td>
    </tr>
    <tr>
        <td>信息描述：</td>
        <td colspan="8">${order.columns.customer_feedback }  ${order.columns.remarks }</td>
    </tr>
    <tr>
    ...
    <tr>
        <td>接单员：</td>
        <td colspan="2">${order.columns.createName}</td>
        <td>服务评价：</td>
        <td colspan="5">
     ...
     <tr>
        <td colspan="9">
        <p class="fu"><span>服务人员:${order.columns.employe_name}</span><span>用户签名:</span></p></td>
    </tr>
    <tr>
