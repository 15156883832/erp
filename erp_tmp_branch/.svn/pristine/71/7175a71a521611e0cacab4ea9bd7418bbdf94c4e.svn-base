	...
	<div class="sfpagebg bk-gray">
		<div class="page-orderWait goodsPage">
			<div class="tabBar cl mb-10">
				...
				<sfTags:pagePermission authFlag="GOODSMGM_EMPLOYEBUY_EMPLOYEBUYSTOCKS_TAB" html='	<a class="btn-tabBar current " href="${ctx}/goods/siteself/BuyEmployeSite">工程师自购库存</a>'></sfTags:pagePermission>
				<sfTags:pagePermission authFlag="GOODSMGM_EMPLOYEBUY_EMPLOYEBUYSRECORD_TAB" html='<a class="btn-tabBar " href="${ctx}/goods/siteGoodsKeep/buyEmpList">自购库存明细</a> '></sfTags:pagePermission>
			...
			<div class="pt-10 pb-5 cl btnsWrap">
				...
				<div class="f-r">
					<sfTags:pagePermission authFlag="GOODSMGM_EMPLOYEBUY_EMPLOYEBUYSTOCKS_EXPORTS_BTN" html='<a onclick="return exports()"class="sfbtn sfbtn-opt2" target="_blank"><i class="sficon sficon-export"></i>导出</a>'></sfTags:pagePermission>
				</div>
			</div>
				...
				<table class="table table-bg text-c goodsTable">
					<c:forEach items="${page.list }" var="lt1">
						<tr class="bb">
							...
							<td class="text-l w-500" style="cursor: pointer;" onclick="showDetail('${lt1.columns.good_id}','${lt1.columns.goodsName}')">
								<div class="imgTD">
							...
							<td class="w-150" >${lt1.columns.zg_stocks }/${lt1.columns.unit }</td>
	...
	<div class="popupContainer pt-15">
		...
		<div class="popupMain pl-15 pr-15 pb-15 pt-30">
			...
			<div class="cl mb-20">
				...
				<label class="f-l w-90"><em class="mark">*</em>退还金额：</label>
				<div class="priceWrap f-l w-140">
					<input type="text" class="input-text" name="thMoney" id="thMoney" />
...
<script type="text/javascript">
	...
	$(function(){//返还：GOODSMGM_EMPLOYEBUY_EMPLOYEBUYSTOCKS_RETURNTOSITE_BTN
	...
	function exports(){
		...
		if(idArr>10000){
			$('body').popup({
				...
				content:content,
				 fnConfirm :function(){
					 location.href="${ctx}/goods/siteself/exportEmpZg?formPath=/a/goods/siteself/WholeEmployeSiteZg&&maps="+$("#searchForm").serialize(); 
				 }
	...
	function search() {
		$('#pageNo').val('1');
		window.location.href="${ctx}/goods/siteself/BuyEmployeSite?map="+$("#searchForm").serialize();
		return;
	}
	...
	//商品详情
	 function showDetail(id,name){
		var href="${ctx}/goods/siteself/showDetail?id="+id+"&jp=s22";
	...
	function showFH() {
		...
		$.ajax({
			...
			success : function(result) {
				...
				$("#lyNumber").text(result.columns.gNumber);
				...
				$("#hideStocks").val(result.columns.zg_stocks);
				$("#empOwnId").val(result.columns.id);
				$("#lyUnit").text(result.columns.zg_stocks+result.columns.gUnit);
	 ...
	 function doReturn(){
		 ...
		 if(parseFloat(nums) > parseFloat(0) ){
			 if(parseFloat(nums) > parseFloat($("#hideStocks").val())){
				 layer.msg("返还数量不能大于工程师自购库存数量！");
				 return;
			 }
			 if(!checkShiShouMy(thMoney)){
				 layer.msg("退还金额格式不正确！");
				 $("#thMoney").focus();
				 return ;
			 }
			 ...
			 $.ajax({
				 type:"post",
				 url:"${ctx}/goods/siteself/doReturnGoodsZg",
				 data:{id:$("#empOwnId").val(),nums:nums,thMoney:thMoney},
				 success:function(result){
					 rtMark = false;
