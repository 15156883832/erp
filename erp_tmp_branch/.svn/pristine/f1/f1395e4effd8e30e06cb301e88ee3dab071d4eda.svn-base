...
<div class="page-orderWait">
		<div class="tabBar cl mb-10">
			...
			<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_REFUSEORDER_TAB" html='<a class="btn-tabBar current" href="${ctx }/order/getWwgList/jjgd">拒接工单<sup id="tab_c3">0</sup></a>'></sfTags:pagePermission>
			...
			<div class="pt-10 pb-5 cl">
				<div class="f-l">
				<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_REFUSEORDER_NEWORDER_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt" onclick="addNew();"><i class="sficon sficon-add"></i>新建工单</a>'></sfTags:pagePermission>
				<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_REFUSEORDER_DISPATCH_BTN" html='<a href="javascript:directDis();" class="sfbtn sfbtn-opt"><i class="sficon sficon-dispatch"></i>派工</a>'></sfTags:pagePermission>
				<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_REFUSEORDER_ZBDISPATCH_BTN" html='<a href="javascript:emporarily();" class="sfbtn sfbtn-opt"><i class="sficon sficon-notdispatch"></i>暂不派工</a>'></sfTags:pagePermission>
				<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_REFUSEORDER_WXORDER_BTN" html='<a href="javascript:showwxgd();" class="sfbtn sfbtn-opt"><i class="sficon sficon-invalid"></i>无效工单</a>'></sfTags:pagePermission>
				...
				<div class="f-r">
					...
					<%-- <sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_REFUSEORDER_IMPORT_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt2"><i class="sficon sficon-import"></i>导入</a>'></sfTags:pagePermission> --%>
					<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_REFUSEORDER_EXPORT_BTN" html='<a  onclick="return exports()"class="sfbtn sfbtn-opt2" target="_blank"><i class="sficon sficon-export"></i>导出</a>'></sfTags:pagePermission>
					<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_REFUSEORDER_TBSETTLE_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt2" id="setHeadersBtn"><i class="sficon sficon-setting"></i>表头设置</a>'></sfTags:pagePermission>
				</div>
...
function initSfGrid(){
	$("#table-waitdispatch").sfGrid({
		url : '${ctx}/order/getOrderMenuList/jjgd?ptype='+${ptype},
		sfHeader: defaultHeader,
		sfSortColumns: sortHeader,
		rownumbers:true,
		gridComplete:function(){
			_order_comm.gridNum();
			$(".ui-jqgrid-labels").find("th:first").css("width","50px");
			$("#table-waitdispatch").find("td:first").css("width","50px");
...
function gridNum(){
	$(".ui-jqgrid-labels").find("th:first").text("序号");
	$(".ui-jqgrid-labels").find("th:first").css("width","50px");
	$("#table-waitdispatch").find("td:first").css("width","50px");
	var sfMark ="0";
	$("#table-waitdispatch").find("tr").find("td:first").each(function(index,item){
	...
	if(sfMark=="2"){
		$(".ui-jqgrid-labels").find("th:first").css("width","60px");
		$("#table-waitdispatch").find("td:first").css("width","60px");
	} 
	if(sfMark=="3"){
		$(".ui-jqgrid-labels").find("th:first").css("width","75px");
		$("#table-waitdispatch").find("td:first").css("width","75px");
	} 
	if(sfMark=="4"){
		$(".ui-jqgrid-labels").find("th:first").css("width","90px");
		$("#table-waitdispatch").find("td:first").css("width","90px");
	} 
	if(sfMark=="5"){
		$(".ui-jqgrid-labels").find("th:first").css("width","105px");
		$("#table-waitdispatch").find("td:first").css("width","105px");
	} 
}
   ...
   /*  if (nowM >= start && nowM <= end) {
	...
	if(idArr>10000){
		$('body').popup({
			...
			content:content,
			 fnConfirm :function(){
				 location.href="${ctx}/order/export?formPath=/a/order/getWwgList/jjgd&&maps="+$("#searchForm").serialize();
			 },
fnCancel: function () {
