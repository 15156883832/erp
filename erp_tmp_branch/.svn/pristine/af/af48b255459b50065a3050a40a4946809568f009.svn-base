...
public class SitePlatformGoodsService {
	...
	// 获取 服务商 商品线上付款交易记录
	public Page<Record> getPlatformGoodsRecordPayed(Page<Record> page, String siteId, Map<String, Object> map) {
	...
	// 获取服务商平台商品list
	public Page<Record> getSitePlatformGoodslist(Page<Record> page, String siteId, Map<String, Object> map) {
	...
	// 获取服务商平台商品list
	public Page<Record> getSitePlatformGoodslistSiteShow(Page<Record> page, String siteId, Map<String, Object> map) {

		List<Record> list = sitePlatformGoodsDao.getSitePlatformGoodsListSiteShow(page, siteId, map);
		for (Record re : list) {
			if (StringUtils.isNotBlank(re.getStr("icon"))) {
				String[] images = re.getStr("icon").split(",");
				re.set("icon", images[0]);
	...
	public GoodsPlatformOrder scPlatOrder1(String outTradeNo, String countMoney, String zfType, String sbJect, String gmMonth, String name, String passWord, String mobile,
			String province, String city, String area, String address, String shareCode) {
		GoodsPlatformOrder gpo = new GoodsPlatformOrder();
	...
	public List<Record> historyOrder(String siteId) {
		return Db.find(
				"select a.*,b.review_status as review_status from crm_goods_platform_order a left join crm_invoice_application as b on a.invoice_record_id=b.id where a.pay_status='1' and a.good_id is null and a.site_id='"
						+ siteId + "'");
	}
	...
	public List<Map<String, Object>> timeDistinct(String siteId, String gmMonth) {
		...
		if (rd.getDate("due_time") == null || (rd.getDate("due_time").getTime() < date.getTime())) {
			Record rd1 = Db.findFirst(
					"select DATE_FORMAT(date_add(date_add(now(),interval " + month + " month),INTERVAL -1 DAY),'%Y.%c.%e') as end1,DATE_FORMAT(now(),'%Y.%c.%e') as start1");
			start1 = rd1.getStr("start1");
			end1 = rd1.getStr("end1");
		} else {
			Record rd2 = Db.findFirst("select DATE_FORMAT(date_add(date_add('" + rd.getDate("due_time") + "',interval " + month
					+ " month),INTERVAL -1 DAY),'%Y.%c.%e') as end1,DATE_FORMAT('" + rd.getDate("due_time") + "','%Y.%c.%e') as start1");
	...
	public List<Record> getPlatGoodsMsgByIds(String ids) {
		return Db.find("select a.* from crm_goods_platform a where a.id in (" + StringUtil.joinInSql(ids.split(",")) + ")");
	}
}
