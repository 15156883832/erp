...
public class SiteDao extends BaseDao<Site> {
	...
	public static List<Record> getTraderSiteList(String name,String ids){
		StringBuilder sf = new StringBuilder().append("SELECT id,IFNULL(contacts,'') as contacts,IFNULL(province,'') as province,"
				+ "IFNULL(city,'') as city,IFNULL(area,'') as area,IFNULL(address,'') as address,IFNULL(name,'') as name,IFNULL(mobile,'') as mobile FROM crm_site WHERE  status= '0' ");
		if(StringUtils.isNotBlank(name)) {
			sf.append("and name LIKE '%"+name+"%' ");
		}
		if(StringUtils.isNotBlank(ids)) {
			String siteId[] = ids.split(",");
			sf.append("and id in (" + StringUtil.joinInSql(siteId) + ")");
		}
