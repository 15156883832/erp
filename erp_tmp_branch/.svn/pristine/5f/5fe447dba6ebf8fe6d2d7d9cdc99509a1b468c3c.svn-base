			...
			<div id="tab-system" class="HuiTab">
				<div class="tabBar cl mb-10">
					...
					<c:if test="${map.guomei eq 'has'}">
						<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_GUOMEI_TAB" html='<a id="guomei" class="btn-tabBar current "  href="${ctx }/order/ChangeSelfOrder/guomeiHeaderList">国美工单</a>'></sfTags:pagePermission>
					</c:if>
				...
				<div class="tabCon">
									...
									<td>
										<span class="w-140">
											<select class="select easyui-combobox"  id="statusFlag" multiple="true" multiline="false" name="status" style="width:100%;height:25px" panelMaxHeight="300px">
												<option value="已派一级网点">已派一级网点</option>
												<option value="一级网点接单">一级网点接单</option>
												<option value="已派二级网点">已派二级网点</option>
												<option value="二级网点接单">二级网点接单</option>
												<option value="已派师傅">已派师傅</option>
												<option value="准备安装">准备安装</option>
												<option value="安装中">安装中</option>
												<!-- 完工状态 -->
												<option value="安装完成">安装完成</option>
												<option value="评价完成">评价完成</option>
												<option value="回执完成">回执完成</option>
												<option value="待网点审核">待网点审核</option>
												<option value="待分部审核">待分部审核</option>
					...
					<div class="pt-10 pb-5 cl">
						<div class="f-l">
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_GUOMEI_ZZJ_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt" onclick="zzj();"><i class="sficon sficon-zzj"></i>转自接</a>'></sfTags:pagePermission>
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_GUOMEI_PLDELETE_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt" onclick="delMore();"><i class="sficon sficon-rubbish"></i>批量删除</a>'></sfTags:pagePermission>
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_GUOMEI_MARKORDER_BTN" html='<a href="javascript:showMarkOrders();" class="sfbtn sfbtn-opt"><i class="sficon sficon-sign"></i>标记工单</a>'></sfTags:pagePermission>
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_GUOMEI_MARKCANCEL_BTN" html='<a href="javascript:cancelMarkOrders();" class="sfbtn sfbtn-opt"><i class="sficon sficon-signCancel"></i>取消标记</a>'></sfTags:pagePermission>
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_GUOMEI_PLPRINT_BTN" html='<a href="javascript:batchPrint();" id="repeatOrder"  class="sfbtn sfbtn-opt"><i class="sficon sficon-print"></i>批量打印</a>'></sfTags:pagePermission>
						</div>
						<div class="f-r">
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_GUOMEI_EXPORT_BTN" html='<a onclick="return exports()"class="sfbtn sfbtn-opt2" target="_blank"><i class="sficon sficon-export"></i>导出</a>'></sfTags:pagePermission>
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_GUOMEI_HEADERSET_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt2" id="setHeadersBtn"><i class="sficon sficon-setting"></i>表头设置</a>'></sfTags:pagePermission>
						</div>
...
<script type="text/javascript">
    ...
    function couts(){
        $.post("${ctx}/order/ChangeSelfOrder/getCounts",{orderType:"i"},function(result){
    ...
    function initSfGrid(){
        $("#table-waitdispatch").sfGrid({
            url : '${ctx}/order/ChangeSelfOrder/FourGuomeiOrder',
            sfHeader: defaultHeader,
            sfSortColumns: sortHeader,
    		rownumbers:true,
    ...
    function exports(){
        ...
        if(idArr>10000){
            $('body').popup({
                ...
                content:content,
                fnConfirm :function(){
                    location.href="${ctx}/order/ChangeSelfOrder/exportGuomei?formPath=/a/order/ChangeSelfOrder/guomeiHeaderList&&maps="+$("#searchForm").serialize();
                }
            });
        }else{
            location.href="${ctx}/order/ChangeSelfOrder/exportGuomei?formPath=/a/order/ChangeSelfOrder/guomeiHeaderList&&maps="+$("#searchForm").serialize();
