...
<script type="text/javascript">
    ...
    function showBuy(id,name){
        ...
        if (!bStop) {
            creatIframe('${ctx}/goods/sitePlatformGoods/showBuyFaucetDetail?id='+id, name);
        } else {
            show_navLi.eq(bStopIndex).addClass('active');
            $('#iframe_box .show_iframe',window.top.document).hide().eq(bStopIndex).show().find('iframe').attr({'src': '${ctx}/goods/sitePlatformGoods/showBuyFaucetDetail?id='+id});
