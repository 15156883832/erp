		...
		<div class="popupMain pos-r" >
				...
				<div class="tabCon pt-10">
					<div class="cl mb-10 mt-10">
						<input type="hidden" name="id" value="${order.id}">
							...
							<div class="f-l pos-r pl-80">
								...
								<select id="origin" name="origin" style="display:none" disabled="disabled" class="select-box w-120 readonly hide">
								<c:choose>
					...
					<div class="cl mt-10">
						...
						<div class="f-l pos-r pl-90">
								<label class="w-90 pos">其他联系方式：</label>
					...
					<div class="cl mt-10 mb-10">
						<div class="f-l" id="pcd">
							...
							<input type="text" class="input-text readonly w-280 f-l" readonly="readonly" id="customerAddress1" name="customerAddress1" autocomplete="off" value="<c:out value='${order.customerAddress}'/>"/>
						...
						<div id="Imgprocess1">
								...
								<c:forEach items="${repairImgs }" var="str" varStatus="da">
									<div class="f-l imgWrap1" id="img${da.index}">
										...
										<a class="sficon btn-delimg" onclick="deleteImg('img${da.index}')" style="display: none;"></a>
				...
				<div class="tabCon pt-10">
					...
					<div class="tabCon pt-10">
						...
						<c:forEach var="pros" items="${fns:getOrderDistribution(order.id)}">
								...
								<p class="processItem orderDistri">
			...
			<div class="btnMenubox pb-80">
				<c:if test="${'0' ne order.status}">
					...
					<c:if test="${'7' ne order.orderType}">
					<sfTags:pagePermission authFlag="ORDER_WXORDER_BTN_OTHERS" html='<input class="sfbtn sfbtn-opt sbtn" onclick="showwxgd(\'${order.orderType}\')" value="无效工单" type="button"/>'></sfTags:pagePermission>
					</c:if>
			...
			<div class="f-r w-460">
				<div class="cl mb-10">
					...
					<div class="f-l w-230 ml-10 f-13 pt-10" style="height: 120px;">
						...
						<p class="custAddr">${order.province}${order.city}${order.area}${order.customerAddress}</p>
					</div>
				</div>
				<div class="cl mb-10">
					<input type="text"  class="input-text w-330 f-l" id="arroundCustomerAddress" value="${order.customerAddress}"/>
...
function updeDistribution(orderId){
	  $.ajax({
          type: "POST",
          traditional: true,
          url: "${ctx }/order/distribution/getdistribution",
          data: {
        	  orderId: orderId
          },
          success: function (result) {
...
$(function(){
	createUploader("#filePicker-add","#Imgprocess1","file_fake_addimg","file_fake_add","delpickerImg");
	$('.imgWrap').imgShow();
}); 
...
//			$("#origin1").hide();
			...
			if($(this).val()!="保存"){
                ...
                var str='origin,promiseTime,promiseLimit,customerFeedback,remark,applianceModel,applianceNum,applianceBarcode,applianceMachineCode,applianceBuyTime,pleaseReferMall,warrantyType,level,customerType';
...
function employe() {
	...
	var address = $("#province").val()+$("#city").val()+$("#area").val()+$("#customerAddress").val();
