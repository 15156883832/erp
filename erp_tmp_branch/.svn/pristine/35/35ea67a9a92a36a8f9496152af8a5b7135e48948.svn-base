			...
			<div class="page-orderWait">
				<div id="tab-system" class="HuiTab">
					<div class="tabBar cl mb-10">
						<a class="btn-tabBar current" href="${ctx }/goods/platform/getAllPlatformOrdersHead">平台产品订单</a>
						<p class="f-r btnWrap">
...
<div class="popupBox promptBox1" style="width:400px;">
		...
		//初始化jqGrid表格，传递的参数按照说明
		   function initSfGrid() {
				sfGrid = $("#table-waitdispatch").sfGrid({
					url : '${ctx}/goods/platform/getAllPlatformOrders',
    ...
    function noPassSubmit(){
		...
		function outStock(id){
			...
			$.ajax({
				type:"post",
				url:"${ctx}/goods/nanDao/platformDetail",
				data:{id:id},
		...
		function pass(){
			...
			$('body').popup({
				...
				fnConfirm:function(){
					$.ajax({
						type:"post",
						url:"${ctx}/goods/nanDao/passForPlatform",
		...
		function fmtOper(rowData){//操作
            ...
            }else if(rowData.status=='4' || rowData.status=='3'){
		...
		function logsitDetail(id){
			pardetaile=layer.open({
				type : 2,
				content:'${ctx}/goods/mjl/logsitDetailforPlat?id='+id,
				title:false,
				area: ['100%','100%'],
				closeBtn:0,
				shade:0,
		...
		function expotrs(){
			...
			if(idArr>10000){
				$('body').popup({
					...
					content:content,
					 fnConfirm :function(){
						 location.href="${ctx}/goods/nanDao/exportForPlat?formPath=/a/goods/platform/getAllPlatformOrdersHead&&maps="+$("#searchForm").serialize();
					 }
				});
			}else{
				 location.href="${ctx}/goods/nanDao/exportForPlat?formPath=/a/goods/platform/getAllPlatformOrdersHead&&maps="+$("#searchForm").serialize();
