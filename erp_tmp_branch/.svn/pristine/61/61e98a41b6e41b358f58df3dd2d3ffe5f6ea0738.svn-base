			...
			<div class="pcontent" style="padding-right: 20px;">
				<div class="tabBarP" style="overflow: visible;">
					...
					<div class="f-r pos-r" style="bottom:8px;">
						<c:if test="${order400.columns.is_convert eq 0 }">
							<a href="javascript:;" onclick="zzj()" class="sfbtn sfbtn-opt w-80">转自接</a>
						</c:if>
						<a href="javascript:;" onclick="senMagPopup('${order400.columns.id }')" class="sfbtn sfbtn-opt w-80 mr-10">短信通知</a>
						<a href="javascript:;" onclick="dygd('${order400.columns.id }')" id="repeteWrite"  class="sfbtn sfbtn-opt w-80 mr-10">打印工单</a>
				...
				<div class="cl mt-10">
					<div class="f-l pos-r txtwrap1">
						...
						<input type="text" class="input-text w-140 readonly" disabled="disabled" value="<fmt:formatDate value='${order400.columns.appliance_buy_time }' pattern='yyyy-MM-dd'/>">
					</div>
					<div class="f-l pos-r txtwrap2">
...
function zzj(){
 ...
 function inputWidth(obj){
	...
	function dygd(id){
		//使用默认
		window.open("${ctx}/print/order400?orderId="+id);
	}

	function dygdcustom(id){
		$.ajax({
			type : 'POST',
			url : "${ctx}/order/printdesign/getOrderKeyName400",
			data : {orderId:id},
			datatype:"JSON",
			success : function(data) {
				if(data == null || data.content == null || data.content.length<=0){
					var newTab=window.open('about:blank');
					newTab.location.href="${ctx}/print/order400?orderId="+id;
					return
				}
				prn_Preview(data);
			}
...
function showMarkOrder400(id) {
    layer.open({
        type: 2,
        content: '${ctx}/order/showMarkOrders?ids=' + id + "&type=400",
        title: false,
        area: ['100%', '100%'],
        closeBtn: 0,
        shade: 0,
