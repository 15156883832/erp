  ...
  <div class="sfpagebg bk-gray">
	...
	<div id="tab-system" class="HuiTab">
		<div class="tabBar cl mb-10">
			<sfTags:pagePermission authFlag="FITTINGMGM_OUTINFITDETAIL_XINJIANFANC_TAB" html='<a class="btn-tabBar current" href="${ctx}/fitting/siteFittingKeep/newFittlist">新件返厂</a>'></sfTags:pagePermission>
			<sfTags:pagePermission authFlag="FITTINGMGM_OUTINFITDETAIL_JIUJIANFANC_TAB" html='<a class="btn-tabBar"  href="${ctx}/fitting/returnFactoryStockFitting">旧件返厂</a>'></sfTags:pagePermission>
			<p class="f-r btnWrap">
						...
						<tr>
							...
							<td>
								<select class="select" name="mxtype" style="width: 140px;"><!--   类型：0领取，1工单使用  2零售，3返还  4调拨 -->
										...
										<option value="6">返厂在途</option>
										<option value="a">已到厂</option>
									</select>
							</td>
...
<div class="popupBox notDispatch showwxgddiv">
	...
	<div class="popupContainer">
		<div class="popupMain " >
				...
				<p class="lb lb1" style='width:300px;padding-left: 18px;margin-top: 7px;' id="xinjiandsj"></p>
...
function initSfGrid(){
	sfGrid = $("#table-waitdispatch").sfGrid({
		...
		url : '${ctx}/fitting/siteFittingKeep/getnewFittinglistList',
...
function initApplyerSelect(){
 ...
 function fmtOperation(rowData){
	 if(rowData.ktype==6){
	 var fitting_name = rowData.fitting_name.replace(/\s/g, "");
			return "<span><a class='c-0383dc' href=javascript:showDetail('"+rowData.id+"','"+fitting_name+"');>确认到厂</a></span>";
		}else{
	...
	function fittingqrrks(){
		...
		if (idArr.length < 1) {
			...
			$('body').popup({
	              ...
	              fnConfirm: function () {
					  $.ajax({
		                    url: '${ctx}/fitting/siteFittingKeep/updateFittingKeepIds?ids=' + idArr.join(","),
		                    type: 'post',
...
function saveqrdc(){
	...
	$.ajax({
		type:"POST",
		url:"${ctx}/fitting/siteFittingKeep/updateFittingKeepById",
				data:{
					id:id,
					remarks:remarks
				},
...
function reset(){
	 location.href="${ctx}/fitting/siteFittingKeep/newFittlist";
}
...
function exports(){
	...
	if(idArr>10000){
		$('body').popup({
			...
			content:content,
			 fnConfirm :function(){
				 location.href="${ctx}/fitting/siteFittingKeep/export?formPath=/a/fitting/siteFittingKeep/newFittlist&&maps="+$("#searchForm").serialize();
			 }
