...
public class SiteselfOrderDao extends BaseDao<GoodsSiteselfOrder> {
	...
	public Long getSiteSelfGoodsOrderCount(String siteId, Map<String, Object> map, String orderType) {
		...
		stringBuilder.append("SELECT distinct(a.id) from crm_goods_siteself_order a ");
		stringBuilder.append(" left join crm_goods_siteself_order_goods_detail b on a.id=b.site_order_id and b.status='0' ");
		stringBuilder.append(" LEFT JOIN crm_order_collections oc ON oc.order_id=a.id AND oc.status='0' AND oc.source='1' ");
		stringBuilder.append(" where " + sqlChange + " and a.site_id=? ");
		stringBuilder.append(getSiteSelfOrderConditions(map));
	...
	public String getSiteSelfOrderConditions(Map<String, Object> map) {
		...
		if (map != null) {
			...
			if (StringUtils.isNotEmpty((CharSequence) map.get("goodCategory"))) {
				String goodCategory = map.get("goodCategory").toString().trim();
				sb.append(" and b.good_category like '%" + goodCategory + "%' ");
			}
		}
		return sb.toString();
	}
