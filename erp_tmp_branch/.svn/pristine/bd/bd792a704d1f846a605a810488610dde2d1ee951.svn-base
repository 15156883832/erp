    ...
    $(function () {
        ...
        $("#btnSubmit").click(function () {
            ...
            names.each(function (indx, el) {
                ...
                if ((sortsArr[indx].length != 0) && (sortsArr[indx].match(/\D/))) {
                    numberflag = true;
                    layer.msg("排序请输入数字");
                }
            });
            ...
            for (var i = 0; i < nameArr.length; i++) {
                if (s.replace(nameArr[i] + ",", "").indexOf(nameArr[i] + ",") > -1) {
                    repeatname = true;
                    layer.msg(nameArr[i]+"有重复");
                    break;
                }

                var namelist=new Array();
                <c:forEach items="${malllist }" var="item">
                namelist.push("${item}");
                </c:forEach>
                if(namelist!=null){
                   if(namelist.indexOf(nameArr[i])>-1){
                       repeatname = true;
                       layer.msg(nameArr[i]+"已存在");
                       break;
                   }
                }
            }
