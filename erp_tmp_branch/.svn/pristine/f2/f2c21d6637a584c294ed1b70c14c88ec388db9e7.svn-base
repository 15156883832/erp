...
<div class="page-orderWait">
	<div id="tab-system" class="HuiTab">
		<div class="tabBar cl mb-10">
			<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_ALLORDER_TAB" html='<a class="btn-tabBar current"  href="${ctx }/order/getWwgList">全部工单<sup id="tab_c1">0</sup></a>'></sfTags:pagePermission>
			...
			<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_JRYYORDER_TAB" html='<a class="btn-tabBar" href="${ctx }/order/getWwgList/jryy">今日预约<sup id="tab_c4">0</sup></a>'></sfTags:pagePermission>
			<p class="f-r btnWrap">
		...
		<div class="tabCon">
			...
			<div class="pt-10 pb-5 cl">
				<div class="f-l">
					<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_ALLORDER_NEWORDER_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt" onclick="addNew(); "  id="btnAddnew"><i class="sficon sficon-add"></i>新建工单</a>'></sfTags:pagePermission>
					...
					<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_ALLORDER_DISPATCH_BTN" html='<a href="javascript:directDis();" class="sfbtn sfbtn-opt" id="directpg"><i class="sficon sficon-dispatch"></i>派工</a>'></sfTags:pagePermission>
					<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_ALLORDER_ZBDISPATCH_BTN" html='<a href="javascript:emporarily();" class="sfbtn sfbtn-opt"><i class="sficon sficon-notdispatch"></i>暂不派工</a>'></sfTags:pagePermission>
					...
					<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_ALLORDER_SENDMSGCOUNTS_BTN" html='<a href="javascript:;" onclick="senMagPopup()" class="sfbtn sfbtn-opt"><i class="sficon sficon-massText"></i>群发短信</a>'></sfTags:pagePermission>
					<%-- <sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_ALLORDER_DISPATCHSECONDSITE_BTN" html='<a href="javascript:erjidirectDis();" class="sfbtn sfbtn-opt"  id="erjidirectpg"><i class="sficon sficon-dispatch"></i>指派给二级网点</a>'></sfTags:pagePermission> --%>
				</div>
				<div class="f-r">
					<a href="javascript:;" class="sfbtn sfbtn-opt2 reloadPageBtn"><i class="sficon sficon-reload"></i>刷新</a>
					<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_ALLORDER_IMPORT_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt2" id="btnImport"><i class="sficon sficon-import"></i>导入</a>'></sfTags:pagePermission>
					<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_ALLORDER_EXPORT_BTN" html='<a  onclick="return exports()"class="sfbtn sfbtn-opt2" target="_blank"><i class="sficon sficon-export"></i>导出</a>'></sfTags:pagePermission>
					<sfTags:pagePermission authFlag="ORDERMGM_WAITDISPATCH_ALLORDER_TBSETTLE_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt2" id="setHeadersBtn"><i class="sficon sficon-setting"></i>表头设置</a>'></sfTags:pagePermission>
				</div>
	...
	<div class="popupContainer">
		<div class="popupMain " >
			...
			<div class="text-c pl-30">
				...
				<input type="button" class="mr-10 w-70 sfbtn sfbtn-opt" value="取消" onclick="$.closeDiv($('.showwxgddiv'));"/>
			</div>
		</div>
	</div>
</div>
...
<!-- 导入结果框 -->
<div class="popupBox massTextNote importEndDiv" style="width: 720px;">
	<h2 class="popupHead">
		导入提示
		...
		<div class="popupMain pd-10">
			<div class="cl mb-10">
				...
				<select class="select w-140 f-l" id="excelTemplateSel" onchange="changeExcelTemplate();">
					...
					<option value="1">苏宁模板</option>
					<option value="5">国美模板</option>
				</select>
			</div>
			<div class="cl">
			...
			<div class="cl mt-20 mb-15">
				<div class="f-l ml-20" id="excel_download_btn">
					<a href="${commonFileHead}/fileDownload/order_excel_template.xlsx" target="_blank" class="sfbtn sfbtn-opt3 w-100">下载模板</a>
				</div>
				<div class="f-r pr-10">
...
//        }
	...
	uploader = WebUploader.create({
	    ...
	    server: '${ctx}/order/checkUnfinishedOrderExcel?templateId=' + curExcelTemplate,
	    pick: '#btn-import',  
	    accept: {
	        title: 'XLSX',  
	        extensions: 'xlsx,xls',  
	...
	uploader.on( 'uploadSuccess', function( file, response ) {
		if(response.operType == "check"){
			...
			if(response.templateError == "y"){
				...
				uploader.stop();
				file.setStatus('queued');
				uploader.option( 'server', '${ctx}/order/importUnfinishedOrder?templateId=' + curExcelTemplate);
				layer.msg("校验成功，可以导入!");
			}else{
				$("#filename").val('');
				layer.msg("解析Excel错误!");
...
function emporarily() {
    ...
    if (idArr.length < 1) {
        ...
        for (var i = 0; i < idArr.length; i++) {
            ...
            disIds.push(rowData.disorderId);
...
function initSfGrid(){
	$("#table-waitdispatch").sfGrid({
		url : '${ctx}/order/getDpgList?ptype='+${ptype},
		sfHeader: defaultHeader,
		sfSortColumns: sortHeader,
		rownumbers:true,
   ...
   /*  if (nowM >= start && nowM <= end) {
    ...
    if (idArr > 10000) {
        $('body').popup({
            ...
            content: content,
            fnConfirm: function () {
                location.href = "${ctx}/order/export?formPath=/a/order/getWwgList&&maps=" + $("#searchForm").serialize();
            },
            fnCancel: function () {
            }
        });
...
function senMagPopup(){
	...
	if(ids.length > 0){
		layer.open({
	        type : 2,
	        content:'${ctx}/order/sendMsgAccounts?ids=' + ids.join(",")+"&marks=2",
	        title:false,
	        area: ['100%','100%'],
	        closeBtn:0,
	        shade:0,
