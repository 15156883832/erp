			...
			<div id="tab-system" class="HuiTab">
				<div class="tabBar cl mb-10">
					...
					<c:if test="${map.jingdong eq 'has'}">
						<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_JINGDONG_TAB" html='<a id="jingdong" class="btn-tabBar current "  href="${ctx }/order/ChangeSelfOrder/jingdongHeaderList">京东工单</a>'></sfTags:pagePermission>
					</c:if>
					<p class="f-r btnWrap">
				...
				<div class="tabCon">
									...
									<td>
										<span class="w-140">
											<select class="select easyui-combobox"  id="statusFlag" multiple="true" multiline="false" name="status" style="width:100%;height:25px" panelMaxHeight="300px">
												...
												<option value="申请改派">申请改派</option>
												<option value="已接收">已接收</option>
												<option value="已预约">已预约</option>
												<option value="已派工">已派工</option>
												<option value="退换机待审核">退换机待审核</option>
												<option value="退换机处理中">退换机处理中</option>
												<option value="人工取消确认中">人工取消确认中</option>
												<option value="待仓库处理">待仓库处理</option>
												<option value="暂停">暂停</option>
												<option value="异常">异常</option>
												<!-- 完工状态 -->
												<option value="上门完成">上门完成</option>
					...
					<div class="pt-10 pb-5 cl">
						<div class="f-l">
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_JINGDONG_ZZJ_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt" onclick="zzj();"><i class="sficon sficon-zzj"></i>转自接</a>'></sfTags:pagePermission>
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_JINGDONG_PLDELETE_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt" onclick="delMore();"><i class="sficon sficon-rubbish"></i>批量删除</a>'></sfTags:pagePermission>
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_JINGDONG_MARKORDER_BTN" html='<a href="javascript:showMarkOrders();" class="sfbtn sfbtn-opt"><i class="sficon sficon-sign"></i>标记工单</a>'></sfTags:pagePermission>
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_JINGDONG_MARKCANCEL_BTN" html='<a href="javascript:cancelMarkOrders();" class="sfbtn sfbtn-opt"><i class="sficon sficon-signCancel"></i>取消标记</a>'></sfTags:pagePermission>
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_JINGDONG_PLPRINT_BTN" html='<a href="javascript:batchPrint();" id="repeatOrder"  class="sfbtn sfbtn-opt"><i class="sficon sficon-print"></i>批量打印</a>'></sfTags:pagePermission>
						</div>
						<div class="f-r">
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_JINGDONG_EXPORT_BTN" html='<a onclick="return exports()"class="sfbtn sfbtn-opt2" target="_blank"><i class="sficon sficon-export"></i>导出</a>'></sfTags:pagePermission>
							<sfTags:pagePermission authFlag="ORDERMGM_FOURORDER_JINGDONG_HEADERSET_BTN" html='<a href="javascript:;" class="sfbtn sfbtn-opt2" id="setHeadersBtn"><i class="sficon sficon-setting"></i>表头设置</a>'></sfTags:pagePermission>
						</div>
...
<script type="text/javascript">
    ...
    function couts(){
        $.post("${ctx}/order/ChangeSelfOrder/getCounts",{orderType:"j"},function(result){
    ...
    function initSfGrid(){
        $("#table-waitdispatch").sfGrid({
            url : '${ctx}/order/ChangeSelfOrder/FourJingdongOrder',
            sfHeader: defaultHeader,
            sfSortColumns: sortHeader,
    		rownumbers:true,
    ...
    function exports(){
        ...
        if(idArr>10000){
            $('body').popup({
                ...
                content:content,
                fnConfirm :function(){
                    location.href="${ctx}/order/ChangeSelfOrder/exportJingdong?formPath=/a/order/ChangeSelfOrder/jingdongHeaderList&&maps="+$("#searchForm").serialize();
                }
            });
        }else{
            location.href="${ctx}/order/ChangeSelfOrder/exportJingdong?formPath=/a/order/ChangeSelfOrder/jingdongHeaderList&&maps="+$("#searchForm").serialize();
