		...
		<div class="container-fluid cl ">
			...
			<nav id="Hui-userbar" class="navbar-userbar userbar-sf f-r">
				<ul class="cl">
					...
					<li class="dropDown dropDown_hover f-l right">
						<a href="#" class=" ">
...
////				init_left_menu('cc3930152a2a4c51989adc4b8c160298','运营管理','v7');
...
//			});
			...
			$("#goToGoods").click(function(){
				...
				init_left_menu('cb0c8c9b182843fc8690453e52e67f59','商品管理','v4', function () {
					...
					//aSide.find("a[data-title='平台商品']").click();
		...
		//过滤掉menu1的
		function menu1Filter(){
			if(sysMenus.menuItems == undefined || sysMenus.menuItems.length == 0){
				$("#menu-orders").hide();
				$("#sf_top_menu").hide();
				$("#iframe_box").hide();
			}else{
				...
				/**一级菜单过滤*/
				$("#sf_top_menu li").each(function(){
					var this_ = $(this);
					var menu1Flag = false;
					$.each(sysMenus.menuItems, function(index, data){
						if(data.level == '1' && $.trim(this_.find("a").text()) == data.name){
							menu1Flag = true;
							return false;
						}
					});
				...
				$("#orderMe li").each(function(){
					...
					var menu2Flag = false;
					$.each(sysMenus.menuItems, function(index, data){
						if(data.level == '2' && $.trim(this2_.find("a").text()) == data.name){
							menu2Flag = true;
							hasleft = true;
							return false;
						}
