...
<div class="popupBox  orderSettle">
	...
	<div class="popupContainer pos-r pb-60">
		<div class="popupMain" >
			<div class="pcontent pl-15 pt-10">
				<span class="w-260">创建时间 ~完工时间：${endTimeslot }</span>
			<span class="w-260 ">派工时间~完工时间：${disTimes }</span>
			<div class="w-830">
				...
				<div class="cl mb-10">
					...
					<div class="priceWrap w-100 f-l factoryPriceWrap">
						<input type="text" class="input-text" id="ffee" value="${st.orderSettlement.factoryMoney}" onkeyup="updatePVAndCalcOverall();"/>
				...
				<div class="cl mb-10">
					...
					<div class="priceWrap w-100 f-l readonly hide">
						<input type="text" class="input-text" readonly="readonly" value="${st.orderMoney}" id="orderfee"/>
						<span class="unit">元</span>
					</div>
					<label class="w-110 text-r f-l">APP中的服务费：</label>
					<div class="priceWrap w-100 f-l readonly">
						<input type="text" class="input-text" readonly="readonly" value="${st.order.serveCost}" />
						<span class="unit">元</span>
					</div>
					<label class="w-110 text-r f-l">APP中的材料费：</label>
					<div class="priceWrap w-100 f-l readonly">
						<input type="text" class="input-text" readonly="readonly" value="${st.order.auxiliaryCost}" />
						<span class="unit">元</span>
					</div>
					<label class="w-110 text-r f-l">APP中的延保费：</label>
					<div class="priceWrap w-100 f-l readonly">
						<input type="text" class="input-text" readonly="readonly" value="${st.order.warrantyCost}" />
						<span class="unit">元</span>
					</div>
				</div>
				...
				<div class="f-l lh-26">
					...
					<span class="f-l"><input id="empCost" readonly="readonly" style="border:none" class="input-text w-60" value="${st.orderSettlement.sumMoney}" />元</span>
					<span class="f-l ml-10 c-888">（当日支付：<input id="payed" readonly="readonly" style="border:none" class="input-text w-60 c-888" value="" /> 元）</span>
				</div>
				<div class="cl mb-10">
					...
					<div id="switch2" class="switch f-l mr-10" data-on-label="有" data-off-label="无">
						<input id="accCostCheck" type="checkbox" <c:if test="${st.orderSettlement.fittingCosts > 0}">checked</c:if>/>
					</div>
					<div class="f-l fucaiCostWrap <c:if test='${st.orderSettlement.fittingCosts == 0}'>hide</c:if>">
						<label class="text-r f-l ">公司成本：</label>
						<div class="priceWrap w-80 f-l " >
							<input type="text" class="input-text"  value="${st.orderSettlement.fittingCosts}" id="afee" onkeyup="calcOverall();"/>
							<span class="unit">元</span>
						</div>
				...
				<div class="retractWrap">
					<div class="pos-r pl-160 ">
						...
						<div class="cl" style="min-height: 26px;">
							<c:forEach items="${st.dispEmpSettlementDetail}" var="detail">
								<div class="f-l mb-10">
									<label class="f-l">${detail.employeName}：</label>
									<div class="priceWrap w-70 f-l readonly mr-15">
										<input type="text" class="input-text empi" readonly="readonly" value="${detail.sumMoney}"/>
										<input type="hidden" class="emp-ratio" value="${detail.ratio}">
										<input type="hidden" class="emp-id" value="${detail.employeId}">
										<span class="unit">元</span>
									</div>
								</div>
							</c:forEach>
							<c:if test="${fn:length(st.dispEmpSettlementDetail) gt 1}">
					...
					<div class="bk-blue-dotted pt-10 mb-10 sdefinedbox retractBox" id="sdefinedbox1">
						<div class="cl mb-10">
							...
							<span class=" w-210 f-l">
								<select class="select" name="fwjsfa" id="smethod">
									<option value="99999">请选择</option><!-- 默认显示上一次结算的结果 -->
									<option value="0">默认结算方案</option>
						...
						<!-- 上一次结算的结果 -->
						<div class="sdefined sdefined99999 cl">
							<c:forEach items="${st.jiesuanItems}" var="item">
								<div class="f-l mb-10 ">
									<!-- 自定义的结算方案名称不能超过7个字（不包含冒号） -->
									<label class="w-100 text-r f-l textColon flabel" title="${item.key}">${item.key}</label>
									<div class="priceWrap w-100 f-l">
										<input type="text" class="input-text ppp" value="${item.value}" onkeyup="calcOverall();"/>
										<span class="unit">元</span>
									</div>
								</div>
							</c:forEach>
							<div class="f-l mb-10" style="width: 774px">
								<div class="pos-r pl-90 ml-10">
									...
									<input type="text" class="input-text tmpl-memo" style="width:100%;" value="${st.orderSettlement.remarks}">
								</div>
							</div>
						</div>

						<div class="sdefined sdefined0 hide">
							<div class="cl mt-10">
							...
							<div class="sdefined sdefined${tmpl.index} cl hide">
								<c:forEach items="${tmpl.items}" var="item">
									<div class="f-l mb-10 ">
										...
										<div class="priceWrap w-100 f-l">
											<input type="text" class="input-text ppp ${item.factoryFeeProportionBasedItem ? 'factoryFeeProportionBasedItem' : ''}" value="<fmt:formatNumber type="number" value="${item.cost}" maxFractionDigits="2" groupingUsed="false"/>" onkeyup="calcOverall();"/>
				...
				<div class="retractWrap">
					<div class="pos-r pl-160">
						...
						<div class="cl" style="min-height: 26px;" id="addedEmps">
							...
							<c:forEach var="item" items="${st.addedJisuan}">
								<div class="f-l mb-10 addedEmpItem" id="${item.empId}">
									<label class="f-l">${item.empName}：</label>
									<div class="priceWrap w-70 f-l readonly mr-15">
									<input type="text" class="input-text addedEmpCosti" readonly="readonly" value="${item.cost}">
									<span class="unit">元</span>
									</div>
								</div>
							</c:forEach>
					...
					<div class="bk-blue-dotted pt-10 mb-10 retractBox" id="addJsBox">
						<!--
						<div class="mb-5">
						...
						<%--
						<c:forEach var="item" items="${st.addedEmpSettlementDetail}">
							<div class="mb-5">
								<span class="w-80 text-r">${item.employeName}：</span>
								<span class="w-150">${item.serviceMeasures}</span>
								<span class="w-150">${item.sumMoney}元</span>
								<span class="w-150">${item.remarks}</span>
								<span class="w-150"><a class="c-0383dc" onclick="deleteTR('${item.employeId}');"><i class="sficon sficon-rubbish"></i>删除</a></span>
							</div>
						</c:forEach>
						--%>
				...
				<div class="cl mb-10 mt-10">
					...
					<input type="text" onfocus="WdatePicker({})" id="bxdatemax" name="bxdatemax" value="<fmt:formatDate value="${st.orderSettlement.settlementTime}" pattern="yyyy-MM-dd"/>" class="input-text Wdate w-140 f-l" >
				</div>
				<!-- <div class="pos-r pl-90 ml-10">
					<label class="w-90 text-r lb">备注：</label>
				...
				<div class="cl mb-10">
					...
					<div class="priceWrap w-100 f-l">
						<input type="text" class="input-text" value="${st.orderSettlement.profits}" id="profit"/>
						<span class="unit">元</span>
					</div>
					<label class="w-100 text-r f-l lh-26" style="margin-left: 320px"> <span class="c-005aab va-t">当日支付</span>：</label>
					<div class="priceWrap w-100 f-l readonly">
						<input type="text" class="input-text readonly" value="" id="payed-mirror" readonly="readonly"/>
						<span class="unit">元</span>
					</div>
	...
	<form id="modDispEmpJsForm">
		<div class="popupContainer pos-r">
			<div class="popupMain pt-30 pb-15">
				<c:forEach items="${st.dispEmpSettlementDetail}" var="emp">
					<div class="cl mb-10">
						<label class="f-l w-140 text-r"><span class="emp-name-sp">${emp.employeName}</span>：</label>
						<div class="priceWrap w-160 f-l">
...
<script type="text/javascript">
    ...
    function isAccCostEnabled() {
        return $('#accCostCheck').is(':checked');
	}
...
//            accCostEnabled = data.value;
        ...
        var idx = 0;
        <c:forEach var="item" items="${st.addedEmpSettlementDetail}">
			// todo fix fixed reason and reasonTxt
			idx++;
        	createAddedSettlementTR('${item.employeId}', '${item.employeName}', mapping['${item.serviceMeasures}'], '${item.serviceMeasures}', '${item.sumMoney}', '<c:out value="${item.remarks}"/>', false, new Date().getTime() + idx);
        </c:forEach>
    });
...
//		parent.closeJiesuanForm();
    ...
    function saveSettlement() {
        ...
        var accCost = isAccCostEnabled() ? $("#afee").val() : 0;
        ...
        if ("99999" === smethod) { // settlement measure not changed.
            sMeth = "KEEPOLD";
        }
        var data =  {
        ...
        formPosted = true;
        $.ajax({
            url: "${ctx}/order/settlement/saveEdit",
            type: 'POST',
