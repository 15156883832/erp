...
!function($) {
	$.shiftHeader = function(id) {
		var $this = $('#'+id);
        $this.children('li').off('click');
		$this.children('li').on('click', function(event){
			
			if(event.target.nodeName=='P'){
				$(this).addClass('optselected').siblings().removeClass('optselected');
			}else if(event.target.getAttribute('title')=='上移'){
				$(this).addClass('optselected').siblings().removeClass('optselected');
				$(this).insertBefore($(this).prev()); 
			}else if(event.target.getAttribute('title')=='下移'){
				$(this).addClass('optselected').siblings().removeClass('optselected');
				$(this).insertAfter($(this).next()); 
			}else if(event.target.getAttribute('title')=='删除'){
				//console.log($(event.target).attr('id'))
				var id = "sfTbhUns_" + $(event.target).attr('id');
				$(this).remove()
				$(".optionalHead").find("ul").
				append('<li><label class="label-cbox2" for="'+id+'"><input type="checkbox" id="'+id+'" name="optionchk" tag="'+$(event.target).attr('tag')+'">'+$(event.target).attr('tag')+'</label></li>');
			}
		})	
		$('.optionalHead ul').click(function(event){

		if(event.target.nodeName=='LABEL'){
			if($(event.target).children().is(":checked")){
				$(event.target).addClass('label-cbox2-selected');
			}else{
				$(event.target).removeClass('label-cbox2-selected');
			}
		}else if(event.target.nodeName=='INPUT'){
			if($(event.target).is(":checked")){
				$(event.target).parent().addClass('label-cbox2-selected');
			}else{
				$(event.target).parent().removeClass('label-cbox2-selected');
			}
		}else if(event.target.nodeName=='LI'){
			if($(event.target).children().children().is(":checked")){
				$(event.target).children().addClass('label-cbox2-selected');
			}else{
				$(event.target).children().removeClass('label-cbox2-selected');
			}
		}
})
	}
...
function refreshPushMsgBox(obj){
    ...
    if(msgType == '10' || msgType == '16'){//ERP拒单提醒
    	...
    	if(msgType == '10'){
            ...
            itemHtml += '<div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-13b6a1" style="color:#ff3400;">ERP工单拒接：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNoF+'\');">'+orderNoF +'</span>' + serviceType + ' / 工程师:' + empName + '<p>拒单原因:'+ rejectDetail +'</p>'+jtHtml+'</div></div>';
            html += '<div class="orderWrapS pos-r msg-item-ejd"><div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-13b6a1" style="color:#ff3400;">ERP工单拒接：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNoF+'\');">'+orderNoF +'</span>' + serviceType + '/工程师:' + empName + '<p>拒单原因:' + rejectDetail + '</p>'+jtHtml+'</div></div></div>';
        }else if(msgType == '16'){
            itemFlag = "msg-item-fk";
            itemHtml += '<div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-e2722f" style="color:#DC55FF;">ERP工单反馈：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNoF+'\');">'+orderNoF +'</span>' + serviceType + ' / 工程师:' + empName +  '<p>反馈内容:'+rejectDetail+'</p>'+jtHtml+'</div></div>';
            html += '<div class="orderWrapS pos-r msg-item-fk"><div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-e2722f" style="color:#DC55FF;">ERP工单反馈：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNoF+'\');">'+orderNoF +'</span>' + serviceType + ' / 工程师:' + empName + '<p>反馈内容:' + rejectDetail +'</p>'+jtHtml+'</div></div></div>';
        }
    }else if(msgType == '11'){//ERP完工提醒
    	itemFlag = "msg-item-ewg";
    	itemHtml += '<div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-13b6a1" style="color:#009966;">ERP工单完工：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNo+'\');">'+orderNo+'</span>' + serviceType + ' / 工程师:' + empName +  '<p>完工时间:'+endTime+'</p>'+jtHtml+'</div></div>';
    	html += '<div class="orderWrapS pos-r msg-item-ewg"><div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-13b6a1" style="color:#009966;">ERP工单完工：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNo+'\');">'+orderNo +'</span>' + serviceType + ' / 工程师:' + empName + '<p>完工时间:'+endTime+'</p>'+jtHtml+'</div></div></div>';
    }else if(msgType == '12'){//微信报单提醒
    	itemFlag = "msg-item-wbd";
    	itemHtml += '<div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-0e8ee7" style="color:#009966;">微信报单：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNo+'\');">'+orderNo +'</span>' + serviceType + ' / 报单员:' + empName +  '<p>报单时间:'+endTime+'</p>'+jtHtml+'</div></div>';
    	html += '<div class="orderWrapS pos-r msg-item-wbd"><div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-0e8ee7" style="color:#009966;">微信报单：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNo+'\');">'+orderNo +'</span>' + serviceType + ' / 报单员:' + empName +  '<p>报单时间:'+endTime+'</p>'+jtHtml+'</div></div></div>';
    }else if(msgType == '13'){//400改约
    	itemFlag = "msg-item-4gy";
    	itemHtml += '<div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-e2722f" style="color:#ffb500;">400工单改约：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNo+'\');">'+orderNo +'</span>' + serviceType + ' / 工程师:' + empName +  '<p>最新改约时间:'+endTime+'</p>'+jtHtml+'</div></div>';
    	html += '<div class="orderWrapS pos-r msg-item-4gy"><div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-e2722f" style="color:#ffb500;">400工单改约：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNo+'\');">'+orderNo +'</span>' + serviceType + ' / 工程师:' + empName +  '<p>最新改约时间:'+endTime+'</p>'+jtHtml+'</div></div></div>';
    }else if(msgType == '14'){//400完工
    	itemFlag = "msg-item-4wg";
    	itemHtml += '<div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-e2722f" style="color:#009966;">400工单完工：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNo+'\');">'+orderNo +'</span>' + serviceType + ' / 工程师:' + empName +  '<p>完工时间:'+endTime+'</p>'+jtHtml+'</div></div>';
    	html += '<div class="orderWrapS pos-r msg-item-4wg"><div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-e2722f" style="color:#009966;">400工单完工：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNo+'\')">'+orderNo +'</span>' + serviceType + ' / 工程师:' + empName +  '<p>完工时间:'+endTime+'</p>'+jtHtml+'</div></div></div>';
    }else if(msgType == '15'){//400接入
        itemFlag = "msg-item-4jr";
        itemHtml += '<div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-13b6a1" style="color:#ff3400;">400工单接入：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNo+'\');">'+orderNo +'</span>' + serviceType + ' / 工程师:' + empName +  '<p>报修时间:'+endTime+'</p>'+jtHtml+'</div></div>';
        html += '<div class="orderWrapS pos-r msg-item-4jr"><div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-13b6a1" style="color:#ff3400;">400工单接入：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="clickPushJT(\''+orderNo+'\');">'+orderNo +'</span>' + serviceType + ' / 工程师:' + empName +  '<p>报修时间:'+endTime+'</p>'+jtHtml+'</div></div></div>';
    }else if(msgType == '17'){//配件申请
        jtHtml = '<span class="jianTou" onclick="goToPeijianPage(\''+orderNo+'\');"></span>';
        itemFlag = "msg-item-bjsq";
        itemHtml += '<div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-13b6a1" style="color:#ff3400;">备件申请：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="goToPeijianPage(\''+orderNo+'\');">'+orderNo +'</span><p>工程师：'+empName+'</p><p>申请时间：'+endTime+'</p>'+jtHtml+'</div></div>';
        html += '<div class="orderWrapS pos-r msg-item-bjsq"><div class="orderWrap"><div class="bb  pb-5 pt-5"><span class="c-13b6a1" style="color:#ff3400;">备件申请：</span><span class="c-13b6a1 cPointer" style="color:#0099ff;" onclick="goToPeijianPage(\''+orderNo+'\');">'+orderNo+'</span><p>工程师：'+empName+'</p><p>申请时间：'+endTime+'</p>'+jtHtml+'</div></div></div>';
	}
    //var $newDom = $(html);
...
function checkMustFill(jurisdiction,jurisdictionvalues){
	...
	if($.trim(jurisdictions[13])=='true'){
		if(isBlank($.trim(jurisdictionvalues[13]))){
            layer.msg("请选择用户类型");
            return false;
		}
	}
