			...
			<div class="loginMain resetMain" id="resetMain">
				<h3 class="lh-26 resetTitle pb-5"> 
					...
					<div class="loginlink" style="right: 90px;">
						...
						<a  class="btn-forgetpwd" onclick="retn()" href="${ctx}/main/redirect/mainIndex">马上登录</a>
					</div>
				</h3>
		...
		<div class="loginWrapBg2 registerpage hide" id="rgzc">
			...
			<div class="loginMain resetMain2">
				<h3 class="lh-26 pb-5 resetTitle"> 
					...
					<div class="loginlink" >
						<span class=" ">已有账号</span>
						<a onclick="retn()" href="${ctx}/main/redirect/mainIndex" class="btn-forgetpwd">马上登录</a>
					</div>
				</h3>
				...
				<div class="tabCon tabCon3">
					...
					<div class=" text-c">
						<a class="btn-login w-120 " onclick="retn()" href="${ctx}/main/redirect/mainIndex">立即登录系统</a>
					</div>
				</div>
			</div>
		</div>
...
$("#city")
	...
	$('#btnNext').on('click', function(){
		 ...
		 if(isBlank(name)){
			 $("#name").focus();
			 $("#errorTip").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>输入企业名称！</p>');
			 $("#errorTip").removeClass("hide");
			 return ;
		 } 
		 if(name.length < 4  ){
			 $("#name").focus();
			 $("#errorTip").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>您输入的企业名称错误，请重新输入！</p>');
			 $("#errorTip").removeClass("hide");
			 return ;
		 }
		 if(name.length > 20  ){
		 ...
		 $.ajax({
			 ...
			 success:function(result){
				 if(result=="no"){
					 $("#name").focus();
					 $("#errorTip").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>您输入的企业名称已存在，请重新输入！</p>');
					 $("#errorTip").removeClass("hide");
					 return ;
				 }else{
					 if(isBlank(address)){
						 $("#address").focus();
						 $("#errorTip").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>请输入详细地址！</p>');
						 $("#errorTip").removeClass("hide");
						 return ;
					 } 
	...
	$('#btnNext2').on('click', function(){
		...
		if(isBlank(password)){
			 $("#password").focus();
			 $("#errorTip1").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>请输入6-16位字符的密码</p>');
		...
		if(isBlank(shareCode)){
			 $("#shareCode").focus();
			 $("#errorTip1").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>请填写激活码，输入激活码即可获取免费使用权限</p>');
			 $("#errorTip1").removeClass("hide");
			 return ;
		} 
		...
		$.ajax({
			...
			success:function(result){
				if(result=="existName"){
					$("#mobile").focus();
					$("#errorTip1").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>该号码已注册，请勿重复注册</p>');
					$("#errorTip1").removeClass("hide");
				}else if(result=="wrongMsg"){
					$("#confirmCode").focus();
					$("#errorTip1").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>您的短信验证码输入错误，请重新输入</p>');
					$("#errorTip1").removeClass("hide");
				}else if(result=="wrongCode"){
					$("#shareCode").focus();
					$("#errorTip1").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>您输入的激活码不存在，请联系思方客服0551-66556995</p>');
					$("#errorTip1").removeClass("hide");
					...
					$("#shareCode").focus();
					$("#errorTip1").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>您输入的激活码分享次数已超上限，请更换其他激活码或联系思方客服：0551-66686515.</p>');
					$("#errorTip1").removeClass("hide");
				}else if(result=="nothing"){
					$("#confirmCode").focus();
					$("#errorTip1").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>您的短信验证码已失效，请重新获取</p>');
					$("#errorTip1").removeClass("hide");
				}else{
					$("#errorTip1").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>注册失败，请检查！</p>');
					$("#errorTip1").removeClass("hide");
				} 
			},
			complete: function() {
	...
	document.getElementById("btn").onclick=function(){
    	...
    	if(mobile.length==11 && (/^1[3|4|5|7|8|9][0-9]\d{4,8}$/.test(mobile))){//点击发送短信验证码
	    	...
	    	$.ajax({
    		 ...
    		 $("#mobile").focus();
			 $("#errorTip1").append('<p class="c-fe0101 f-13 lh-20 hide1"><i class="sficon sficon-errorwran2"></i>请输入正确的手机号</p>');
			 $("#errorTip1").removeClass("hide");
    	}
	}
