...
<div id="printContainer">
	<c:forEach items="${orders}" var="order" varStatus="stat">
		<div class="printpage wrap" style="position:relative;">
			<h2 class="title" style="text-align:center;">${order.columns.siteName }</h2>
			...
			<table class="printTable"style="margin-top:5px">
				...
				<tr>
					...
					<td style="width: 33%;"  colspan="2">
						联系方式：
						<c:set var="cmobile" value="false"></c:set>
						<c:if test="${!empty order.columns.customer_mobile}">
							${order.columns.customer_mobile}
							<c:set var="cmobile" value="true"></c:set>
						</c:if>
						<c:if test="${!cmobile and !empty order.columns.customer_telephone}">
							${order.columns.customer_telephone}
							<c:set var="cmobile" value="true"></c:set>
						</c:if>
						<c:if test="${!cmobile and !empty order.columns.customer_telephone2}">
							${order.columns.customer_telephone2}
							<c:set var="cmobile" value="true"></c:set>
						</c:if>
					</td>
				...
				<tr>
							...
							<c:if test="${order.columns.province eq order.columns.city}">
								<td colspan="2">详细地址：${order.columns.city}${order.columns.area}${order.columns.customer_address}</td>
							</c:if>
							<c:if test="${order.columns.province ne order.columns.city}">
								<td colspan="2">详细地址：${order.columns.province}${order.columns.city}${order.columns.area}${order.columns.customer_address}</td>
							</c:if>
			...
			<table class="printTable" style="margin-top:5px">
				...
				<tr>
					<td style="width: 33%;">服务费：<c:if test="${order.columns.serve_cost != '0.0' && order.columns.serve_cost != '' }">${order.columns.serve_cost }</c:if></td>
					<td style="width: 33%;">材料费: <c:if test="${order.columns.auxiliary_cost != '0.0' && order.columns.auxiliary_cost != ''}">${order.columns.auxiliary_cost }</c:if></td>
					<td style="width: 34%;">延保费: <c:if test="${order.columns.warranty_cost != '0.0' && order.columns.warranty_cost != '' }">${order.columns.warranty_cost }</c:if></td>
				</tr>
				<tr>
					<td>合计金额：<c:if test="${order.columns.confirm_cost != '0.0' && order.columns.confirm_cost != ''  }">${order.columns.confirm_cost }</c:if></td>
					<td colspan="2">备注：</td>
				</tr>
				<tr>
				...
				<tr>
					<td >服务商：${order.columns.siteName}</td>
					<td>监督电话：${order.columns.sms_phone}</td>
					<td >服务工程师：${order.columns.employeName}</td>
				</tr>
			</table>
		</div>
		 <c:if test="${!stat.last}" >
