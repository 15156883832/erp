...
public class SiteMsgService extends BaseService {
	...
	public String registerSign(Map<String, Object> map, String msgCode) {
		...
		//注册新网点的时候添加关系表(_00中)
		Db.update(" insert into crm_site_order_map (id, site_id, suffix, create_time, remark) values (?, ?, ?, ?, ?) ",
				IdGen.uuid(), site.getId(), "_00", dt, "");
    ...
    //获取平台商品的html内容
    public String getPlatformGoodsHtmlById(String goodId){
        Record rd = Db.findFirst("select * from crm_goods_platform a where a.id='"+goodId+"'");
        if(rd!=null){
            //return rd.getStr("all_html");
            return rd.getStr("html");
        }else{
