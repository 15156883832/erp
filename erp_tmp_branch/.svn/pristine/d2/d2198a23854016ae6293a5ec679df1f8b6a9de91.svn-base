		...
		<div class="page-orderWait goodsPage">
			...
			<div class="mb-10">
				<table class="table table-bg table-border table-bordered table-sdrk text-c">
					...
					<tr>
					 <c:forEach var="orderDetails" items="${goods}">
                    <tr>
                            <td class="text-l">
							<div class="imgWrap pos-r pl-90 lh-24 ml-10" style="min-height: 80px;">
								<img src="${commonStaticImgPath}${orderDetails.columns.good_icon }" class="pos" />
								<p class="f-14">${orderDetails.columns.good_name }</p>
								<p class="f-12">商品类别：${orderDetails.columns.good_category }</p>
								<p>商品型号：${orderDetails.columns.good_model }</p>
							</div>
						</td>
                        <td>${orderDetails.columns.good_number }</td>
			...
			<div class="cl">
				<div class="col-2-1 f-l">
					<div class="lh-26 mb-5">
						<strong class="f-14 w-70 text-r">订单信息:</strong>
						<c:if test="${mark == 'zy' }">
							<c:if test="${detail.columns.status == '2' || detail.columns.status == '3' }">
								<sfTags:pagePermission authFlag="GOODSMGM_GOODSORDERMSG_SELFGOODS_EDITORDER_BTN"
									html='<a id="editOrderDetail"><i class="sficon sficon-edit"></i><span class="c-0383dc">修改</span></a>'></sfTags:pagePermission>
							</c:if>
						</c:if>
					</div>
					...
					<c:if test="${detail.columns.status == '2' || detail.columns.status == '3' }">
						<div class="lh-26 mb-5 pl-70 pos-r">
							...
							<c:if test="${detail.columns.zfId!=null && detail.columns.zfId!='' }">
								<span class="lh-26">
									&nbsp;（无现金收款：
									<span class="c-fd6e32 va-t">
										<c:if test="${detail.columns.zfType=='0' }">支付宝</c:if>
										<c:if test="${detail.columns.zfType=='1' }">微信</c:if>
									</span>${detail.columns.zfMoney }）</span>
								<a class="c-0383dc proofImg">
									<i class="sficon sficon-view"></i>
									查看凭证
									<img src="${commonStaticImgPath}${detail.columns.zfImg}" />
								</a>
							</c:if>
					...
					<div class="lh-26 mb-5 pl-70 pos-r">
						...
						<p class="">
							<fmt:formatDate value="${detail.columns.placing_order_time }" pattern="yyyy-MM-dd HH:mm" />
						</p>
					</div>
					<!-- 别删 -->
					<c:if test="${detail.columns.status == '2' || detail.columns.status == '3' }">
						<div class="lh-26 mb-5 pos-r pl-70">
							...
							<c:forEach items="${list }" var="lt1">
								<p>
									<span> ${lt1.columns.salesman }</span>
									&nbsp;&nbsp;&nbsp;
									<strong> ${lt1.columns.sales_commissions }元</strong>
								</p>
							</c:forEach>
						</div>
					</c:if>
					<c:choose>
						<c:when test="${detail.columns.status != '2' && detail.columns.status != '3' && detail.columns.status != '0'  }"> 
							<c:if test="${detail.columns.outstock_type!='3' }">
	       						<div class="lh-26 mb-5 pl-70 pos-r ">
				...
				<div class="col-2-1 f-l">
					<div class="lh-26 mb-5">
						<strong class="f-14 text-r">收货人信息:</strong>
					</div>
		...
		<div class="popupContainer ">
			<div class="popupMain pt-25 pb-20 pl-30 pr-30">
				...
				<div class="cl mb-10">
					...
					<div id="ifHasWxjsh" class="f-l"></div>
				</div>
				<div class="cl mb-10">
				...
				<div class="bg-e8f2fa pt-15 pb-5 cl mb-15 xsEmployesss ">
					...
					<div class="f-l w-600" id="xsEmployes"></div>
				</div>
