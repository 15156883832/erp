...
public class EmployeFittingDao extends BaseDao<EmployeFitting> {
	...
	public EmployeFitting getEmpId(String empId, String siteId, String fittingId) {
		...
		if (rd != null) {
			...
			efi = getEmpFittingId(empId,fittingId,siteId);
		}

		return efi;
	}

	public EmployeFitting getEmpFittingId(String empId, String fittingId,String siteId) {
		return getByHql("from EmployeFitting a where a.employeId =:p1 and a.fittingId=:p2 and a.status=:p3 and a.siteId=:p4", new Parameter(empId, fittingId, "1",siteId));
	}
	...
	public void doDFH(String fittingId, double num, String employeId) {
		SqlKit kit = new SqlKit().append("update crm_employe_fitting").append("set cjnum=cjnum+:num").append(",number=number-:num1,newest_keep_time ='"+DateUtils.getDateTime()+"'")
	...
	public void BjEmpFit(String code, String empId, String num) {// 工程师id,数量
		logger.error("EmployeFittingDao BjEmpFit emp found,code ="+code+"empId=" + empId +",num:"+num+ ";time"+ DateUtils.getDate());
	...
	public int updateEmpFittingWarning(String empFittingId, double amount) {
		...
		SqlKit kit = new SqlKit().append("update crm_employe_fitting").append("set `warning`=`warning`" + op1 + amount).append(",`cjnum`=`cjnum`" + op2 + amount)
				.append(",newest_keep_time ='"+DateUtils.getDateTime()+"' where id=:id and `status`='1'");
