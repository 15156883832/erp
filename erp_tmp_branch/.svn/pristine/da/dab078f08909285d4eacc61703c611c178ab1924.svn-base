...
<div class="popupBox bjsdrkbox" style="width: 1130px;">
    ...
    <div class="popupContainer pos-r">
        ...
        <div class="text-c btnWrap">
            ...
            <a href="javascript:doSdrk(1);" class="sfbtn sfbtn-opt3 w-70 mr-5">保存</a>
            <a href="javascript:doSdrk(2);" class="sfbtn sfbtn-opt w-80 mr-5">保存并打印</a>
...
<form action="${ctx}/printFitting/printappStocklist" id="printfittingForm" target="_blank" method="post">
...
<script type="text/javascript">
    function sfprint(){
        ...
        var str = "";
        var printJson = {};
        printJson.template = 'BJDY_1';
        var printItems = [];
        printJson.items = printItems;
        printJson.userName = '${fns:getUserName()}';
        printJson.empName = employeName;
        ...
        $("tr[name='sdrk_tr']").each(function (index) {
            ...
            if(!isBlank(id)){
                var pitem = {};
                pitem.id=id;
                pitem.num = isBlank(num) ? "''" : num;
                pitem.remarks = isBlank(remarks) ? "''" : remarks;
                printItems.push(pitem);
            }
        });

        var jsonStr = JSON.stringify(printJson);
      
       // $("#p-json").val(jsonStr);
        $("#fittingappId").val(jsonStr);
        $("#pf1").submit();
    }
    ...
    function doSdrk(type) {
        ...
        $("tr[name='sdrk_tr']").each(function (index) {
            ...
            str = str + id + "," + num + "," + orderNumber + "," + remarks + "-";
	            if(isBlank(fittingId)){
	            	fittingId = id;
	            }else{
	            	fittingId = fittingId + "," +id;
	            }
        });
        if (!check) {
        } else {
            ...
            $.post("${ctx}/fitting/fittingApply/saveFittingApplyBatch", {"data": str,"employeId":employeId,"employeName":employeName}, function (data) {
            	if (data.noInventory) {
                    ...
                    manuallyStockPosted = false;
                    window.parent.reloadPage();
            		 $("#fittingappId").val(data.fittingApplyId);
            	 if(type=='2'){
                 	prinfitting();
                 }else{
                    $.closeDiv($(".bjsdrkbox"));
